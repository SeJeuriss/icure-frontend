        ...
        <template is="dom-if" if="[[messages]]">
            <div id="main-details-panel" class="details-panel" on-dragover="_onDrag">
                <div class="action-buttons">
                    <div class="buttons">
                        <template is="dom-if" if="[[ehealthSession]]">
                            <template is="dom-if" if="[[!isSentbox]]">
                                <paper-button class="msg-detail-btn" on-tap="_replyMsgs" msgid\$="[[msg.id]]">
                                    <iron-icon id="reply-msg" icon="reply"></iron-icon>
                                    <span class="nomobile-wide">[[localize('reply','Reply',language)]]</span>
                                    <paper-tooltip class="wide-only" for="reply-msg">
                                        [[localize('reply','Reply',language)]]
                                    </paper-tooltip>
                                </paper-button>
                            </template>
                        ...
                        <template is="dom-if" if="[[isHidden]]">
                            <paper-button class="msg-detail-btn" on-tap="_restoreMsgs">
                                <iron-icon id="visibility-msg" icon="visibility"></iron-icon>
                                <span class="nomobile-wide">[[localize('restore','Restore',language)]]</span>
                                <paper-tooltip class="wide-only" for="visibility-msg">
                                    [[localize('restore','Restore',language)]]
                                </paper-tooltip>
                            </paper-button>
                        </template>
                        <template is="dom-if" if="[[!isHidden]]">
                            <template is="dom-if" if="[[!isDeleted]]">
                                <paper-button class="msg-detail-btn" on-tap="_hideMsg">
                                    <iron-icon id="visibility-off-msg" icon="visibility-off"></iron-icon>
                                    <span class="nomobile-wide">[[localize('hid_msg','Hide message',language)]]</span>
                                    <paper-tooltip class="wide-only" for="visibility-off-msg">[[localize('hid_msg','Hide
                                        message',language)]]
                                    </paper-tooltip>
                                </paper-button>
                            </template>
                        ...
                        <div class="del-button">
                            <template is="dom-if" if="[[isDeleted]]">
                                <paper-button class="msg-detail-btn" on-tap="_restoreMsgs">
                                    <iron-icon id="undo-msg" icon="undo"></iron-icon>
                                    <span class="nomobile-wide">[[localize('restore','Restore',language)]]</span>
                                    <paper-tooltip class="wide-only" for="undo-msg">
                                        [[localize('restore','Restore',language)]]
                                    </paper-tooltip>
                                </paper-button>
                            ...
                            <template is="dom-if" if="[[!isDeleted]]">
                                ...
                                <paper-button class="msg-detail-btn" on-tap="_msgToBin" msgid\$="[[msg.id]]">
                                    <iron-icon id="delete-msg" icon="delete"></iron-icon>
                                    <span class="nomobile-wide">[[localize('del','Delete',language)]]</span>
                                    <paper-tooltip class="wide-only" for="delete-msg">
                    ...
                    <div>
                        <paper-button class="msg-detail-btn" on-tap="close">
                ...
                <template is="dom-repeat" items="[[messages]]" as="msg">
                    <paper-card class="msg-detail-card">
                        ...
                        <template is="dom-if" if="[[msg.patientInss]]">
                            <div class="patient">[[localize('inv_pat','Patient',language)]] :
                                [[_getPatientDataByInss(msg.patientInss)]]
                            </div>
                        </template>
                    </paper-card>
                </template>
                ...
                <template is="dom-repeat" items="[[_sorted(selectedDocuments)]]">
                    <template is="dom-if" if="[[_isEqualTo(item.documentLocation,'body')]]">
                        <div class="layout-vertical annexe-card">
                            <dynamic-doc api="[[api]]" user="[[user]]" patient="[[patient]]" preview="true" document-id="[[item.id]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" title="[[localize('doc_body','Message Body')]]" mailbody="true" downloadable="false"></dynamic-doc>
                        </div>
                    </template>
                    <template is="dom-if" if="[[!_isEqualTo(item.documentLocation,'body')]]">
                        <div class="layout-vertical annexe-card">
                            <dynamic-doc id="[[item.id]]" api="[[api]]" user="[[user]]" patient="[[patient]]" document-id="[[item.id]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" title="[[item.label]]" downloadable="true" on-open-suggestions="openSuggestions" unassigned="[[item.unimportedPats]]"></dynamic-doc>
                        </div>
                    </template>
                </template>
                ...
                <template is="dom-if" if="[[assignModal]]">
                    <div id="import-suggest">
                        <div class="line">
                            <h4>[[localize('set_attach','Set attachment',language)]]</h4>
                            <ht-spinner class="patlist-spinner" active="[[isLoadingPatsSuggest]]"></ht-spinner>
                        </div>
                        <template is="dom-if" if="[[documentBeingAssigned.unimportedPats.length]]">
                            <div class="twocol">
                                            ...
                                            <template is="dom-repeat" items="[[documentBeingAssigned.unimportedPats]]">
                                                <paper-item class="unassigned pointer">
                                                    <div class="line">
                                                        ...
                                                        <div class="grow">
                                                            <div class="pat-name">[[item.lastName]] [[item.firstName]]
                                                                [[item.dateOfBirtth]]
                                                            </div>
                                                            <small class="labo-info labo-name"><b>[[item.labo]]</b>
                                                            </small>
                                                            <small>[[localize('prot','Protocol',language)]]:
                                                                [[item.protocol]]
                                                            </small>
                                                            <small>[[localize('date','Date',language)]] :
                                                                [[item.formatedDemandDate]]
                                                            </small>
                                                        </div>
                                                    </div>
                                                </paper-item>
                                ...
                                <div>
                                    <h5>[[patsLng]] [[localize('found_matches','Found matches',language)]]
                                        [[selectName]]</h5>
                                    <div class="scrollbox patlist">
                                        <template is="dom-if" if="[[!manualAssign]]">
                                            <paper-button class="search-patients" on-tap="toggleManualAssign">
                                                <iron-icon icon="perm-identity"></iron-icon>
                                                [[localize('sch_pats','Search patients',language)]]
                                            </paper-button>
                                        </template>
                                        <template is="dom-if" if="[[manualAssign]]">
                                            <div>
                                                <div class="line">
                                                    <paper-input class="search-patients" value="{{searchPatValue}}" label="[[localize('pati','Patient',language)]]"></paper-input>
                                                    <iron-icon class="search-patients" icon="close" on-tap="toggleManualAssign"></iron-icon>
                                                </div>
                                            </div>
                                        </template>
                                        <template is="dom-repeat" items="[[toImportPatsSuggestions]]">
                                            <paper-checkbox class="checkbox-item" on-iron-change="_getPatsFromCheckbox" data-item\$="[[item]]"><b>[[item.lastName]]
                                                [[item.firstName]]</b><br>[[localize('dat_of_bir','Date of
                                                birth',language)]]: [[item.dateOfBirth]]<br>SSIN: [[item.ssin]]
                                            </paper-checkbox>
                                        </template>
                        ...
                        <template is="dom-if" if="[[!documentBeingAssigned.unimportedPats.length]]">
                            <div>
                                <h5>[[patsLng]] [[localize('poss_matches','Possible matches',language)]]
                                    [[selectName]]</h5>
                                <div class="scrollbox patlist">
                                    <template is="dom-if" if="[[!manualAssign]]">
                                        <paper-button class="search-patients" on-tap="toggleManualAssign">
                                    ...
                                    <template is="dom-if" if="[[manualAssign]]">
                                        <div>
                                            <div class="line">
                                                <paper-input class="search-patients" value="{{searchPatValue}}" label="[[localize('pati','Patient',language)]]"></paper-input>
                                                <iron-icon class="search-patients" icon="close" on-tap="toggleManualAssign" disabled="[[isLoadingPatsSuggest]]"></iron-icon>
                                            </div>
                                        </div>
                                    </template>
                                    <template is="dom-repeat" items="[[toImportPatsSuggestions]]">
                        ...
                        <div class="line save-line">
                            ...
                            <template is="dom-if" if="[[canAssign]]">
                                <paper-button id="patients-importBtn" class="confirmPats" on-tap="_assignToPat">
                                    [[localize('set','Assign',language)]]
                                    <iron-icon id="patients-icon" icon="check"></iron-icon>
                                </paper-button>
                            </template>
                        </div>
                    </div>
...
`;
                      ...
                      if (doc && doc.id && doc.id.length) {
                          const svc = this.api.contact().service().newInstance(this.user, {
                              content: _.fromPairs([[this.language, {
                                  documentId: doc.id,
                                  stringValue: doc.name || 'annex-' + this.api.moment(Date.now()).format('YYYYMMDD')
                              }]]), label: 'document'
                          });
  ...
  _forwardMsgs(e) {
      if (this.docTxt != null) {
          ...
          target.dispatchEvent(new CustomEvent('fw-msg', {
              detail: {
                  bubbles: true,
                  composed: true,
                  params: msgToFw
              ...
              .then(paginated => {
                  if (paginated.rows && paginated.rows[0]) {
                      ...
                      if (hcp.name && !hcp.name.startsWith('Noreply') || hcp.ssin !== "12345678912") {
                          ...
                          const target = this.parentElement.parentNode.querySelector('ht-msg-menu')
                          target.dispatchEvent(new CustomEvent('re-msg', {
                              detail: {
                                  bubbles: true,
                                  composed: true,
                                  params: msgToRe
