    ...
    return html`
        <custom-style>
            <style include="shared-styles vaadin-icure-theme spinner-style">
                ...
                *.txtcolor--purpleStatus {
                ...
                #messagesGridContainer, #messagesGridContainer2 {
                    overflow-y: auto;
                }
        ...
        <div class="invoice-panel">
            ...
            <div style="display:none" id="_DEVELOPERS_ONLY_deleteFlatrateMessagesAndInvoices_container">
                <paper-button class="tool-btn" on-tap="_DEVELOPERS_ONLY_deleteFlatrateMessagesAndInvoices">
                    <iron-icon icon="icons:error" class="w30px h30px"></iron-icon> &nbsp; DELETE ALL INVOICES - NOT FOR
                    PROD !!!
                </paper-button>
            </div>
                    ...
                    <div id="loadingContentContainer">
                        ...
                        <div id="loadingContent">
                            <p>
                                ...
                                [[localize("mhListing.spinner.step_1", language)]]
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div id="batchStatus" class="batch-status">
                ...
                <template is="dom-if" if="[[_isEqual(flatrateMenuSection,'j20_toBeCorrected')]]"><span class="batchNumber j20_batchNumber batchToBeCorrected">[[messagesCachedData.countByStatus.error]]</span>
                </template>
                <template is="dom-if" if="[[_isEqual(flatrateMenuSection,'j20_toBeSend')]]"><span class="batchNumber j20_batchNumber batchToBeSend">+
                    <!--[[messagesCachedData.countByStatus.xxx]]]--></span></template>
                <template is="dom-if" if="[[_isEqual(flatrateMenuSection,'j20_process')]]"><span class="batchNumber j20_batchNumber batchProcessed">[[messagesCachedData.countByStatus.pending]]</span>
                </template>
                <template is="dom-if" if="[[_isEqual(flatrateMenuSection,'j20_partiallyAccepted')]]"><span class="batchNumber j20_batchNumber batchToBeSend">[[messagesCachedData.countByStatus.partiallyAccepted]]</span>
                </template>
                <template is="dom-if" if="[[_isEqual(flatrateMenuSection,'j20_accept')]]"><span class="batchNumber j20_batchNumber batchAccepted">[[messagesCachedData.countByStatus.fullyAccepted]]</span>
                </template>
                <template is="dom-if" if="[[_isEqual(flatrateMenuSection,'j20_reject')]]"><span class="batchNumber j20_batchNumber batchRejected">[[messagesCachedData.countByStatus.rejected]]</span>
                </template>
                <template is="dom-if" if="[[_isEqual(flatrateMenuSection,'j20_archive')]]"><span class="batchNumber j20_batchNumber batchArchived">[[messagesCachedData.countByStatus.archived]]</span>
                </template>
            </div>


            <template is="dom-if" if="[[_isEqual(flatrateMenuSection, 'j3')]]">
                <template is="dom-if" if="[[_showWarningMessageEarlyInvoicingListing()]]">
                    <div class="warningMessage" id="warningMessageListing">
                        <div class="warningMessageBody">
                            <iron-icon icon="icons:warning" class="w30px h30px"></iron-icon> &nbsp;
                            [[localize('earlyInvoicingWarningMessage', 'Attention\\, veuillez ne procéder à la
                            facturation qu\\'entre le premier et le cinquième jour du mois.', language)]]
                        </div>
                    </div>
                </template>
                <div class="centerCenter">
                    <div class="exportMonthPicker">
                        <div class="exportMonthPickerTitle">
                            <iron-icon icon="vaadin:calendar" style="max-width:20px; max-height:20px; margin-right:7px;"></iron-icon>
                            [[localize('j20_monthToGenerate','Month to generate',language)]]
                        </div>
                    ...
                    <paper-button class="tool-btn p-35px m-t-20 f-s-1em bordered" on-tap="_getListingJ3">
                        <iron-icon icon="icons:cloud-download" class="w30px h30px"></iron-icon> &nbsp;
                        [[localize('downloadListing','Téléchargement listing',language)]] (*)
                    </paper-button>
            ...
            <template is="dom-if" if="[[_isEqual(flatrateMenuSection, 'archivej3')]]">
                ...
                <div id="rightPanelToggleContainer" class="">
                    <div class="header">
                        <paper-button class="tool-btn p-r-15px p-l-15px w-100-pc" on-tap="_toggleRightPanel">
                            <iron-icon icon="icons:build" class="w20px h20px"></iron-icon> &nbsp; [[localize('tools',
                            language)]]
                        </paper-button>
                    </div>
                </div>
                ...
                <div id="rightPanel" class="">
                    <div class="header" on-tap="_toggleRightPanel">
                        <label>
                            <iron-icon icon="icons:build" class="w20px h20px"></iron-icon> &nbsp;[[localize('tools',
                            language)]]</label>
                        <paper-button class="pullRightIcon">
                            <iron-icon icon="icons:arrow-forward"></iron-icon>
                        </paper-button>
                    </div>
                    <div class="body">
                        <h4 class="m-t-50">
                            <iron-icon icon="date-range"></iron-icon> &nbsp; [[localize('filterByDate','Filtrer par
                            date',language)]]
                        </h4>
                        ...
                        <paper-button class="tool-btn p-r-15px p-l-15px m-t-20 w-100-pc" on-tap="_getListingArchives">
                            <iron-icon icon="icons:update" class="force-left"></iron-icon> &nbsp;
                            [[localize('filterResults','Filtrer les résultats',language)]]
                        </paper-button>
                    </div>
                </div>
                                ...
                                <div class="containScroll">
                                    <vaadin-grid id="noscroll" items="[[archiveListingMessages]]">
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="35%" class="">
                                            <template class="header">
                                                <vaadin-grid-sorter path="filename">
                                                    [[localize('filename','Fichier',language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                            <template>
                                                <iron-icon icon="image:picture-as-pdf" class="force-left textRed"></iron-icon> &nbsp;
                                                [[item.filename]]
                                            </template>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column flex-grow="0" width="15%" class="">
                                            <template class="header">
                                                <vaadin-grid-sorter path="totalPages">[[localize('totalPages','Nombre de
                                                    pages',language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                        ...
                                        <vaadin-grid-column flex-grow="1" class="">
                                            <template class="header">
                                                <vaadin-grid-sorter path="id">[[localize('downloadArchive',"Télécharger
                                                    l'archive",language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                            <template>
                                                <paper-button class="tool-btn p-r-15px p-l-15px downloadListingArchive" on-tap="_downloadListingArchive" messageid="[[item.id]]">
                                                    <iron-icon icon="icons:cloud-download" class="force-left"></iron-icon> &nbsp;
                                                    [[localize('download','Télécharger',language)]]
                                                </paper-button>
                                                </template>
                                        </vaadin-grid-column>
                                    </vaadin-grid>
                            ...
                            <div class="scrollBox">
                                <vaadin-grid id="messagesGrid" items="[[messagesGridData]]" data-status\$="[[_getBatchStatusByMenuSection(flatrateMenuSection)]]" class="invoicesToBeCorrectedGrid">
                                    ...
                                    <vaadin-grid-column flex-grow="0" width="15%" class="recipient-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="oaLabel">[[localize('name','Name',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                    ...
                                    <vaadin-grid-column flex-grow="0" width="5%" class="recipient-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="invoiceData.invoiceReference">N° fact.
                                            </vaadin-grid-sorter>
                                        </template>
                                    ...
                                    <vaadin-grid-column flex-grow="0" width="17%" class="recipient-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="patientData.firstName">
                                                [[localize('inv_pat','Patient',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                        <template>[[item.patientData.firstName]] [[item.patientData.lastName]]
                                        </template>
                                    </vaadin-grid-column>
                                    <vaadin-grid-column flex-grow="0" width="8%" class="recipient-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="patientData.ssin">
                                                [[localize('inv_niss','Niss',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                    ...
                                    <vaadin-grid-column flex-grow="0" width="9%" class="recipient-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="invoiceData.invoicingCodes">Nmcl
                                            </vaadin-grid-sorter>
                                        </template>
                                    ...
                                    <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="invoiceData.invoiceDate">Date presta.
                                            </vaadin-grid-sorter>
                                        </template>
                                    ...
                                    <vaadin-grid-column-group>
                                        <vaadin-grid-column flex-grow="0" width="5%" class="recipient-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="invoicedAmount">
                                                    [[localize('inv_batch_amount','Amount',language)]]<br>[[localize('inv_batch_amount_invoiced','Invoiced',language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                            <template><span class\$="invoice-status [[_getTxtStatusColor(item.invoiceStatusHr,item.invoicedAmount)]]">[[item.invoicedAmount]]€</span>
                                            </template>
                                            <template class="footer">
                                                [[_invoicesToBeCorrectedInvoicedAmount(flatrateMenuSection,
                                                'flatFiles')]]€
                                            </template>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column flex-grow="0" width="5%" class="recipient-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="refusedAmount">
                                                    [[localize('inv_batch_amount','Amount',language)]]<br>[[localize('inv_batch_amount_rej','Rejected',language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                            <template><span class\$="invoice-status [[_getTxtStatusColor('force-red',item.refusedAmount)]]">[[item.refusedAmount]]€</span>
                                            </template>
                                            <template class="footer">
                                                [[_invoicesToBeCorrectedRefusedAmount(flatrateMenuSection,
                                                'flatFiles')]]€
                                            </template>
                                        </vaadin-grid-column>
                                    </vaadin-grid-column-group>
                                    <vaadin-grid-column flex-grow="0" width="5%" class="recipient-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="invoiceStatusHr">
                                                [[localize('inv_stat','Status',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                        <template>
                                    ...
                                    <vaadin-grid-column flex-grow="1" class="stat-col">
                                        <template>
                                            <paper-button class="grid-btn-small modal-button-accepted" on-tap="_flagInvoiceAsCorrected" data-invoice-id\$="[[item.invoiceData.id]]">
                                                <iron-icon icon="check-circle" class="force-left" data-message-id\$="[[item.id]]" data-invoice-id\$="[[item.invoiceData.id]]"></iron-icon>
                                                &nbsp; [[localize('invoiceGotCorrected',"I corrected the
                                                invoice",language)]]
                                            </paper-button>
                                        </template>
                                    </vaadin-grid-column>
                                    <vaadin-grid-column flex-grow="1" class="stat-col">
                                        <template>
                                            <paper-button class="grid-btn-small modal-button-rejected" on-tap="_flagInvoiceAsLostConfirmationDialog" data-invoice-id\$="[[item.invoiceData.id]]">
                                                <iron-icon icon="error" class="force-left" data-message-id\$="[[item.id]]" data-invoice-id\$="[[item.invoiceData.id]]"></iron-icon>
                                                &nbsp; [[localize('invoiceIsUnrecoverable',"Unrecoverable
                                                invoice",language)]]
                                            </paper-button>
                                        </template>
                                    </vaadin-grid-column>
            ...
            <template is="dom-if" if="[[_isEqual(flatrateMenuSection, 'j20_toBeSend')]]">
                <template is="dom-if" if="[[_showWarningMessageEarlyInvoicingDownload()]]">
                    <div class="warningMessage" id="warningMessageInvoicing">
                        <div class="warningMessageBody">
                            <iron-icon icon="icons:warning" class="w30px h30px"></iron-icon> &nbsp;
                            [[localize('earlyInvoicingDownloadWarningMessage', 'Please only download invoicing at the
                            end of the month', language)]]
                        </div>
                    </div>
                </template>
                <div class="textAlignCenter">
                    ...
                    <paper-button class="tool-btn p-15px f-s-1em bordered" on-tap="_exportFlatRateInvoicing">
                        <iron-icon icon="icons:cloud-download" class="w30px h30px"></iron-icon> &nbsp;
                        [[localize('invoicingExport','Télécharger la facturation',language)]]
                    </paper-button>
                </div>
            </template>
            ...
            <template is="dom-if" if="[[_isIn(flatrateMenuSection, 'j20_process\\,j20_accept\\,j20_partiallyAccepted\\,j20_reject\\,j20_archive')]]">
                <div class="invoicesGridContainer">
                    <div class="invoiceContainer">
                        ...
                        <div id="messagesGridContainer" class="invoiceSubContainerMiddle">
                            <div class="scrollBox">
                                <vaadin-grid id="messagesGrid" items="[[messagesGridData]]" data-status\$="[[_getBatchStatusByMenuSection(flatrateMenuSection)]]" active-item="{{activeGridItem}}" on-tap="_toggleBatchDetails">
                                    <vaadin-grid-column flex-grow="0" width="4%" class="oa-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="messageInfo.parentOaCode">
                                                [[localize('inv_oa','Oa',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                    ...
                                    <vaadin-grid-column flex-grow="0" width="24%" class="ref-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="messageInfo.parentOaLabel">
                                    ...
                                    <vaadin-grid-column flex-grow="0" width="8%" class="invoice-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="messageInfo.batchReference">
                                                [[localize('inv_batch_num','Batch reference',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                    ...
                                    <vaadin-grid-column flex-grow="0" width="8%" class="month-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="messageInfo.invoicedMonth">
                                                [[localize('inv_batch_month','Billed month',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                    ...
                                    <vaadin-grid-column flex-grow="0" width="10%" class="invoiceDate-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="messageInfo.generationDate">
                                                [[localize('inv_date_fact','Invoice date',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                    ...
                                    <vaadin-grid-column flex-grow="0" width="9%" class="invAmount-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="messageInfo.invoicedAmount">
                                                [[localize('inv_batch_amount','Amount',language)]]<br>[[localize('inv_batch_amount_invoiced','Invoiced',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                        <template><span class\$="invoice-status [[_getTxtStatusColor(item.messageInfo.invoiceStatusHr,item.messageInfo.invoicedAmount)]]">[[item.messageInfo.invoicedAmount]]€</span>
                                        </template>
                                        <template class="footer">[[_invoicedAmount(flatrateMenuSection,
                                            'flatFiles')]]€
                                        </template>
                                    </vaadin-grid-column>
                                    <vaadin-grid-column flex-grow="0" width="9%" class="accAmount-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="messageInfo.acceptedAmount">
                                                [[localize('inv_batch_amount','Amount',language)]]<br>[[localize('inv_batch_amount_acc','Accepted',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                        <template><span class\$="[[_getTxtStatusColor('force-green',item.messageInfo.acceptedAmount)]]">[[item.messageInfo.acceptedAmount]]€</span>
                                        </template>
                                        <template class="footer">[[_acceptedAmount(flatrateMenuSection,
                                            'flatFiles')]]€
                                        </template>
                                    </vaadin-grid-column>
                                    <vaadin-grid-column flex-grow="0" width="9%" class="refAmount-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="messageInfo.refusedAmount">
                                                [[localize('inv_batch_amount','Amount',language)]]<br>[[localize('inv_batch_amount_rej','Rejected',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                        <template><span class\$="[[_getTxtStatusColor('force-red',item.messageInfo.refusedAmount)]]">[[item.messageInfo.refusedAmount]]€</span>
                                        </template>
                                        <template class="footer">[[_rejectedAmount(flatrateMenuSection,
                                            'flatFiles')]]€
                                        </template>
                                    </vaadin-grid-column>
                                    <vaadin-grid-column flex-grow="0" width="9%" class="stat-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="messageInfo.invoiceStatusHr">
                                                [[localize('inv_stat','Status',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                        <template><span class\$="invoice-status [[_getIconStatusClass(item.messageInfo.invoiceStatusHr)]]"><iron-icon icon="vaadin:circle" class\$="statusIcon [[_getIconStatusClass(item.messageInfo.invoiceStatusHr)]]"></iron-icon> [[item.messageInfo.invoiceStatusHr]]</span>
                                        </template>
                                    </vaadin-grid-column>
                                    <vaadin-grid-column flex-grow="1" class="stat-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="id">
                                                [[localize('download',"Télécharger",language)]] PDF
                                            </vaadin-grid-sorter>
                                        </template>
                                        <template>
                                            <paper-button class="grid-btn-small noBg noPad" on-tap="_downloadParentOaPdf" data-message-id\$="[[item.id]]">
                                                <iron-icon icon="image:picture-as-pdf" class="force-left textRed" data-message-id\$="[[item.id]]"></iron-icon> &nbsp; PDF
                                            </paper-button>
                                        </template>
                                    </vaadin-grid-column>
                                    <vaadin-grid-column flex-grow="1" class="stat-col">
                                        ...
                                        <template>
                                            <paper-button class="grid-btn-small" on-tap="_downloadParentOaArchive" data-message-id\$="[[item.id]]">
                                                <iron-icon icon="icons:folder" class="force-left" data-message-id\$="[[item.id]]"></iron-icon> &nbsp; ZIP
                                            </paper-button>
                                        </template>
                                    </vaadin-grid-column>
                                </vaadin-grid>
                        ...
                        <div id="invoiceDetailContainer" class="invoiceDetailContainer">
                            <div id="invoiceDetailHeader" class="mb30">
                                <div class="actionButtonsRight">
                                    <template is="dom-if" if="[[_isEqual(flatrateMenuSection, 'j20_process')]]">
                                        <paper-button class="modal-button modal-button-accepted" on-tap="_acceptBatch">
                                            <iron-icon icon="check-circle"></iron-icon>
                                            [[localize('j20_accept','Accepted batches',language)]]
                                        </paper-button>
                                        <paper-button class="modal-button modal-button-partially-accepted" on-tap="_partiallyAcceptBatch">
                                            <iron-icon icon="settings-backup-restore"></iron-icon>
                                            [[localize('inv_par_acc','Partially accepted',language)]]
                                        </paper-button>
                                        <paper-button class="modal-button modal-button-rejected" on-tap="_rejectBatch">
                                            <iron-icon icon="cancel"></iron-icon>
                                            [[localize('j20_reject','Refused batches',language)]]
                                        </paper-button>
                                    </template>
                                    <template is="dom-if" if="[[_isIn(flatrateMenuSection, 'j20_accept\\,j20_partiallyAccepted\\,j20_reject')]]">
                                        <paper-button class="modal-button" on-tap="_openArchiveDialogForBatch">
                                            <iron-icon icon="markunread-mailbox"></iron-icon>
                                            [[localize('j20_archive_batch','Archive batch',language)]]
                                        </paper-button>
                                    </template>
                                </div>
                                <h4>[[localize('batchDetails',"Batch details",language)]]<span class="txtcolor--redStatus">[[activeGridItem.messageInfo.batchReference]]</span>
                                    [[localize('ofOa',"of OA",language)]] <span class="txtcolor--blueStatus">[[activeGridItem.messageInfo.parentOaCode]] - [[activeGridItem.messageInfo.parentOaLabel]]</span>
                                </h4>
                            </div>
                            <vaadin-grid id="invoiceAndBatchesGridDetail" items="[[messageDetailsData]]">
                                ...
                                <vaadin-grid-column-group>
                                    ...
                                    <vaadin-grid-column flex-grow="0" width="5%" class="recipient-col">
                                        <template class="header">
                                            <vaadin-grid-sorter path="acceptedAmount">
                                                [[localize('inv_batch_amount','Amount',language)]]<br>[[localize('inv_batch_amount_acc','Accepted',language)]]
                                            </vaadin-grid-sorter>
                                        </template>
                                        <template><span class\$="invoice-status [[_getTxtStatusColor('force-green',item.acceptedAmount)]]">[[item.acceptedAmount]]€</span>
                                        </template>
                                    </vaadin-grid-column>
                                    <vaadin-grid-column flex-grow="0" width="5%" class="recipient-col">
                                        <template class="header">
                                ...
                                <vaadin-grid-column flex-grow="1" class="stat-col">
                                    <template class="header">
                                        <vaadin-grid-sorter path="id">[[localize('download',"Télécharger",language)]]
                                            PDF
                                        </vaadin-grid-sorter>
                                    </template>
                                    <template>
                                        <paper-button class="grid-btn-small noBg noPad" on-tap="_downloadOaPdfByMessageId" data-message-id\$="[[item.id]]">
                                            <iron-icon icon="image:picture-as-pdf" class="force-left textRed" data-message-id\$="[[item.id]]"></iron-icon> &nbsp; PDF
                                        </paper-button>
                                    </template>
                                </vaadin-grid-column>
                                <vaadin-grid-column flex-grow="1" class="stat-col">
                                    <template>
                                        <paper-button class\$="grid-btn-small modal-button-rejected [[_showRejectInvoiceButton(item.messageOriginalStatus, item.invoiceFinalStatus)]]" on-tap="_rejectInvoice" data-message-id\$="[[item.id]]" data-invoice-id\$="[[item.invoiceData.id]]">
                                            <iron-icon icon="cancel" class="force-left" data-message-id\$="[[item.id]]" data-invoice-id\$="[[item.invoiceData.id]]"></iron-icon> &nbsp;
                                            [[localize('rejectVerb',"Reject",language)]]
                                        </paper-button>
                                    </template>
                                </vaadin-grid-column>
                            </vaadin-grid>
            ...
            <paper-dialog class="modalDialog" id="archiveDialog">
                ...
                <div class="modalDialogContent m-t-50">
                    <p>[[localize('archiveBatchConfirmation',"Voulez-vous vraiment archiver l'envoi",language)]] <b>[[activeGridItem.messageInfo.batchReference]]</b>
                    </p>
                    <p>OA <span class="txtcolor--blueStatus">[[activeGridItem.messageInfo.parentOaCode]]</span>
                        [[activeGridItem.messageInfo.parentOaLabel]] ?</p>
                </div>
                <div class="buttons">
                    <paper-button class="modal-button" dialog-dismiss="">
                        <iron-icon icon="icons:close" class="mr5 smallIcon"></iron-icon>
                        [[localize('can','Cancel',language)]]
                    </paper-button>
                    <paper-button class="modal-button" on-tap="_archiveBatch">
            ...
            <paper-dialog class="modalDialog" id="missingNihiiDialog" no-cancel-on-outside-click="" no-cancel-on-esc-key="">
                ...
                <div class="modalDialogContent m-t-50">
                    <p class="textAlignCenter">[[localize('incompleteUserProfile','Incomplete user
                        profile',language)]].</p>
                    <p class="textAlignCenter m-t-25">[[localize('provideNihiiNumber','Please provide your
                        number',language)]] <b>[[localize('inami','INAMI',language)]]</b>.</p>
                    <p class="textAlignCenter m-t-25 bold">
                        <iron-icon icon="communication:phone" class="mr5 smallIcon colorAppSecondaryColorDark"></iron-icon>
                        <a href="tel:+3223192241" class="textDecorationNone">+32(0)2/319.22.41</a> -
                        <iron-icon icon="icons:mail" class="mr5 smallIcon colorAppSecondaryColorDark"></iron-icon>
                        <a href="mailto:support@topaz.care" class="textDecorationNone">support@topaz.care</a>.
                    </p>
                </div>
                <div class="buttons">
                    <paper-button class="modal-button" on-tap="_closeDialogs">
                        <iron-icon icon="icons:close" class="mr5 smallIcon"></iron-icon>
                        [[localize('clo','Close',language)]]
                    </paper-button>
                    <paper-button class="modal-button" on-tap="_gotoMyProfileTab1">
                        <iron-icon icon="icons:settings" class="mr5 smallIcon"></iron-icon>
                        [[localize('configure','Configure',language)]]
                    </paper-button>
                </div>
            </paper-dialog>
            <paper-dialog class="modalDialog" id="missingMedicalHouseValorisations" no-cancel-on-outside-click="" no-cancel-on-esc-key="">
                ...
                <div class="modalDialogContent m-t-50">
                    <h3 class="textAlignCenter">[[localize('incompleteUserProfile','Incomplete user
                        profile',language)]].</h3>
                    <p class="textAlignCenter m-t-25">[[localize('provideMissingValorisations','Please provide your flat
                        rates',language)]].</p>
                    <p class="textAlignCenter m-t-25 bold">
                        <iron-icon icon="communication:phone" class="mr5 smallIcon colorAppSecondaryColorDark"></iron-icon>
                        <a href="tel:+3223192241" class="textDecorationNone">+32(0)2/319.22.41</a> -
                ...
                <div class="buttons">
                    ...
                    <paper-button class="modal-button" on-tap="_gotoMyAdmin">
                        <iron-icon icon="icons:settings" class="mr5 smallIcon"></iron-icon>
                        [[localize('configure','Configure',language)]]
                    </paper-button>
                </div>
            ...
            <paper-dialog class="modalDialog" id="noDataToExport" no-cancel-on-outside-click="" no-cancel-on-esc-key="">
                ...
                <div class="modalDialogContent m-t-50">
                    <h3 class="textAlignCenter">[[localize('noDataToExport','We could not find any data to
                        export',language)]].</h3>
                    <p class="textAlignCenter m-t-25 bold">
                        <iron-icon icon="communication:phone" class="mr5 smallIcon colorAppSecondaryColorDark"></iron-icon>
                        <a href="tel:+3223192241" class="textDecorationNone">+32(0)2/319.22.41</a> -
            ...
            <paper-dialog class="modalDialog" id="noHcpContactPerson" no-cancel-on-outside-click="" no-cancel-on-esc-key="">
                ...
                <div class="modalDialogContent m-t-50">
                    <h3 class="textAlignCenter">[[localize('incompleteUserProfile','Incomplete user
                        profile',language)]].</h3>
                    <p class="textAlignCenter m-t-25">[[localize('missingMhHcpContactPerson1','Please provide an
                        invoicing contact person',language)]].<br>[[localize('missingMhHcpContactPerson2','Required
                        information for Insurances',language)]].</p>
                    <p class="textAlignCenter m-t-25 bold">
                        <iron-icon icon="communication:phone" class="mr5 smallIcon colorAppSecondaryColorDark"></iron-icon>
                        <a href="tel:+3223192241" class="textDecorationNone">+32(0)2/319.22.41</a> -
                        <iron-icon icon="icons:mail" class="mr5 smallIcon colorAppSecondaryColorDark"></iron-icon>
            ...
            <paper-dialog class="modalDialog" id="noHcpBce" no-cancel-on-outside-click="" no-cancel-on-esc-key="">
                ...
                <div class="modalDialogContent m-t-50">
                    <h3 class="textAlignCenter">[[localize('incompleteUserProfile','Incomplete user
                        profile',language)]].</h3>
                    <p class="textAlignCenter m-t-25">[[localize('missingMhBce','Please provide a valid
                        BCE',language)]].</p>
                    <p class="textAlignCenter m-t-25 bold">
                        <iron-icon icon="communication:phone" class="mr5 smallIcon colorAppSecondaryColorDark"></iron-icon>
                        <a href="tel:+3223192241" class="textDecorationNone">+32(0)2/319.22.41</a> -
                ...
                <div class="buttons">
                    ...
                    <paper-button class="modal-button" on-tap="_gotoMyProfileTab2">
                        <iron-icon icon="icons:settings" class="mr5 smallIcon"></iron-icon>
                        [[localize('configure','Configure',language)]]
                    </paper-button>
                </div>
            ...
            <paper-dialog class="modalDialog" id="noHcpBankAccount" no-cancel-on-outside-click="" no-cancel-on-esc-key="">
                ...
                <div class="modalDialogContent m-t-50">
                    <h3 class="textAlignCenter">[[localize('incompleteUserProfile','Incomplete user
                        profile',language)]].</h3>
                    <p class="textAlignCenter m-t-25">[[localize('missingMhBankAccount','Please provide with a valid
                        bank account',language)]].</p>
                    <p class="textAlignCenter m-t-25 bold">
                        <iron-icon icon="communication:phone" class="mr5 smallIcon colorAppSecondaryColorDark"></iron-icon>
                        <a href="tel:+3223192241" class="textDecorationNone">+32(0)2/319.22.41</a> -
                        <iron-icon icon="icons:mail" class="mr5 smallIcon colorAppSecondaryColorDark"></iron-icon>
                ...
                <div class="buttons">
                    ...
                    <paper-button class="modal-button" on-tap="_gotoMyProfileTab3">
                        <iron-icon icon="icons:settings" class="mr5 smallIcon"></iron-icon>
                        [[localize('configure','Configure',language)]]
                    </paper-button>
                </div>
            ...
            <paper-dialog class="modalDialog" id="exportAlreadyRan" no-cancel-on-outside-click="" no-cancel-on-esc-key="">
                ...
                <div class="modalDialogContent m-t-50">
                    <h3 class="textAlignCenter">[[localize('flatRateInvoicingAlreadyRan','Incomplete user
                        profile',language)]].</h3>
                    <p class="textAlignCenter m-t-25">[[localize('getInTouchWithUsToUnlock','Please provide with a valid
                        bank account',language)]].</p>
                    <p class="textAlignCenter m-t-25 bold">
                        <iron-icon icon="communication:phone" class="mr5 smallIcon colorAppSecondaryColorDark"></iron-icon>
                        <a href="tel:+3223192241" class="textDecorationNone">+32(0)2/319.22.41</a> -
            ...
            <paper-dialog class="modalDialog" id="flagInvoiceAsLostConfirmationDialog" no-cancel-on-outside-click="" no-cancel-on-esc-key="">
                ...
                <div class="modalDialogContent m-t-50">
                    <h3 class="textAlignCenter">[[localize('areYouSureFlagInvoiceAsLost','Are you sure you wish to flag
                        invoice as permanently lost?',language)]]</h3>
                    <p class="textAlignCenter m-t-50 bold">[[localize('unrecoverableAction','This action is
                        unrecoverable',language)]].</p>
                </div>
                <div class="buttons">
                    <paper-button class="modal-button" on-tap="_closeDialogs">
                        <iron-icon icon="icons:close" class="mr5 smallIcon"></iron-icon>
                    ...
                    <paper-button class="modal-button" on-tap="_flagInvoiceAsLost">
...
`;
      ...
      return {
          ...
          invoiceMessageStatuses: {
              ...
              value: () => ({
                  ...
                  partiallyAccepted: {
                      status: (1 << 16),
                      menuSection: "j20_partiallyAccepted",
                      llKey: 'inv_par_acc'
                  },
  ...
  _flatRateListingMenuSectionChanged() {
      ...
      this.set('messagesGridData', null);

      if (this.flatrateMenuSection === "archivej3") {
          this._getListingArchives();
          return;
      }

      if (['j20_process', 'j20_reject', 'j20_accept', 'j20_partiallyAccepted', 'j20_archive', 'j20_toBeCorrected'].indexOf(_.trim(this.flatrateMenuSection)) > -1) {
          ...
          if (_.size(this.messagesCachedData.dataByStatus)) {
              ...
              const messagesGrid = this.root.querySelector('#messagesGrid');
              messagesGrid && messagesGrid.clearCache();
              const invGridDetail = this.root.querySelector('#invoiceAndBatchesGridDetail');
              invGridDetail && invGridDetail.clearCache();
  ...
  _gotoMyProfileTab1() {
      this._closeDialogs()
      this.dispatchEvent(new CustomEvent('trigger-open-my-profile', {
          bubbles: true,
          composed: true,
          detail: {tabIndex: 0}
      }));
  ...
  _getInsuranceTitularyInfo(inputPatientsList = false) {
      return new Promise(resolve => {
          const insuranceTitularyIds = _.compact(_.uniq(_.filter((inputPatientsList ? inputPatientsList : this.flatRateAllPatients), (i) => {
              return _.trim(_.get(i, "titularyId", ""))
          }).map(i => i.titularyId)));
          return !_.size(insuranceTitularyIds) ? resolve((inputPatientsList ? inputPatientsList : this.flatRateAllPatients)) : this.api.patient().getPatientsWithUser(this.user, new models.ListOfIdsDto({ids: insuranceTitularyIds})).then(results => {
              return resolve(
                  _.map((inputPatientsList ? inputPatientsList : this.flatRateAllPatients), (i => {
                      ...
                      let titularyRecord = _.head(_.filter(results, (j => {
                          return _.trim(j.id) === _.get(i, "titularyId", "")
                      })))
  ...
  _getListingJ3(e) {
      ...
      this._setLoadingMessage({
          message: this.localize('mhInvoicing.spinner.step_2', this.language),
          icon: "arrow-forward"
      });
      ...
      this._getPatientsByHcp(_.get(this.hcp, "id"))
          .then(hcpPat => {
              this._setLoadingMessage({
                  message: this.localize('mhListing.spinner.step_3_done', this.language),
                  icon: "check-circle",
              ...
              this._getInsuranceTitularyInfo().then(data => {
                  this._setLoadingMessage({
                      message: this.localize('mhListing.spinner.step_4', this.language),
                      icon: "arrow-forward"
                  });
  ...
  _setLoadingMessage(messageData) {
      if (messageData.updateLastMessage) {
      ...
      return `
          <table id="documentHeader" class="boderBottomGrey" cellspacing="0" cellpadding="0">
              <tbody>
                  <tr class="">
                      <td width="100%">
                          <span class="bold uppercase colorBlue fs12">` + this.localize("flatrate_" + (isInvoicingExport ? "invoicing" : "list") + "_" + listIndex + "_header", this.language) + `</span><br />
                          <span class="bold uppercase colorBlue fs10">` + this.localize("flatrate_" + (isInvoicingExport ? "invoicing" : "list") + "_" + listIndex + "_subheader", this.language) + `</span><br />
                      </td>
                  </tr>
              </tbody>
          </table>
                      ...
                      pl.rows = _
                          ...
                          .filter((i) => {
                              return (
                                  ...
                                  && !!parseInt(_.size(_.filter(_.get(i, "medicalHouseContracts", []), (i) => {
                                      return !!i && _.trim(_.get(i, "hcpId", "something")) === _.trim(_.get(this, "user.healthcarePartyId", "else"))
                                  })))
                              )
                          })
                          .uniqBy('ssin')
  ...
  _getInsurancesData() {
      ...
      this.api.insurance().getInsurances(new models.ListOfIdsDto({
          ids: _.compact(_.chain(this.flatRateAllPatients).map((i) => {
              return (i && !!_.get(i, "finalInsurability", false)) ? _.get(i, "finalInsurability.insuranceId", false) : false
          }).uniq().value())
      }))
          .then((ins) => {
              this.oaData = _
  ...
  _generateJ3FileAndDownload() {
      ...
      this._setLoadingMessage({
          message: this.localize('mhListing.spinner.step_4_done', this.language),
          icon: "check-circle",
          updateLastMessage: true,
          done: true
      });
      this._setLoadingMessage({
          message: this.localize('mhListing.spinner.step_5', this.language),
          icon: "arrow-forward"
      });
          ...
          pdfData[oa.id] = {
              ...
              entries: {
                  ...
                  list1: _.compact(_.map(OaPatients, pat => {
                      return parseInt(_.size(_.filter(_.get(pat, "medicalHouseContracts", []), mhc => {
                          return (
                  ...
                  // 03 : Unsubscriptions
                  list3: _.compact(_.map(OaPatients, pat => {
                      return parseInt(_.size(_.filter(_.get(pat, "medicalHouseContracts", []), mhc => {
                          return moment(_.get(mhc, "endOfContract", 0), 'YYYYMMDD').startOf('month').isSame(this.reportCurrentDateMomentObject.startOf('month'))
                      }))) ? pat : false
                  })),
      ...
      this._setLoadingMessage({
          message: this.localize('mhListing.spinner.step_5_done', this.language),
          icon: "check-circle",
          updateLastMessage: true,
          done: true
      });
      this._setLoadingMessage({
          message: this.localize('mhListing.spinner.step_6', this.language),
          icon: "arrow-forward"
      });
      ...
      this.api.pdfReport(mustache.render(pdfTemplate, pdfData))
          .then(({pdf: pdfFileContent, printed: printed}) => {
              this._setLoadingMessage({
                  message: this.localize('mhListing.spinner.step_6_done', this.language),
                  icon: "check-circle",
                  updateLastMessage: true,
                  done: true
              });
              this._setLoadingMessage({
                  message: this.localize('mhListing.spinner.step_7', this.language),
                  icon: "arrow-forward"
              });
              ...
              return this.api.message().newInstance(this.user)
                  ...
                  .then(createdDocument => this.api.encryptDecryptFileContentByUserHcpIdAndDocumentObject("encrypt", this.user, createdDocument, pdfFileContent).then(encryptedFileContent => ({
                      createdDocument,
          ...
          .then(() => {
              this._setLoadingMessage({
                  message: this.localize('mhListing.spinner.step_7_done', this.language),
                  icon: "check-circle",
                  updateLastMessage: true,
                  done: true
              })
              this._setLoadingMessage({
                  message: this.localize('mhListing.spinner.step_8', this.language),
                  icon: "arrow-forward"
              })
          })
  ...
  _getListingArchives() {
      ...
      this.api.message().findMessagesByTransportGuid('MH:ARCHIVES:LISTING', null, null, null, 1000).then(m => {
          this.archiveListingMessages = _
              .chain(m.rows)
              .filter((m) => {
                  return _.get(m, 'fromHealthcarePartyId', false) === this.user.healthcarePartyId && (!dateRangeStart ? true : (!moment(m.created).isBefore(moment(dateRangeStart, "YYYY-MM-DD")))) && (!dateRangeEnd ? true : (moment(m.created).isBefore(moment(dateRangeEnd, "YYYY-MM-DD"))))
              })
              .map((i) => {
  ...
  _downloadListingArchive(e) {
      let messageId = _.get(_.head(_.chain(e.path).filter((i) => {
          return _.get(_.get(i, "classList", []), "value", "").indexOf('downloadListingArchive') !== -1
  ...
  _getExportYearsList() {
      let toReturn = [];
      for (let i = (parseInt(moment().format('YYYY')) + 1); i >= (parseInt(moment().format('YYYY')) - 2); i--) toReturn.push({
          id: i,
          label: i
      })
      return toReturn
  ...
  _createOrUpdateMedicalHousePatientsInvoices() {
      ...
      return this.api.insurance().getInsurances(new models.ListOfIdsDto({ids: parentInsuranceIds})).then(
          parentInsurances => {
                  ...
                  prom = prom.then(pats =>
                      this.api.crypto().extractDelegationsSFKs(pat, this.user.healthcarePartyId)
                          .then(secretForeignKeys => {
                                  ...
                                  return this.api.invoice().appendCodes(this.user.id, "patient", "cdrom", (pat.finalInsurability && pat.finalInsurability.insuranceId ? pat.finalInsurability.insuranceId : ''), secretForeignKeys.extractedKeys.join(","), null, (365 * 2), pat.invoicingCodes)
                                      .then(invoices =>
                                          !_.trim(_.get(invoices, "[0].id", "")) ? this.api.invoice().newInstance(this.user, pat, invoices[0])
                                      ...
                                      .then(newInvoice => {
                                          ...
                                          this._setLoadingMessage({
                                              message: this.localize('mhInvoicing.spinner.step_4', this.language) + " " + (loopIndex + 1) + "/" + patsCount,
                                              icon: "arrow-forward",
                                              updateLastMessage: true,
                                              done: false
                                          });
  ...
  _generateJ20FlatFiles() {
      return Promise.all(this.flatRateInvoicingDataObject.iosData.map(OA => {
          ...
          //loop patients of OA
          //const oaMuts = this.flatRateInvoicingDataObject.insurancesData.filter(i=> _.trim(_.get(i, "parent", "")) === _.trim(_.get(OA, "id", "")));
          ...
          // const oaInvoicesData = this.flatRateInvoicingDataObject.invoicesData.filter(idt => oaMutIds.includes(idt.insurabilities[0].insuranceId));
  ...
  slipDataFromBatchAndFlatFile(batch, flatWithMetadata) {
      ...
      fileInfo = fileInfo.replace("[batch.version]", "0001999").replace("[batch.nr]", batch.batchRef.substr(0, 3)).replace("[batch.fileName]", fileName).replace("[batch.yearMonth]", batch.invoicingMonth + '/' + batch.invoicingYear).replace("[batch.creationDate]", moment().format("DD/MM/YYYY")).replace("[mm.contactPerson]", hcp.contactPerson);
      let columnHeaders = "---------------------------------------------------------------------------\n" +
  ...
  getPatientInvoice(patientData, OA) {
      ...
      inv.patient = {}
      inv.patient.dateOfBirth = this.convertDate(patientData.DateOfBirth);
      ...
      // const patIns = patientData.insurabilities[0];
      ...
      _.forEach(patientData.invoices, invItm => {
          _.forEach(invItm.invoicingCodes, invCd => {
              ...
              itm.reimbursedAmount = invCd.reimbursement * 100;
              itm.units = 1;
              inv.items.push(itm);
          })
      });

      //patientData.invoices[0].patient =
      return inv;
  }
  ...
  _updateBatchStatus(action) {
      ...
      const actionSettings = action === "reject" ? {
              newStatusKey: "rejected",
              newInvoicingCodeStatus: "canceled"
          } :
          action === "accept" ? {newStatusKey: "fullyAccepted", newInvoicingCodeStatus: "accepted"} :
              action === "partialAccept" ? {
                      newStatusKey: "partiallyAccepted",
                      newInvoicingCodeStatus: "accepted"
                  } :
          ...
          Promise.all(
              ...
              return (!_.trim(actionSettings.newInvoicingCodeStatus)) ? false : this.api.invoice().getInvoices(new models.ListOfIdsDto({ids: _.uniq(_.flatten(_.map(allMessages, msg => msg.invoiceIds.map(i => i))))}))
                  ...
                  .then(invoices => {
                      ...
                      const acceptedAmount = this.api._powRoundFloatByPrecision(_.sumBy(_.filter(_.map(invoices, inv => {
                          return {
                              acceptedOrRefused: !!_.get(inv, "invoicingCodes[0].canceled", false) ? "refused" : "accepted",
                              amount: this.api._powRoundFloatByPrecision(parseFloat(_.sumBy(_.get(inv, "invoicingCodes"), i => parseFloat(i.totalAmount))), 2)
                          }
                      }), {acceptedOrRefused: "accepted"}), i => parseFloat(i.amount)), 2)
                      const refusedAmount = this.api._powRoundFloatByPrecision(_.sumBy(_.filter(_.map(invoices, inv => {
                          return {
                              acceptedOrRefused: !!_.get(inv, "invoicingCodes[0].canceled", false) ? "refused" : "accepted",
                              amount: this.api._powRoundFloatByPrecision(parseFloat(_.sumBy(_.get(inv, "invoicingCodes"), i => parseFloat(i.totalAmount))), 2)
                          }
                      }), {acceptedOrRefused: "refused"}), i => parseFloat(i.amount)), 2)
                      let updatedFlatFileMessage = _.get(_.filter(allMessages, {id: this.activeGridItem.id}), "[0]", {})
                      updatedFlatFileMessage.metas = _.merge(updatedFlatFileMessage.metas, {
          ...
          .then(() => {
              return this.api.invoice().getInvoices(new models.ListOfIdsDto({ids: _.compact(_.uniq(_.map(_.get(this, "activeGridItem.invoiceIds", []), i => i)))}))
                  .then(invoices => {
                      ...
                      this.activeGridItem.metas = _.merge(this.activeGridItem.metas, {
                          acceptedAmount: acceptedAmount,
                          refusedAmount: refusedAmount
                      });
                      return this.api.message().modifyMessage(this.activeGridItem).then(msg => {
                          this.api.register(msg, 'message');
                      })
                  })
          })
          .catch((e) => {
  ...
  _downloadParentOaArchive(e) {
      ...
      return !messageId ? null : this.api.document().findByMessage(this.user.healthcarePartyId, batchMessage)
          .then(doc => (doc && doc[0] && doc[0].id && doc[0].attachmentId && doc[0].secretForeignKeys) ? this.api.document().getAttachment(doc[0].id, doc[0].attachmentId, doc[0].secretForeignKeys).then(attachment => {
              return {doc: _.head(doc), attachment: attachment}
          }) : prom)
          .then(({doc, attachment}) => attachment ? this.api.encryptDecryptFileContentByUserHcpIdAndDocumentObject("decrypt", this.user, doc, attachment).then(decryptedContent => {
              return {doc: doc, decryptedContent: decryptedContent}
          }) : prom)
  ...
  _generateJ20PdfTemplateObjects() {
      ...
      const pdfDataToPrint = _.compact(
          _.map(pdfData, (oaData) => {
              ...
              const totalAmount = this.api._powRoundFloatByPrecision(_.sumBy(_.filter(this.flatRateInvoicingDataObject.oaTotalPrices, i => {
                  return _.trim(i.oaId) === _.trim(oaData.oa.id)
              }), i => parseFloat(i.listTotal)), 2);
                  ...
                  _.compact(
                      _.map(oaData.entries, (listEntries, listKey) => {
                          ...
                          return !parseInt(_.size(listEntries)) ? false :
                              ...
                              this.api.hrFormatNumber(_.get(_.filter(this.flatRateInvoicingDataObject.oaTotalPrices, i => {
                                  return _.trim(i.oaId) === _.trim(oaData.oa.id) && _.trim(i.listId) === _.trim(listKey)
                              }), "[0].listTotal", 0)) + ' €'
                      })
                  ).join("<br />") +
                  "</p>" +
  ...
  _generateJ20PdfTemplateParentOaObjects() {
      ...
      pdfDataToPrint = _.map(this.flatRateInvoicingDataObject.iosData, (parentOaData) => {
          ...
          _.map(_.filter(pdfData, i => _.trim(_.get(i, "oa.parent", "")) === _.trim(_.get(parentOaData, "id", ""))), (oaData) => {
              ...
              const totalAmount = this.api._powRoundFloatByPrecision(_.sumBy(_.filter(oasTotalPrices, i => {
                  return _.trim(i.oaId) === _.trim(oaData.oa.id)
              }), i => parseFloat(i.listTotal)), 2);
              ...
              pdfTemplate +=
                  ...
                  _.compact(
                      _.map(oaData.entries, (listEntries, listKey) => {
                          ...
                          return !parseInt(_.size(listEntries)) ? false :
                              this.localize('flatrate_invoicing_listTotal', this.language) + ' (' + _.trim(listIndex) + ') "<span class="italic colorBlue">' + _.trim(this.localize('flatrate_invoicing_' + _.trim(listIndex) + '_subheader', this.language)).toLowerCase() + '</span>": ' +
                              this.api.hrFormatNumber(_.get(_.filter(oasTotalPrices, i => {
                                  return _.trim(i.oaId) === _.trim(oaData.oa.id) && _.trim(i.listId) === _.trim(listKey)
                              }), "[0].listTotal", 0)) + ' €'
                      })
                  ).join("<br />") +
  ...
  _exportFlatRateInvoicing() {
      ...
      this.set('_isLoading', true);
      this._setLoadingMessage({
          message: this.localize('mhInvoicing.spinner.step_1', this.language),
          icon: "arrow-forward"
      });
      ...
      this.api.hcparty().getHealthcareParty(_.get(this.user, "healthcarePartyId", ""))
          .then(hcp => {
              ...
              this.flatRateInvoicingDataObject = {
                  hcpData: {
                      ...
                      financialInfo: _.head(_.filter(_.get(this.hcp, "financialInstitutionInformation", []), i => {
                  ...
                  finalArchiveSpeakingName: {
                      archiveDownloadFileName: _.kebabCase(_.compact([
                          moment().format('YYYY-MM-DD-HH[h]-mm[m]-ss[s]'),
                          "medical house",
                          _.trim(_.get(this.hcp, "name", "")) || _.trim(_.trim(_.get(this.hcp, "firstName", "")) + " " + _.trim(_.get(this.hcp, "lastName", ""))) || _.trim(_.get(this.user, "name", "")),
                          "invoicing export",
                          +new Date()
                      ]).join(" ")) + ".zip"
                  },
              ...
              // Get valorisations for last 24 months - as of export date
              let valorisationMonths = [];
              for (let i = 0; i < 24; i++) {
                  valorisationMonths.push(_.trim(moment(_.trim(this.flatRateInvoicingDataObject.exportedDate), "YYYYMMDD").startOf('month').subtract(i, "month").format("YYYYMMDD")))
              }
              ...
              const medicalHouseAvailableValorisationsByNihii = _.compact(_.map(["physician", "physiotherapist", "nurse"], (v, k) => {
                  return !!parseInt(medicalHouseNihiiLastThreeDigits[k]) ? {flatRateType: v} : false
              }))

              // At least one MH valorisation is missing
              if (
                  !parseInt(_.size(medicalHouseAvailableValorisationsByNihii))
                  || _.size(medicalHouseAvailableValorisationsByNihii) !== _.size(_.compact(_.map(medicalHouseAvailableValorisationsByNihii, mhValorisation => !!parseInt(_.size(_.filter(_.get(this, "flatRateInvoicingDataObject.hcpValorisationsByMonth[0].valorisations", {}), i => {
                      return _.trim(_.get(i, "flatRateType", "")) === _.trim(mhValorisation.flatRateType) && parseFloat(_.get(i, "price", 0))
                  }))))))
          ...
          .then(() => {
              ...
              return this.api.getRowsUsingPagination(
                              ...
                              return {
                                  rows: _.filter(pl.rows, m => {
                                      ...
                                      return m
                                          ...
                                          && (
                                              (// Either current month
                                              parseInt(_.get(m, "metas.exportedDate", "")) === parseInt(this.flatRateInvoicingDataObject.exportedDate) || parseInt(_.get(m, "metas.exportedDate", "")) < parseInt(this.flatRateInvoicingDataObject.exportedDate))    // Or before, NEVER take resent invoices "in the future"
                                          );
                                  }),
                                  nextKey: pl.nextKeyPair && pl.nextKeyPair.startKey,
                                  nextDocId: pl.nextKeyPair && pl.nextKeyPair.startKeyDocId,
                  ...
                  this.flatRateInvoicingDataObject.exportAlreadyRanThisMonth = !!parseInt(_.size(_.filter(foundMessages, m => {
                      return _.trim(_.get(m, "metas.exportedDate", 0)) === _.trim(this.flatRateInvoicingDataObject.exportedDate)
                  })))
                  ...
                  return !parseInt(_.size(foundMessages)) ? [] :
                      ...
                      !parseInt(_.size(_.filter(foundMessages, msg => {
                          return ["archived", "error", "partiallyAccepted", "rejected"].indexOf(msg.evaluatedStatus) > -1
                      }))) ? [] :

                          this.api.invoice().getInvoices(new models.ListOfIdsDto({
                              ids: _.compact(_.uniq(_.flatMap(_.map(_.filter(foundMessages, msg => {
                                  return ["archived", "error", "partiallyAccepted", "rejected"].indexOf(msg.evaluatedStatus) > -1
                              }), "invoiceIds"))))
                          }))
                              .then(invoicesToBeCorrected => !parseInt(_.size(invoicesToBeCorrected)) ? [] :
                                  this.api.invoice().getInvoices(new models.ListOfIdsDto({ids: _.compact(_.uniq(_.map(_.filter(invoicesToBeCorrected, _.trim("correctiveInvoiceId")), "correctiveInvoiceId")))}))
                                      .then(correctiveInvoices => !parseInt(_.size(correctiveInvoices)) ? [] :
                                          _.compact(
                                              _.filter(correctiveInvoices, i => !_.trim(_.get(i, "sentDate", ""))).map(inv => {
                                                  ...
                                                  // If I try to resend a corrective invoice for current month, DROP its invoicingCodes (otherwise append code will double the charged amounts).
                                                  // If not, append codes so that both previous & current month will be charged, which is legit
                                                  inv.resentMonth = parseInt(_.get(inv, "invoiceDate", 0)) === parseInt(this.flatRateInvoicingDataObject.exportedDate) ? "currentMonth" : "inThePast"
                                      ...
                                      .catch(e => {
                                          console.log("Could not getInvoices (corrective ones) by ", _.compact(_.uniq(_.map(_.filter(invoicesToBeCorrected, _.trim("correctiveInvoiceId")), "correctiveInvoiceId"))));
                                          console.log(e);
                                          return false;
                                      })
                              )
                              .catch(e => {
                                  console.log("Could not getInvoices (to be corrected) by ", _.compact(_.uniq(_.flatMap(_.map(foundMessages, "invoiceIds")))));
                                  console.log(e);
                                  return false;
                              })
              });
          ...
          .then(pendingInvoicesToResend => {
              ...
              this._setLoadingMessage({
                  message: this.localize('mhInvoicing.spinner.step_1_done', this.language),
                  icon: "check-circle",
                  updateLastMessage: true,
                  done: true
              });
              ...
              return !this.flatRateInvoicingDataObject.exportAlreadyRanThisMonth ? this._getPatientsByHcp(_.get(this.hcp, "id")).then(myPatients => _.chain(myPatients).uniqBy('ssin').orderBy(['lastName', 'firstName'], ['asc', 'asc']).value()) : false
          })
          .then(myPatients => {
              return !parseInt(_.size(this.flatRateInvoicingDataObject.pendingInvoicesToResend)) ?
                  ...
                  Promise.all(this.flatRateInvoicingDataObject.pendingInvoicesToResend.map(inv => this.api.crypto().extractCryptedFKs(inv, this.user.healthcarePartyId).then(ids => [inv, ids.extractedKeys[0]]).catch(e => {
                      console.log(e);
                      console.log("Could not extractCryptedFKs for invoices to resend");
                  })))
                      .then(invAndIdsPat => this.api.patient().getPatientsWithUser(this.user, new models.ListOfIdsDto({ids: _.uniq(invAndIdsPat.map(x => x[1]))})).then(pats => invAndIdsPat.map(it => [it[0], pats.find(p => p.id === it[1])])).catch(e => {
                          console.log(e);
                          console.log("Could not get getPatientsWithUser for invoices to resend");
                      }))
                      .then(invoicesAndPatient => {
                          invoicesAndPatient = _.map(invoicesAndPatient, invAndPat => {
                              let tempPat = _.cloneDeep(invAndPat[1])
                              tempPat.patientToBeResent = true
                              ...
                              tempPat.finalInsurability = _.get(
                                  _.filter(
                          ...
                          return {
                              myPatients: _.compact(myPatients),
                              myPatientsToResend: _.compact(_.map(invoicesAndPatient, x => x[1]))
                          }
                      })
          })
          .then(patsAndPatsToResend => {
              let myPatients = patsAndPatsToResend.myPatients
              let myPatientsToResend = _.cloneDeep(patsAndPatsToResend.myPatientsToResend)
              let resolvedPatientsToBeResent = []
              const commonPatientsBetweenExportsWithSameIns = _.compact(
                  _.map(myPatients, pat => {
                      const matchingPat = _.filter(myPatientsToResend, patToResend => {
                          return (
                              _.trim(_.get(patToResend, "ssin", "something")) === _.trim(_.get(pat, "ssin", "else")) &&
                              _.trim(_.get(patToResend, "finalInsurability.identificationNumber", "something")) === _.trim(_.get(pat, "finalInsurability.identificationNumber", "else"))
                          )
                      })
                      return _.size(matchingPat) ? matchingPat : false
                  })
              )
              _.map(commonPatientsBetweenExportsWithSameIns, commonPat => {
                  myPatients = _.map(myPatients, existingPat => {
                      return _.trim(_.get(existingPat, "ssin", "something")) !== _.trim(_.get(commonPat, "[0].ssin", "else")) ?
                          existingPat :
                          _.merge(existingPat, {
                              invoiceToBeResent: _.compact(_.map(commonPat, "invoiceToBeResent")),
                              patientToBeResent: true,
                              patientIsInCurrentAndPreviousExport: true
                          })
                  })
                  // _.remove(myPatientsToResend, i=> _.trim(_.get(i,"ssin","something")) === _.trim(_.get(commonPat,"[0].ssin","else")) )
                  _.remove(myPatientsToResend, i => _.trim(_.get(i, "ssin", "something")) === _.trim(_.get(commonPat, "[0].ssin", "else")))
              })

              const nonCommonPatientsToAdded = _.compact(_.map(_.map(myPatientsToResend, "id"), patId => {
                  if (resolvedPatientsToBeResent.indexOf(patId) > -1) {
                      return false;
                  } else {
                      resolvedPatientsToBeResent.push(patId);
                  }
                  let singlePat = _.find(myPatientsToResend, {id: patId})
                  singlePat.invoiceToBeResent = _.map(_.filter(myPatientsToResend, {id: patId}), "invoiceToBeResent")
                  singlePat.patientToBeResent = true
                  singlePat.patientIsInCurrentAndPreviousExport = false
                  return singlePat
              }))
              if (!!parseInt(_.size(nonCommonPatientsToAdded))) myPatients = _.concat(myPatients, nonCommonPatientsToAdded)
                              ...
                              _
                                  ...
                                  .filter(mhc => {
                                      ...
                                      (
                                          ...
                                          (parseInt(_.get(mhc, "startOfCoverage", 0)) && moment(_.trim(_.get(mhc, "startOfCoverage", "0")), "YYYYMMDD").endOf('month').isBefore(mhcExportDateOrResentDate) || !parseInt(_.get(mhc, "startOfCoverage", 0)) && moment(_.trim(_.get(mhc, "startOfContract", 0)), "YYYYMMDD").startOf('month').add(1, 'months').isBefore(mhcExportDateOrResentDate))
                                      );
                                  })
                                  .value(),
                              ["startOfContract"],
              ...
              this._setLoadingMessage({
                  message: this.localize('mhInvoicing.spinner.step_3', this.language),
                  icon: "arrow-forward"
              });
          ...
          .then((myPatientsWithInsuranceTitularyInfo) => {
              ...
              this._setLoadingMessage({
                  message: this.localize('mhInvoicing.spinner.step_3_done', this.language),
                  icon: "check-circle",
                  updateLastMessage: true,
                  done: true
              });
          ...
          .then((insurancesData) => {
              ...
              return this._getIOsDataByInsurancesList(this.flatRateInvoicingDataObject.insurancesData).then(x => x);
          })
          .then((iosData) => {
              ...
              return this._getInsurancesDataByCode(300)
                  .then(ins300 => {
                      ...
                      this.flatRateInvoicingDataObject.insurancesData = _.map(this.flatRateInvoicingDataObject.insurancesData, ins => {
                          if (_.trim(_.get(ins, "code", "")) === "306") ins.parent = _.trim(_.get(_.filter(this.flatRateInvoicingDataObject.iosData, {code: "300"}), "[0].id", ""));
                          return ins
                      })
                      return null
                  })
                  .then(() => {
                      ...
                      return {
                          ...
                          list8: _.filter(this.flatRateInvoicingDataObject.patientsData, pat => {
                              let mhcExportDateOrResentDate = !!parseInt(_.size(_.get(pat, "invoiceToBeResent[0]", []))) ? moment(_.trim(_.get(pat, "invoiceToBeResent[0].invoiceDate", 0)), "YYYYMMDD") : moment(_.trim(this.flatRateInvoicingDataObject.exportedDate), "YYYYMMDD")
                              return (
                                  !!moment(pat.finalMedicalHouseContracts.startOfCoverage, "YYYYMMDD").startOf("month").isBefore(mhcExportDateOrResentDate.startOf("month").subtract(1, 'month')) &&
                                  (!!_.get(pat, "finalMedicalHouseContracts.gp", false) || !!_.get(pat, "finalMedicalHouseContracts.kine", false) || !!_.get(pat, "finalMedicalHouseContracts.nurse", false))
                              )
                          }),
                          ...
                          list5: _.filter(this.flatRateInvoicingDataObject.patientsData, pat => {
                              let mhcExportDateOrResentDate = !!parseInt(_.size(_.get(pat, "invoiceToBeResent[0]", []))) ? moment(_.trim(_.get(pat, "invoiceToBeResent[0].invoiceDate", 0)), "YYYYMMDD") : moment(_.trim(this.flatRateInvoicingDataObject.exportedDate), "YYYYMMDD")
                              return (
                                  !!moment(pat.finalMedicalHouseContracts.startOfCoverage, "YYYYMMDD").startOf("month").isSame(mhcExportDateOrResentDate.startOf("month").subtract(1, 'month')) &&
                          ...
                          // Start of month of start cover must be EQUAL TO (start of export month -1 month)
                          // AND Start of month of cover - 1 month > Start of month of contract
                          list6: _.filter(this.flatRateInvoicingDataObject.patientsData, pat => {
                              let mhcExportDateOrResentDate = !!parseInt(_.size(_.get(pat, "invoiceToBeResent[0]", []))) ? moment(_.trim(_.get(pat, "invoiceToBeResent[0].invoiceDate", 0)), "YYYYMMDD") : moment(_.trim(this.flatRateInvoicingDataObject.exportedDate), "YYYYMMDD")
                              return (
                                  !!moment(pat.finalMedicalHouseContracts.startOfCoverage, "YYYYMMDD").startOf("month").isSame(mhcExportDateOrResentDate.startOf("month").subtract(1, 'month')) &&
          ...
          .then((listsData) => {
              ...
              this._setLoadingMessage({
                  message: this.localize('mhInvoicing.spinner.step_4', this.language),
                  icon: "arrow-forward"
              });
              ...
              return Promise.all(_.toPairs(this.flatRateInvoicingDataObject.listsData).map(list => {
                  ...
                  return patientList.map(pat => {
                      ...
                      const additionalInvoicingCodes = _.flatMap(_.map(_.get(pat, "invoiceToBeResent", []), invoiceToBeResent => {
                          ...
                          return _.map(_.get(invoiceToBeResent, "invoicingCodes", []).filter(ic => _.compact(_.uniq(_.map(originalInvoicingCodes, oic => _.trim(_.get(oic, "valorisationMonth", 0))))).indexOf(_.trim(_.get(ic, "dateCode", 0))) === -1), ic => _.merge({}, {
                              code: _.get(ic, "code", ""),
                              label: {
                                  fr: _.get(ic, "label", ""),
                                  nl: _.get(ic, "label", ""),
                      ...
                      return _.assign(pat, {
                          invoicingCodes: _.compact(_.concat(additionalInvoicingCodes || [], originalInvoicingCodes)).map(val => ({
                              code: val.code,
                      ...
                      return !parseInt(_.size(invoicesToBeModified || [])) ?
                          ...
                          Promise.all(invoicesToBeModified.map(inv => {
                              prom = prom.then(invoicesToBeModified => this.api.invoice().modifyInvoice(inv)
                                  ...
                                  .catch(e => {
                                      console.log("Can't modify invoice", inv, e);
                                      return false;
                                  })
                              )
                          }))
          ...
          .then(invoicesData => {
              ...
              this._setLoadingMessage({
                  message: this.localize('mhInvoicing.spinner.step_4_done', this.language),
                  icon: "check-circle",
                  updateLastMessage: true,
                  done: true
              });
              this._setLoadingMessage({
                  message: this.localize('mhInvoicing.spinner.step_7', this.language),
                  icon: "arrow-forward"
              });
                                          ...
                                          _.map(
                                              _.compact(
                                                  _.filter(
                                                      ...
                                                      inv => _.size(_.filter(_.get(inv, "invoices", []), i => {
                                                          return _.compact(_.uniq(_.get(flatFile, "oaInvoiceIds", []))).indexOf(i.id) > -1
                                                      }))
                                                  )
                                              ),
              ...
              return Promise.all(this.flatRateInvoicingDataObject.generatedFiles.flatFiles.map(singleFileObject => {
                  return this.api.message().newInstance(this.user)
                      ...
                      .then(createdMessage => {
                          ...
                          return this.api.document().newInstance(this.user, createdMessage, {
                              documentType: 'report',
                              mainUti: this.api.document().uti("text/plain"),
                              name: _.trim(singleFileObject.downloadFileName)
                          }).then(newDocumentInstance => this.api.document().createDocument(newDocumentInstance))
                      })
                      .then(createdDocument => {
                          _.merge(singleFileObject, {documentId: createdDocument.id})
                          return this.api.encryptDecryptFileContentByUserHcpIdAndDocumentObject("encrypt", this.user, createdDocument, this.api.crypto().utils.ua2ArrayBuffer(this.api.crypto().utils.text2ua(singleFileObject.fileContent))).then(encryptedFileContent => ({
                              createdDocument,
                              encryptedFileContent
                          }))
                      })
                      .then(({createdDocument, encryptedFileContent}) => {
                          return this.api.document().setAttachment(createdDocument.id, null, encryptedFileContent).then(x => x).catch((e) => {
                              console.log("---error upload attachment flatfile---");
                  ...
                  .then(() => {
                      return Promise.all(this.flatRateInvoicingDataObject.generatedFiles.slips.map(singleFileObject => {
                          return this.api.message().newInstance(this.user)
                              ...
                              .then(({createdDocument, encryptedFileContent}) => {
                                  return this.api.document().setAttachment(createdDocument.id, null, encryptedFileContent).then(x => x).catch((e) => {
                                      console.log("---error upload attachment slip---");
                                      console.log(createdDocument);
                                      console.log(encryptedFileContent);
                                  })
                              })
                      ...
                      this._setLoadingMessage({
                          message: this.localize('mhInvoicing.spinner.step_7_done', this.language),
                          icon: "check-circle",
                          updateLastMessage: true,
                          done: true
                      });
                      this._setLoadingMessage({
                          message: this.localize('mhInvoicing.spinner.step_5', this.language),
                          icon: "arrow-forward"
                      });
          ...
          .then(pdfTemplateObjects => {
              return Promise.all(pdfTemplateObjects.map(singlePdfFileObject => this.api.pdfReport(mustache.render(singlePdfFileObject.template, {type: "doc-off-format"})).then(x => _.merge(singlePdfFileObject, {fileContent: x.pdf}))))
          })
                  ...
                  return this.api.message().newInstance(this.user)
                      ...
                      .then(createdMessage => {
                          ...
                          return this.api.document().newInstance(this.user, createdMessage, {
                              documentType: 'report',
                              mainUti: this.api.document().uti("application/pdf"),
                              name: _.trim(singlePdfFileObject.downloadFileName)
                          }).then(newDocumentInstance => this.api.document().createDocument(newDocumentInstance))
                      })
                      .then(createdDocument => {
                          _.merge(singlePdfFileObject, {documentId: createdDocument.id})
                          return this.api.encryptDecryptFileContentByUserHcpIdAndDocumentObject("encrypt", this.user, createdDocument, singlePdfFileObject.fileContent).then(encryptedFileContent => ({
                              createdDocument,
                              encryptedFileContent
                          }))
                      })
                      .then(({createdDocument, encryptedFileContent}) => {
                          return this.api.document().setAttachment(createdDocument.id, null, encryptedFileContent).then(x => x).catch((e) => {
                              console.log("---error upload attachment pdf---");
                              console.log(createdDocument);
                              console.log(encryptedFileContent);
                          })
                      })
          ...
          .then((parentOaPdfTemplateObjects) => {
              return Promise.all(parentOaPdfTemplateObjects.map(singlePdfFileObject => this.api.pdfReport(mustache.render(singlePdfFileObject.template, {type: "doc-off-format"})).then(x => _.merge(singlePdfFileObject, {fileContent: x.pdf}))))
          })
          .then(generatedOaPdfFiles => {
              ...
              return Promise.all(this.flatRateInvoicingDataObject.generatedFiles.oaPdfs.map(singlePdfFileObject => {
                  return this.api.message().newInstance(this.user)
                      ...
                      .then(({createdDocument, encryptedFileContent}) => {
                          return this.api.document().setAttachment(createdDocument.id, null, encryptedFileContent).then(x => x).catch((e) => {
                              console.log("---error upload attachment parent oa pdf---");
                              console.log(createdDocument);
                              console.log(encryptedFileContent);
                          })
                      })
          ...
          .then(() => {
              this._setLoadingMessage({
                  message: this.localize('mhInvoicing.spinner.step_5_done', this.language),
                  icon: "check-circle",
                  updateLastMessage: true,
                  done: true
              });
              this._setLoadingMessage({
                  message: this.localize('mhInvoicing.spinner.step_8', this.language),
                  icon: "arrow-forward"
              })
          ...
          .then(batchZipFileBlob => {
              ...
              return this.api.message().newInstance(this.user)
                  ...
                  .then(createdMessage => {
                      ...
                      return this.api.document().newInstance(this.user, createdMessage, {
                          documentType: 'report',
                          mainUti: this.api.document().uti("application/zip"),
                          name: _.trim(this.flatRateInvoicingDataObject.finalArchive.archiveDownloadFileName)
                      }).then(newDocumentInstance => this.api.document().createDocument(newDocumentInstance))
                  })
                  .then(createdDocument => {
                      _.merge(this.flatRateInvoicingDataObject.finalArchive, {documentId: createdDocument.id})
                      return this.api.encryptDecryptFileContentByUserHcpIdAndDocumentObject("encrypt", this.user, createdDocument, this.flatRateInvoicingDataObject.finalArchive.batchZipFileBlob).then(encryptedFileContent => ({
                          createdDocument,
                          encryptedFileContent
                      }))
                  })
                  .then(({createdDocument, encryptedFileContent}) => {
                      return this.api.document().setAttachment(createdDocument.id, null, encryptedFileContent).then(x => x).catch((e) => {
                          console.log("---error upload attachment batch---");
                          console.log(createdDocument);
                          console.log(encryptedFileContent);
                      })
                  })
          ...
          .then(() => {
              this._setLoadingMessage({
                  message: this.localize('mhInvoicing.spinner.step_8_done', this.language),
                  icon: "check-circle",
                  updateLastMessage: true,
                  done: true
              });
          ...
          .finally(() => {
              ...
              invGridDetail && invGridDetail.clearCache();
              this._fetchJ20Messages(true).then(() => {
                  this.dispatchEvent(new CustomEvent('do-route', {
                      bubbles: true,
                      composed: true,
                      detail: {selection: {item: "e_flatrateinvOut", status: "j20_process"}}
                  }));
              })
          })
          ...
          .then(messages => {
              ...
              this.messagesCachedData = _.merge(this.messagesCachedData, {
                  ...
                  roughData: _.chain(messages).uniqBy('id').value().map(message => {
                      ...
                      const invoiceStatus =
                          !!(st & (1 << 21)) ? {
                                  hrLabel: this.localize('inv_arch', 'Archived', this.language),
                                  finalStatus: "archived"
                              } :
                              !!(st & (1 << 17)) ? {
                                      hrLabel: this.localize('inv_err', 'Error', this.language),
                                      finalStatus: "error"
                                  } :
                                  !!(st & (1 << 16)) ? {
                                          hrLabel: this.localize('inv_par_acc', 'Partially accepted', this.language),
                                          finalStatus: "partiallyAccepted"
                                      } :
                                      !!(st & (1 << 15)) ? {
                                              hrLabel: this.localize('inv_full_acc', 'Fully accepted', this.language),
                                              finalStatus: "fullyAccepted"
                                          } :
                                          !!(st & (1 << 12)) ? {
                                                  hrLabel: this.localize('inv_rej', 'Rejected', this.language),
                                                  finalStatus: "rejected"
                                              } :
                                              !!(st & (1 << 11)) ? {
                                                      hrLabel: this.localize('inv_tre', 'Treated', this.language),
                                                      finalStatus: "treated"
                                                  } :
                                                  !!(st & (1 << 10)) ? {
                                                          hrLabel: this.localize('inv_acc_tre', 'Accepted for treatment', this.language),
                                                          finalStatus: "acceptedForTreatment"
                                                      } :
                                                      !!(st & (1 << 9)) ? {
                                                              hrLabel: this.localize('inv_succ_tra_oa', 'Successfully transmitted to OA', this.language),
                                                              finalStatus: "successfullySentToOA"
                                                          } :
                                                          !!(st & (1 << 8)) ? {
                                                              hrLabel: this.localize('inv_pen', 'Pending', this.language),
                                                              finalStatus: "pending"
                                                          } : ""
              ...
              return Promise.all(_.map(_.concat(dataByStatus.partiallyAccepted.flatFiles, dataByStatus.rejected.flatFiles), msg => {
                              ...
                              _.map(invoicesToBeCorrected, i => {
                                  return (
                                      ...
                                      && _.every(_.get(i, "invoicingCodes", []), ic => {
                                          return _.get(ic, "pending", false) === true && _.get(ic, "resent", false) === true && _.get(ic, "lost", false) === false
                                      })
                                  ) ? i : false
                              })
                          )
                  ...
                  // For msg-menu
                  this.dispatchEvent(new CustomEvent('initialize-batch-counter-j20', {
                      bubbles: true,
                      composed: true,
                      detail: this.messagesCachedData.countByStatus
                  }));
                  return false;
  ...
  _toggleBatchDetails(e) {
      ...
      if (_.get(e, "path", false)) {
          var paths = _.map(e.path, (pathNode, index) => {
              return {
                  index: index,
                  nodeName: _.get(pathNode, "nodeName", ""),
                  target: _.get(pathNode, "id", "").toLowerCase() === "messagesgrid"
              }
          })
          if (!parseInt(_.size(_.compact(_.map(paths.slice(0, _.get(_.filter(paths, "target"), "[0].index", 0)), (path, index) => {
              return ["vaadin-grid-cell-content", "slot", "td", "tr"].indexOf(_.trim(path.nodeName).toLowerCase()) > -1
          }))))) return;
      }
      ...
      if (_.size(this.activeGridItem)) {
          ...
          let batchExportPdfs = _.filter(this.messagesCachedData.dataByStatus[dataStatus].pdfs, i => {
              return parseInt(_.get(i, "metas.batchExportTstamp", 0)) === parseInt(_.get(this.activeGridItem, "metas.batchExportTstamp", 0)) && _.get(i, "metas.parentOaId", "") === _.trim(_.get(this.activeGridItem, "metas.parentOaId", ""))
          })
                      ...
                      _.map(batchExportPdfs, message => {
                          return _.map(_.get(message, "invoiceIds", []), invoiceId => {
                              ...
                              return {
                                  ...
                                  invoicedAmount: this.api.hrFormatNumber(this.api._powRoundFloatByPrecision(_.sumBy(_.get(_.get(_.filter(invoicesAndPatient, i => _.get(i, "[0].id", "") === invoiceId), "[0][0]", {}), "invoicingCodes", []), i => parseFloat(i.totalAmount)))),
  ...
  _duplicateRejectedInvoices(rejectedInvoiceIds) {
      ...
      return !parseInt(_.size(rejectedInvoiceIds || [])) ?
          ...
          this.api.invoice().getInvoices(new models.ListOfIdsDto({ids: _.uniq(rejectedInvoiceIds)}))
              .then(invoices => {
                  return this._getInsurancesDataByIds(_.compact(_.uniq(_.map(invoices, "recipientId", "")))).then(ins => {
                      insData = ins;
                      return invoices;
                  })
              })
              .then(invoices => {
                  ...
                  return Promise.all(invoices.map(inv => this.api.crypto().extractCryptedFKs(inv, _.get(this, "user.healthcarePartyId", ""))
                      ...
                      .catch(e => {
                          console.log("Can't extractCryptedFKs for", inv, e);
                          return [{}, ""];
                      })
                  ))
              })
              .then(invoicesAndPatIds => {
                  ...
                  return this.api.patient().getPatientsWithUser(this.user, new models.ListOfIdsDto({ids: _.uniq(invoicesAndPatIds.map(x => x[1]))}))
                      ...
                      .catch(e => {
                          console.log("Can't getPatientsWithUser for", invoicesAndPatIds, e);
                          return false;
                      })
              })
              .then(invoicesAndPatients => {
                  ...
                  return Promise.all(invoicesAndPatients.map(invoiceAndPatient => {
                      ...
                      return this.api.invoice().newInstance(
                          ...
                          .catch(e => {
                              console.log("Can't get invoice newInstance based on ", oldInvoice, e);
                              return false;
                          })
                  }))
                      .then(x => x)
              ...
              .then(oldInvoicesObjectsAndNewInvoicesInstances => {
                  ...
                  return Promise.all(oldInvoicesObjectsAndNewInvoicesInstances.map(i => {
                      i.newInvoiceInstance.invoicingCodes = _.map(i.oldInvoice.invoicingCodes, ic => {
                          return _.assign({}, ic, {
                              id: this.api.crypto().randomUuid(),
                              accepted: false,
                              canceled: false,
              ...
              .then(oldInvoicesObjectsAndNewInvoicesInstancesWithInvoicingCodes => {
                  ...
                  return Promise.all(oldInvoicesObjectsAndNewInvoicesInstancesWithInvoicingCodes.map(i => {
                      prom = prom.then(oldInvoicesObjectsAndNewInvoicesInstancesWithInvoicingCodes => {
                          ...
                          return this.api.invoice().modifyInvoice(i.oldInvoice).catch(e => {
                              console.log("Can't modify old invoice ", i.oldInvoice, e);
                              return false;
                          }).then(() =>
                              this.api.invoice().createInvoice(i.newInvoiceInstance, 'invoice:' + this.user.healthcarePartyId + ':' + parentInsCode + ':')
                                  ...
                                  .catch(e => {
                                      console.log("Can't createInvoice for ", i.newInvoiceInstance, e);
                                      return false;
                                  })
                          )
                      })
