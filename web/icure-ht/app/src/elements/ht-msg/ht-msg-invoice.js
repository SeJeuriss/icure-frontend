    ...
    return html`
        ...
        <div class="invoice-panel">
            ...
            <div id="batchStatus" class="batch-status">
                ...
                <template is="dom-if" if="[[_isEqual(invoicesStatus,'toBeSend')]]"><span class="batchNumber batchPending">{{_forceZeroNum(selectedInvoicesToBeSend.length)}}</span>
                </template>
                <template is="dom-if" if="[[_isEqual(invoicesStatus,'toBeCorrected')]]"><span class="batchNumber batchToBeCorrected">{{_forceZeroNum(messagesToBeCorrected.length)}}</span>
                </template>
                ...
                <template is="dom-if" if="[[_isBatchCanBeAutoArchived(invoicesStatus.*, messageIdsCanBeAutoArchived.splices)]]">
                    <paper-button class="modal-button alignRight" on-tap="_archiveBatchAuto">
                        [[localize('arch','Archive',language)]]
                        [[_getNbMsgToArchive(messageIdsCanBeAutoArchived.splices)]] envoi(s)
                    </paper-button>
                </template>
            </div>
            <template is="dom-if" if="[[statusToBeSend]]">
                <paper-input id="filter" label="[[localize('sch','Rechercher',language)]]" value="{{filterInput}}" on-keyup="refreshFilter" param="invoiceGrid" where="toBeSend">&gt;
                </paper-input>
            </template>
            <template is="dom-if" if="[[!statusToBeSend]]">
            ...
            <template is="dom-if" if="[[statusToBeSend]]">
                    ...
                    <div class="containScroll">
                        <vaadin-grid id="noscroll" class="pendingTable" theme="force-sizer">
                            ...
                            <vaadin-grid-column flex-grow="0" width="10%" class="recipient-col">
                                <template class="header">
                                    <vaadin-grid-sorter path="patient.insurance.code">
                            ...
                            <vaadin-grid-column flex-grow="0" width="8%" class="recipient-col">
                                <template class="header">
                                    <vaadin-grid-sorter path="invoice.invoiceReference">
                                        [[localize('inv_num_fac','Invoice number',language)]]
                                    </vaadin-grid-sorter>
                                </template>
                            </vaadin-grid-column>
                            <vaadin-grid-column flex-grow="0" width="20%" class="recipient-col">
                                <template class="header">
                                    <vaadin-grid-sorter path="invoice.patient.firstName">
                            ...
                            <vaadin-grid-column flex-grow="0" width="8%" class="recipient-col">
                                <template class="header">
                                    <vaadin-grid-sorter path="patient.ssin">[[localize('inv_niss','Inss',language)]]
                                    </vaadin-grid-sorter>
                                </template>
                            </vaadin-grid-column>
                            <vaadin-grid-column flex-grow="0" width="8%" class="recipient-col">
                                <template class="header">
                                    <vaadin-grid-sorter path="invoice.invoiceDate">[[localize('inv_date_fact','Invoice
                                        date',language)]]
                                    </vaadin-grid-sorter>
                                </template>
                            </vaadin-grid-column>
                            <vaadin-grid-column-group>
                                ...
                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                    <template class="header border-left">
                                        <vaadin-grid-sorter path="invoice.reimbursement">
                                ...
                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                    <template class="header">
                                        <vaadin-grid-sorter path="invoice.patientIntervention">
                                            [[localize('inv_pat','Patient',language)]]
                                        </vaadin-grid-sorter>
                                    </template>
                                </vaadin-grid-column>
                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                    <template class="header">
                                        <vaadin-grid-sorter path="invoice.doctorSupplement">
                                            [[localize('inv_supp','Extra',language)]]
                                        </vaadin-grid-sorter>
                                    </template>
                                </vaadin-grid-column>
                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col border-right">
                                    <template class="header border-right">
                                        <vaadin-grid-sorter path="invoice.totalAmount">
                                            [[localize('inv_tot','Total',language)]]
                                        </vaadin-grid-sorter>
                                    </template>
                                </vaadin-grid-column>
                            </vaadin-grid-column-group>
                            <vaadin-grid-column flex-grow="0" width="10%" class="recipient-col">
                                <template class="header">
                                    <vaadin-grid-sorter path="invoice.statut">
                            ...
                            <iron-collapse-button>
                                ...
                                <div slot="collapse-content">
                                    <vaadin-grid id="invoiceGrid" class="invoiceGrid" items="[[item.pat]]" multi-sort="[[multiSort]]" active-item="{{activeGridItem}}" on-tap="_showInvoiceToBeSendDetails">
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="8%" class="recipient-col">
                                            <template>
                                                <small>[[item.invoiceReference]]</small>
                                            </template>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column flex-grow="0" width="20%" class="recipient-col">
                                            <template>
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="8%" class="recipient-col">
                                            <template>
                                                <small>[[item.patientSsin]]</small>
                                            </template>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column flex-grow="0" width="8%" class="recipient-col">
                                            <template>
                                                <small>[[formatDate(item.invoiceDate,'date')]]</small>
                                            </template>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column-group>
                                            <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                                <template class="border-left">
                                                    <small><span class\$="[[_getTxtStatusColor(item.statut,item.reimbursement)]]">[[_formatAmount(item.reimbursement)]]€</span>
                                                    </small>
                                                </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                                <template>
                                                    <small><span class\$="[[_getTxtStatusColor(item.statut,item.patientIntervention)]]">[[_formatAmount(item.patientIntervention)]]€</span>
                                                    </small>
                                                </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                                <template>
                                                    <small><span class\$="[[_getTxtStatusColor(item.statut,item.doctorSupplement)]]">[[_formatAmount(item.doctorSupplement)]]€</span>
                                                    </small>
                                                </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col border-right">
                                                <template class="border-right">
                                                    <small><span class\$="[[_getTxtStatusColor(item.statut,item.totalAmount)]]">[[_formatAmount(item.totalAmount)]]€</span>
                                                    </small>
                                                </template>
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="10%" class="recipient-col">
                                            <template><span class\$="invoice-status [[_getIconStatusClass(item.statut)]]"><iron-icon icon="vaadin:circle" class\$="statusIcon [[_getIconStatusClass(item.statut)]]"></iron-icon> [[item.statut]]</span>
                                            </template>
                                        </vaadin-grid-column>
                                    </vaadin-grid>
                                    <vaadin-grid items="toggledDatas">
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="8%" class="recipient-col">
                                            <template class="footer">[[localize('tot_sub','Sous-total',language)]] :
                                            </template>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column-group>
                                            <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                                <template class="footer"><b>[[_formatAmount(item.sum.oa)]]€</b>
                                                </template>
                                            </vaadin-grid-column>
                                            <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                                <template class="footer"><b>[[_formatAmount(item.sum.pat)]]€</b>
                                                </template>
                                            </vaadin-grid-column>
                                            <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                                <template class="footer"><b>[[_formatAmount(item.sum.sup)]]€</b>
                                                </template>
                                            </vaadin-grid-column>
                                            <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col border-right">
                                                <template class="footer"><b>[[_formatAmount(item.sum.tot)]]€</b>
                                                </template>
                                            </vaadin-grid-column>
                                        </vaadin-grid-column-group>
                                        <vaadin-grid-column flex-grow="0" width="10%" class="recipient-col">
                            ...
                            <vaadin-grid-column-group>
                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                    <template class="footer">
                                        [[_formatAmount(totalInvoicesToBeSend.totalReimbursement)]]€
                                    </template>
                                </vaadin-grid-column>
                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                    <template class="footer">
                                        [[_formatAmount(totalInvoicesToBeSend.totalPatientIntervention)]]€
                                    </template>
                                </vaadin-grid-column>
                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                    <template class="footer">
                                        [[_formatAmount(totalInvoicesToBeSend.totalDoctorSupplement)]]€
                                    </template>
                                </vaadin-grid-column>
                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col border-right">
                                    <template class="footer">[[_formatAmount(totalInvoicesToBeSend.totalAmount)]]€
                                    </template>
                                </vaadin-grid-column>
                            </vaadin-grid-column-group>
                            <vaadin-grid-column flex-grow="0" width="10%" class="recipient-col">
                ...
                <div class="buttons">
                    ...
                    <template is="dom-if" if="[[api.tokenId]]">
                        <paper-button on-tap="receiveInvoices" class="modal-button" disabled="[[cannotGet]]">
                            [[localize('inv_get','Get',language)]]
                        </paper-button>
                        <paper-button on-tap="_checkBeforeSend" class="modal-button" disabled="[[cannotSend]]">
                            [[localize('inv_send','Send',language)]]
                        </paper-button>
                    </template>
                    <template is="dom-if" if="[[!api.tokenId]]">
                        <paper-button on-tap="" class="modal-button-error" disabled="" title="Pas de connexion ehealth active">[[localize('inv_get','Get',language)]]
                        </paper-button>
                        <paper-button on-tap="" class="modal-button-error" disabled="" title="Pas de connexion ehealth active">[[localize('inv_send','Send',language)]]
                        </paper-button>
                    </template>
                                ...
                                <div id="toBeCorrected" class="gridContainer grid-pending">
                                    <div class="scrollBox">
                                        <vaadin-grid id="noscroll" class="pendingTable" theme="force-sizer" items="[[messagesByStatus(invoicesStatus,messagesProcessed,messagesRejected,messagesAccepted, messagesToBeCorrected, messagesArchived)]]" active-item="{{activeGridItem}}" on-tap="_showMessagesToBeCorrectedDetail">
                                            <vaadin-grid-column flex-grow="0" width="4%" class="recipient-col">
                                                <template class="header">
                                                    <vaadin-grid-sorter path="sentMediumType">
                                                        [[localize('inv_type','Type',language)]]
                                                    </vaadin-grid-sorter>
                                                </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="10%" class="recipient-col">
                                                <template class="header">
                                                    <vaadin-grid-sorter path="insuranceCode">
                                                        [[localize('inv_mut','Mutual',language)]]
                                                    </vaadin-grid-sorter>
                                                </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="20%" class="recipient-col">
                                                <template class="header">
                                                    <vaadin-grid-sorter path="patientName">
                                                        [[localize('inv_pat','Patient',language)]]
                                                    </vaadin-grid-sorter>
                                                </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="8%" class="recipient-col">
                                                <template class="header">
                                                    <vaadin-grid-sorter path="patientSsin">
                                                        [[localize('inv_niss','Inss',language)]]
                                                    </vaadin-grid-sorter>
                                                </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="8%" class="recipient-col">
                                                <template class="header">
                                                    <vaadin-grid-sorter path="invoiceDate">
                                                        [[localize('inv_date','Invoice date',language)]]
                                                    </vaadin-grid-sorter>
                                                </template>
                                            ...
                                            <vaadin-grid-column-group>
                                                ...
                                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                                    <template class="header border-left">
                                                        <vaadin-grid-sorter path="reimbursement">
                                                            [[localize('inv_oa','Oa',language)]]
                                                        </vaadin-grid-sorter>
                                                    </template>
                                                ...
                                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                                    <template class="header">
                                                        <vaadin-grid-sorter path="patientIntervention">
                                                            [[localize('inv_pat','Patient',language)]]
                                                        </vaadin-grid-sorter>
                                                    </template>
                                                ...
                                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col">
                                                    <template class="header">
                                                        <vaadin-grid-sorter path="doctorSupplement">
                                                            [[localize('inv_supp','Extra',language)]]
                                                        </vaadin-grid-sorter>
                                                    </template>
                                                ...
                                                <vaadin-grid-column flex-grow="0" width="7%" class="recipient-col border-right">
                                                    <template class="header border-right">
                                                        <vaadin-grid-sorter path="totalAmount">
                                                            [[localize('inv_tot','Total',language)]]
                                                        </vaadin-grid-sorter>
                                                    </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="10%" class="recipient-col">
                                                <template class="header">
                                                    <vaadin-grid-sorter path="statut">
                                                        [[localize('inv_stat','Status',language)]]
                                                    </vaadin-grid-sorter>
                                                </template>
                                                <template>
                                ...
                                <div class="scrollBox">
                                    <vaadin-grid id="messagesGrid" class="processTable" items="[[messagesByStatus(invoicesStatus,messagesProcessed,messagesRejected,messagesAccepted, messagesToBeCorrected, messagesArchived)]]" active-item="{{activeGridItem}}" on-tap="_showDetail">
                                        <vaadin-grid-column flex-grow="0" width="14%" class="recipient-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.hcp">
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="4%" class="oa-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.oa">
                                                    [[localize('inv_oa','Oa',language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="20%" class="ref-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.hcpReference">
                                                    [[localize('inv_phys_ref','Physician reference',language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="8%" class="invoice-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.invoiceNumber">
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="8%" class="month-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.invoiceMonth">
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="9%" class="invoiceDate-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.invoiceDate">
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="8%" class="invAmount-col">
                                            ...
                                            <template><span class\$="[[_getTxtStatusColor(_getIconStatusClass(item.messageInfo.invoiceStatus),item.messageInfo.refusedAmount)]]">[[_formatAmount(item.messageInfo.invoicedAmount)]]€</span>
                                            </template>
                                            <template class="footer">
                                                [[_invoicedAmount(invoicesStatus,messagesProcessed,messagesRejected,messagesAccepted,
                                                messagesToBeCorrected, messagesArchived)]]€
                                            </template>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column flex-grow="0" width="8%" class="accAmount-col">
                                            ...
                                            <template><span class\$="[[_getTxtStatusColor(_getIconStatusClass(item.messageInfo.invoiceStatus),item.messageInfo.refusedAmount)]]">[[_formatAmount(item.messageInfo.acceptedAmount)]]€</span>
                                            </template>
                                            <template class="footer">
                                                [[_acceptedAmount(invoicesStatus,messagesProcessed,messagesRejected,messagesAccepted,
                                                messagesToBeCorrected, messagesArchived)]]€
                                            </template>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column flex-grow="0" width="8%" class="refAmount-col">
                                            ...
                                            <template><span class\$="[[_getTxtStatusColor(_getIconStatusClass(item.messageInfo.invoiceStatus),item.messageInfo.refusedAmount)]]">[[_formatAmount(item.messageInfo.refusedAmount)]]€</span>
                                            </template>
                                            <template class="footer">
                                                [[_refusedAmount(invoicesStatus,messagesProcessed,messagesRejected,messagesAccepted,
                                                messagesToBeCorrected, messagesArchived)]]€
                                            </template>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column flex-grow="0" width="10%" class="stat-col">
                                            ...
                                            <template><span class\$="invoice-status [[_getIconStatusClass(item.messageInfo.invoiceStatus)]]"><iron-icon icon="vaadin:circle" class\$="statusIcon [[_getIconStatusClass(item.messageInfo.invoiceStatus)]]"></iron-icon> [[item.messageInfo.invoiceStatus]]</span>
                                            </template>
                                        </vaadin-grid-column>
                                ...
                                <div class="scrollBox">
                                    <vaadin-grid id="messagesGrid" class="archiveAndAccepted" items="[[messagesByStatus(invoicesStatus,messagesProcessed,messagesRejected,messagesAccepted, messagesToBeCorrected, messagesArchived)]]" active-item="{{activeGridItem}}" on-tap="_showDetail">
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="9%" class="invoice-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.invoiceNumber">
                                                    [[localize('inv_batch_num','Batch number',language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                            <template>[[item.messageInfo.invoiceNumber]]</template>
                                        </vaadin-grid-column>
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="9%" class="invAmount-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.invoicedAmount">
                                                    [[localize('inv_batch_amount','Amount',language)]]
                                                    [[localize('inv_batch_amount_invoiced','Invoiced',language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                            <template><span class\$="[[_getTxtStatusColor(_getIconStatusClass(item.messageInfo.invoiceStatus),item.messageInfo.refusedAmount)]]">[[_formatAmount(item.messageInfo.invoicedAmount)]]€</span>
                                            </template>
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="9%" class="accAmount-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.acceptedAmount">
                                                    [[localize('inv_batch_amount','Amount',language)]]
                                                    [[localize('inv_batch_amount_acc','Accepted',language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                            ...
                                            <template><span class\$="[[_getTxtStatusColor('force-green',item.messageInfo.acceptedAmount)]]">[[_formatAmount(item.messageInfo.acceptedAmount)]]€</span>
                                            </template>
                                            <template class="footer">
                                                [[_acceptedAmount(invoicesStatus,messagesProcessed,messagesRejected,messagesAccepted,
                                                messagesToBeCorrected, messagesArchived)]]€
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="9%" class="refAmount-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.refusedAmount">
                                                    [[localize('inv_batch_amount','Amount',language)]]
                                                    [[localize('inv_batch_amount_rej','Rejected',language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                            <template><span class\$="[[_getTxtStatusColor('force-red',item.messageInfo.refusedAmount)]]">[[_formatAmount(item.messageInfo.refusedAmount)]]€</span>
                                            </template>
                                            <template class="footer">
                                                [[_refusedAmount(invoicesStatus,messagesProcessed,messagesRejected,messagesAccepted,
                                        ...
                                        <vaadin-grid-column-group>
                                            <template class="header hasBorder">
                                                [[localize('inv_batch_paid_informations','Paid information',language)]]
                                            </template>
                                            <vaadin-grid-column flex-grow="0" width="7%" class="reject-col">
                                                <template class="header">
                                                    <vaadin-grid-sorter path="messageInfo.paymentReference">
                                                        [[localize('inv_batch_paid_reference','Ref',language)]]
                                                    </vaadin-grid-sorter>
                                                </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="7%" class="reject-col">
                                                <template class="header">
                                                    <vaadin-grid-sorter path="messageInfo.amountPaid">
                                                        [[localize('inv_batch_amount','Amount',language)]]
                                                    </vaadin-grid-sorter>
                                                </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="7%" class="reject-col">
                                                <template class="header">
                                                    <vaadin-grid-sorter path="messageInfo.paid">
                                                        [[localize('inv_batch_amount_paid','Paid',language)]]
                                                    </vaadin-grid-sorter>
                                                </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="7%" class="reject-col">
                                                <template class="header">
                                                    <vaadin-grid-sorter path="messageInfo.paymentAccount">
                                                        [[localize('inv_batch_bank_account','Bank account',language)]]
                                                    </vaadin-grid-sorter>
                                                </template>
                                            ...
                                            <vaadin-grid-column flex-grow="0" width="7%" class="reject-col">
                                                <template class="header">
                                                    <vaadin-grid-sorter path="messageInfo.paymentDate">
                                                        [[localize('inv_batch_paid_date','Payement date',language)]]
                                                    </vaadin-grid-sorter>
                                                </template>
                                ...
                                <div class="scrollBox">
                                    <vaadin-grid id="messagesGrid" class="rejectedTable" items="[[messagesByStatus(invoicesStatus,messagesProcessed,messagesRejected,messagesAccepted, messagesToBeCorrected, messagesArchived)]]" active-item="{{activeGridItem}}" on-tap="_showDetail" selected="{{selectedInvoiceIndex}}">
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="8%" class="month-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.invoiceMonth">
                                                    [[localize('inv_batch_month','Invoiced month',language)]]
                                                </vaadin-grid-sorter>
                                            </template>
                                            <template>[[formatDate(item.messageInfo.invoiceMonth,'month')]]</template>
                                        </vaadin-grid-column>
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="7%" class="refAmount-col">
                                            ...
                                            <template><span class\$="[[_getTxtStatusColor(item.messageInfo.invoiceStatus,item.messageInfo.refusedAmount)]]">[[_formatAmount(item.messageInfo.refusedAmount)]]€</span>
                                            </template>
                                            <template class="footer">
                                                [[_refusedAmount(invoicesStatus,messagesProcessed,messagesRejected,messagesAccepted,
                                                messagesToBeCorrected, messagesArchived)]]€
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="7%" class="reject-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="messageInfo.rejectionReason">Motif rejet
                                                </vaadin-grid-sorter>
                                            </template>
                        ...
                        <template is="dom-if" if="[[ifInvoiceSelected]]">
                            <template is="dom-if" if="[[isToBeCorrected(invoicesStatus.*)]]">
                                <h4 class="mb0">Détail de la facture n°[[activeGridItem.invoice.invoiceReference]]
                                    <paper-button on-tap="_openInvoicingDialog" class="modal-button alignRight" id="[[activeGridItem.invoice.id]]">Corriger
                                    </paper-button>
                                    <paper-button class="grid-btn-small modal-button-rejected alignRight" on-tap="_flagInvoiceAsLostConfirmationDialog" data-invoice-id\$="[[activeGridItem.invoice.id]]">
                                        <iron-icon icon="error" class="force-left" data-invoice-id\$="[[activeGridItem.invoice.id]]"></iron-icon> &nbsp;
                                ...
                                <div id="toBeCorrectedDetailContainer" class="toBeCorrectedContainerDetail">
                                    <vaadin-grid id="toBeCorrectedGridDetail" items="[[toBeCorrectedMessageDetail]]">
                                        <vaadin-grid-column class="recipient-col">
                                            <template class="header">
                                                [[localize('inv_batch_paid_date','Date',language)]]
                                            </template>
                                        ...
                                        <vaadin-grid-column-group class="hasBorder">
                                            ...
                                            <vaadin-grid-column class="recipient-col">
                                                ...
                                                <template><span class\$="[[_getTxtStatusColor(item.messageInfo.invoiceStatus,item.reimbursement)]]">[[_formatAmount(item.reimbursement)]]</span>
                                                </template>
                                            </vaadin-grid-column>
                                            <vaadin-grid-column class="recipient-col">
                                                <template class="header">Patient</template>
                                                <template><span class\$="[[_getTxtStatusColor(item.messageInfo.invoiceStatus,item.patientIntervention)]]">[[_formatAmount(item.patientIntervention)]]</span>
                                                </template>
                                            </vaadin-grid-column>
                                            <vaadin-grid-column class="recipient-col">
                                                <template class="header">Supplément</template>
                                                <template><span class\$="[[_getTxtStatusColor(item.messageInfo.invoiceStatus,item.doctorSupplement)]]">[[_formatAmount(item.doctorSupplement)]]</span>
                                                </template>
                                            </vaadin-grid-column>
                                            <vaadin-grid-column class="recipient-col">
                                                <template class="header">Total</template>
                                                <template><span class\$="[[_getTxtStatusColor(item.messageInfo.invoiceStatus,item.totalAmount)]]">[[_formatAmount(item.totalAmount)]]</span>
                                                </template>
                                            </vaadin-grid-column>
                                        </vaadin-grid-column-group>
                                        <vaadin-grid-column class="recipient-col" width="300px">
                                            <template class="header">Erreur</template>
                                            <template><span class\$="[[_getIconStatusClass(item.error)]]">[[item.error]]</span>
                                            </template>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column class="recipient-col">
                                            <template class="header"></template>
                                    ...
                                    <paper-tooltip for="edit-[[item.id]]" position="left">
                                        [[localize('edit','Edit',language)]]
                                    </paper-tooltip>
                                </div>
                            </template>
                            <template is="dom-if" if="[[!isToBeCorrected(invoicesStatus.*)]]">
                                <h4 class="invoice-detail-title mb0">Détail de l'envoi
                                    n°[[activeGridItem.message.externalRef]] de l'oa [[activeGridItem.messageInfo.oa]]
                                    <template is="dom-if" if="[[!isArchived(invoicesStatus.*)]]">
                                        <paper-button class="modal-button alignRight" on-tap="_openArchiveDialogForBatch">Archiver
                                        </paper-button>
                                    </template>
                                    <template is="dom-if" if="[[isSendError]]">
                                        <paper-button class="modal-button alignRight" on-tap="_transferInvoicesForResending">Transférer pour réenvoi
                                        </paper-button>
                                    </template>
                                </h4>
                                <template is="dom-if" if="[[invoicesErrorMsg]]">
                                    <h2 class="invoice-error-msg mb0 invoice-status--redStatus">
                                        [[invoicesErrorMsg]]</h2>
                                </template>
                                ...
                                <div id="invoiceDetailContainer" class="invoiceDetailContainer">
                                    <vaadin-grid id="invoiceGridDetail" data-provider="[[_batchDetailDataProvider()]]">
                                        <vaadin-grid-column flex-grow="0" width="96px" class="recipient-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="invoiceReference">N° fact.
                                                </vaadin-grid-sorter>
                                            </template>
                                            <template>
                                        ...
                                        <vaadin-grid-column-group>
                                            <vaadin-grid-column flex-grow="0" width="5%" class="recipient-col">
                                                ...
                                                <template><span class\$="[[_getTxtStatusColor(item.statut,item.totalAmount)]]">[[_formatAmount(item.invoicedAmount)]]€</span>
                                                </template>
                                            </vaadin-grid-column>
                                            <vaadin-grid-column flex-grow="0" width="5%" class="recipient-col">
                                                <template class="header">
                                                ...
                                                <template><span class\$="[[_getTxtStatusColor('force-green',item.acceptedAmount)]]">[[_formatAmount(item.acceptedAmount)]]€</span>
                                                </template>
                                            </vaadin-grid-column>
                                            <vaadin-grid-column flex-grow="0" width="5%" class="recipient-col">
                                                <template class="header">
                                                ...
                                                <template><span class\$="[[_getTxtStatusColor('force-red',item.refusedAmount)]]">[[_formatAmount(item.refusedAmount)]]€</span>
                                                </template>
                                            </vaadin-grid-column>
                                        </vaadin-grid-column-group>
                                        <vaadin-grid-column flex-grow="0" width="36%" class="rejectionReason-col">
                                            <template class="header">
                                                <vaadin-grid-sorter path="rejectionReason">Motif de rejet
                                                </vaadin-grid-sorter>
                                            </template>
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="5%" class="recipient-col">
                                            ...
                                            <template><span class\$="[[_getTxtStatusColor(item.statut,item.paid)]]">[[_formatAmount(item.paid)]]€</span>
                                            </template>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column flex-grow="0" width="10%" class="recipient-col">
                                            <template class="header">
        ...
        <paper-dialog id="helpdeskInfoDialog">
            ...
            <div class="modal-content">
                ...
                <paper-input class="flex-container-nmcl-row modal-input" always-float-label="" label="Montant facturé" value="[[infoHelpdesk.messageInfo.invoicedAmount]]" readonly="">
                    <div slot="suffix">€</div>
                </paper-input>
                ...
                <template is="dom-if" if="[[isCompleteHelpDesk]]">
                    ...
                    <paper-input class="flex-container-nmcl-row modal-input" always-float-label="" label="Montant facturé" value="[[infoHelpdeskDet.invoicedAmount]]" readonly="">
                        <div slot="suffix">€</div>
                    </paper-input>
                    <paper-input class="flex-container-nmcl-row modal-input" always-float-label="" label="Montant accepté" value="[[infoHelpdeskDet.acceptedAmount]]" readonly="">
                        <div slot="suffix">€</div>
                    </paper-input>
                    <paper-input class="flex-container-nmcl-row modal-input" always-float-label="" label="Montant refusé" value="[[infoHelpdeskDet.refusedAmount]]" readonly="">
                        <div slot="suffix">€</div>
                    </paper-input>
        ...
        <paper-dialog id="archiveDialog">
            ...
            <div class="buttons">
                <paper-button class="modal-button" dialog-dismiss="">[[localize('clo','Close',language)]]</paper-button>
                <paper-button class="modal-button" on-tap="_archiveBatch">[[localize('confirm','Confirm',language)]]
                </paper-button>
            </div>
        </paper-dialog>
        ...
        <paper-dialog id="warningBeforeSend">
            ...
            <div class="previousCheck">
                ...
                <template is="dom-if" if="[[!checkBeforeSendEfact.invoiceCheck100]]">
                    <div class="errorBeforeSendInvoice">[[localize('pr_inv_numb_OA100_inv','- Anomaly detected on the
                        invoiceNumber for OA100. Please contact helpdesk',language)]]
                    </div>
                </template>
                <template is="dom-if" if="[[!checkBeforeSendEfact.invoiceCheck200]]">
                    <div class="errorBeforeSendInvoice">[[localize('pr_inv_numb_OA200_inv','- Anomaly detected on the
                        invoiceNumber for OA200. Please contact helpdesk',language)]
                    </div>
                </template>
                <template is="dom-if" if="[[!checkBeforeSendEfact.invoiceCheck300]]">
                    <div class="errorBeforeSendInvoice">[[localize('pr_inv_numb_OA300_inv','- Anomaly detected on the
                        invoiceNumber for OA300. Please contact helpdesk',language)]]
                    </div>
                </template>
                <template is="dom-if" if="[[!checkBeforeSendEfact.invoiceCheck306]]">
                    <div class="errorBeforeSendInvoice">[[localize('pr_inv_numb_OA306_inv','- Anomaly detected on the
                        invoiceNumber for OA306. Please contact helpdesk',language)]]
                    </div>
                </template>
                <template is="dom-if" if="[[!checkBeforeSendEfact.invoiceCheck400]]">
                    <div class="errorBeforeSendInvoice">[[localize('pr_inv_numb_OA400_inv','- Anomaly detected on the
                        invoiceNumber for OA400. Please contact helpdesk',language)]]
                    </div>
                </template>
                <template is="dom-if" if="[[!checkBeforeSendEfact.invoiceCheck500]]">
                    <div class="errorBeforeSendInvoice">[[localize('pr_inv_numb_OA500_inv','- Anomaly detected on the
                        invoiceNumber for OA500. Please contact helpdesk',language)]]
                    </div>
                </template>
                <template is="dom-if" if="[[!checkBeforeSendEfact.invoiceCheck600]]">
                    <div class="errorBeforeSendInvoice">[[localize('pr_inv_numb_OA600_inv','- Anomaly detected on the
                        invoiceNumber for OA600. Please contact helpdesk',language)]]
                    </div>
                </template>
                <template is="dom-if" if="[[!checkBeforeSendEfact.invoiceCheck900]]">
                    <div class="errorBeforeSendInvoice">[[localize('pr_inv_numb_OA900_inv','- Anomaly detected on the
                        invoiceNumber for OA900. Please contact helpdesk',language)]]
                    </div>
                </template>
            </div>
            <template is="dom-if" if="[[patientWithoutMutuality.length]]">
                <div class="unsentInvoice">
                    ...
                    <vaadin-grid id="patientsWithoutAssurabilityGrid" items="[[patientWithoutMutuality]]">
                        <vaadin-grid-column class="recipient-col">
                            <template class="header">
                                <vaadin-grid-sorter path="patientName">[[localize('pr_pat_inv','Patient',language)]]
                                </vaadin-grid-sorter>
                            </template>
                            <template>[[item.patientName]]</template>
                        </vaadin-grid-column>
                        <vaadin-grid-column class="recipient-col">
                            ...
                            <template>[[localize('pr_pat_ass_inf_inv','Patient without assurability
            ...
            <div class="buttons">
                <paper-button class="modal-button" dialog-dismiss="">[[localize('clo','Close',language)]]</paper-button>
                <template is="dom-if" if="[[patientWithoutMutuality.length]]">
                    <paper-button class="modal-button" on-tap="sendInvoices">
                        [[localize('continue','Continue',language)]]
                    </paper-button>
                </template>
            </div>
        </paper-dialog>
...
`;
                      ...
                      return this.api.document().findByMessage(this.user.healthcarePartyId, msg)
                          .then(docs => {
                              ...
                              return jsonDoc ? this.api.document().getAttachment(jsonDoc.id, jsonDoc.attachmentId, jsonDoc.secretForeignKeys).then(a => {
                                  ...
                                  if (typeof a === "string") {
                                      try {
                                          a = JSON.parse(this.cleanStringForJsonParsing(a))
                                      } catch (ignored) {
                                      }
                                  } else if (typeof a === "object") {
                                      try {
                                          a = JSON.parse(this.cleanStringForJsonParsing(new Uint8Array(a).reduce((data, byte) => data + String.fromCharCode(byte), '')));
                                      } catch (ignored) {
                                      }
                                  }
  ...
  _toggleDataProvider(input) {
      ...
      inputList.map(patient => {
          ...
          let grpExists = false
          mutGroups.map(grp => {
              if (grp.code === thisMutGrp) { // check if grp exists
                  grpExists = true
              }
          })
          if (!grpExists) {
              mutGroups.push({ // else create object
                  code: thisMutGrp, pat: [], sum: {oa: 0, pat: 0, sup: 0, tot: 0}
              })
          }
          mutGroups.map(grp => {
              if (grp.code === thisMutGrp) { // get into this grp
  ...
  checkFilters(str) { // will (set the path type for vaadin filter and) format search output
      ...
      if (str.length) {
          if (!isNaN(str)) { // is a number
              ...
              // format output ->
              if (searchType === "patient.ssin") {
                  output = this.findAndReplace(output, '.', '');
              }
              if (searchType === "invoice.totalAmount") {
                  output = this.findAndReplace(output.toString(), '.', ',')
              }
          } else { // is a string
              searchType =
              ...
              if (searchType === "invoice.invoiceDate") {
                  output = this.fourNumYears(output)
              }
          }
      }
      return output
  ...
  sendInvoices() {
      ...
      if (maySend) {
          ...
          this.api.hcparty().getHealthcareParty(this.user.healthcarePartyId).then(hcp => {
              ...
              _.chain(this.selectedInvoicesToBeSend.filter(inv => inv.insurabilityCheck === true))
                  .groupBy(fact => fact.insuranceParent)
                  .toPairs().value()
                  .forEach(([fedId, invoices]) => {
                      prom = prom.then(() => this.api.message().sendBatch(this.user, hcp, invoices.map(iv => ({
                              invoiceDto: iv.invoice,
                              patientDto: iv.patient
                          })), this.api.keystoreId, this.api.tokenId, this.api.credentials.ehpassword, this.api.fhc().Efactcontroller(),
                          undefined,
  ...
  initializeBatchCounter(toBeSend, toBeCorrected, processedCounter, acceptedCounter, rejectedCounter, archivedCounter) {
      this.dispatchEvent(new CustomEvent('initialize-batch-counter', {
          bubbles: true,
          composed: true,
          detail: {
              toBeSend: toBeSend,
              toBeCorrected: toBeCorrected,
              processing: processedCounter,
  ...
  _isPartiallyAccepted(test) {
      return (test === this.localize('inv_par_acc', 'Partially accepted', this.language))
  }
      ...
      if (this.activeGridItem && this.activeGridItem.message && this.activeGridItem.message.id && this.activeGridItem.message.invoiceIds.length) {
          ...
          this.api.invoice().getInvoices(new models.ListOfIdsDto({ids: this.activeGridItem.message.invoiceIds.map(id => id)}))
              .then(invs => {
                  ...
                  prom.then(() => {
                      this.api.message().getMessage(this.activeGridItem.message.id).then(msg => {
                          this._setLoadingMessage({
                              message: this.localize('arch_mess', this.language),
                              icon: "arrow-forward"
                          });
                          msg.status = (msg.status | (1 << 21))
                          this.api.message().modifyMessage(msg)
