  ...
  static get template() {
    return html`
        ...
        <div class="invoice-panel">
            <template is="dom-if" if="[[isLoadingDmg]]">
                <ht-spinner class="center" active="[[isLoadingDmg]]"></ht-spinner>
            </template>
            <div class="grid-panel">
                <div id="messagesGrid" class\$="[[detailPanelClass(tempoSubMessages)]]">
                    <paper-input id="filter" label="[[localize('fil','Filter',language)]]" value="{{leftFilterValue}}" on-keydown="refreshFilter">
                    </paper-input>
                    <vaadin-grid id="messages-vaadin-grid" active-item="{{activeItem}}" selected-items="{{selectedItems}}" data-provider="[[_messagesDataProvider()]]">
                        ...
                        <vaadin-grid-column class="received-col" width="120px">
                            <template class="header">
                                <vaadin-grid-sorter path="sentreceived">[[localize("dat","Date",language)]]
                                </vaadin-grid-sorter>
                            </template>
                        ...
                        <vaadin-grid-column class="subject-col" width="calc(30% - 93px)">
                            <template class="header">
                                <vaadin-grid-sorter path="subject">[[localize("tel_type","Type",language)]]
                                </vaadin-grid-sorter>
                            </template>
                            <template>
                                [[item.subject]]
                        ...
                        <vaadin-grid-column class="adress-col" width="80px">
                            <template class="header">
                                <vaadin-grid-sorter path="fromAddress">[[localize("From","From",language)]]
                                </vaadin-grid-sorter>
                            </template>
                        ...
                        <vaadin-grid-column class="patient-col" width="calc(30% - 93px)">
                            <template class="header">
                                <vaadin-grid-sorter path="patient.lastName">[[localize("pati","Patient",language)]]
                                </vaadin-grid-sorter>
                            </template>
                        ...
                        <vaadin-grid-column class="metas-col" width="calc(30% - 93px)">
                            <template class="header">
                                <vaadin-grid-sorter path="metas">[[localize("info","Informations",language)]]
                                </vaadin-grid-sorter>
                            </template>
                            <template>
                                <template is="dom-if" if="[[_listRequest(item.metas.type)]]">
                                    <template is="dom-repeat" items="[[ios]]" as="io">
                                        <template is="dom-if" if="[[_tack(item.metas,io)]]">
                                            <iron-icon class="status" id="status_[[item.id]]_[[io]]" icon="done"></iron-icon>
                                        ...
                                        <template is="dom-if" if="[[!_tack(item.metas,io)]]">
                                            ...
                                            <paper-tooltip position="right" id="tt_status_[[item.id]]_[[io]]" for="status_[[item.id]]_[[io]]">
                                                [[io]]: [[localize("req_not_yet_rec","request not yet
                                                received",language)]]
                                            </paper-tooltip>
                                        </template>
                                    </template>
                ...
                <div id="detailGrid" class\$="[[detailPanelClass(tempoSubMessages)]]">
                    <paper-input id="subFilter" label="[[localize('fil','Filter',language)]]" value="{{rightFilterValue}}" on-keydown="refreshSubFilter">
                    </paper-input>
            ...
            <div class="btns-container">
                ...
                <paper-button class="bottom-btn" id="requestMessagesBtn" on-tap="requestMessages" disabled="[[isLoadingReq]]">[[localize('req_msg_lists','Request Messages
                    Lists',language)]]
                </paper-button>
                <paper-button class="bottom-btn" id="getMessagesBtn" on-tap="getMessages" disabled="[[isLoadingDmg]]">
                    [[localize('get_msg','Get Messages',language)]]
                </paper-button>
            </div>
        </div>
`;
  ...
  _total(messages) {
      return _.toPairs(messages.reduce((acc, m) => {
          m.payments.forEach(p => acc[(p.currency || 'eur').toLowerCase()] = (acc[(p.currency || 'eur').toLowerCase()] || 0) + p.amount);
          return acc
      }, {})).map(([c, a]) => `${a.toFixed(2)} ${c.replace('eur', '€')}`)
  }
  ...
  _messagesDataProvider() {
      return (params, callback) => {
          if (!params.sortOrders || !params.sortOrders.length) {
              params.sortOrders = [{path: 'sentreceived', direction: 'desc'}]
          }
