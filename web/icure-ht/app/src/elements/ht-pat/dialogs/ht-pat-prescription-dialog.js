  ...
  static get template() {
    return html`
        ...
        <paper-dialog id="confirmNoRecipe" class="presc-dialog">
            ...
            <div class="endline">
                <div class="buttons">
                    <paper-button class="modal-button modal-button--cancel" on-tap="" dialog-dismiss="">[[localize('can',
                        "Annuler", language)]]
                    </paper-button>
                    <paper-button class="modal-button modal-button--cancel" on-tap="_printPrescriptionNoRecipe" disabled="[[!_printEnabled(_drugsOnPrescriptions,_drugsOnPrescriptions.*,_splitColumns)]]">
                        [[localize('print_no_recipe', "Imprimer sans recipe", language)]]
                    </paper-button>
                    <paper-button class="modal-button modal-button--save" dialog-dismiss="" autofocus="" on-tap="_printPrescriptions" disabled="[[!_printEnabled(_drugsOnPrescriptions,_drugsOnPrescriptions.*,_splitColumns)]]">
                        [[localize('try_again','Réessayer',language)]]
                    </paper-button>
                </div>
            </div>
        </paper-dialog>
        ...
        <paper-dialog id="tryAgainRevoke" class="presc-dialog">
            ...
            <div class="endline">
                <div class="buttons">
                    ...
                    <paper-button class="modal-button modal-button--save" dialog-dismiss="" autofocus="" on-tap="_revokeTryAgainCallback">[[localize('try_again','Réessayer',language)]]
                    </paper-button>
                </div>
            </div>
        </paper-dialog>
        ...
        <paper-dialog id="confirmRevoke" class="presc-dialog">
            ...
            <div class="endline">
                <div class="buttons">
                    ...
                    <paper-button class="modal-button modal-button--save" dialog-dismiss="" autofocus="" on-tap="_revokeTryAgainCallback">[[localize('revoke','Révoquer',language)]]
                    </paper-button>
                </div>
            </div>
        </paper-dialog>
                            ...
                            <template>
                                <div class="cell frozen">[[item.drugDescription]]
                                    <template is="dom-if" if="[[_drugIsType(item, 'local')]]">
                                        [[localize('recipe_not_found_remote', '(Non trouvé sur serveur recipe)',
                                        language)]]
                                    </template>
                                </div>
                            </template>
            ...
            <template is="dom-if" if="[[_isToSendView(prescriptionTab)]]">
                ...
                <div class="content">
                    <vaadin-grid id="entities-list" class="material" overflow="bottom" items="[[_drugsOnPrescriptions]]" active-item="{{activeDrug}}">
                        <vaadin-grid-column width="100px">
                            <template class="header">
                                <vaadin-grid-sorter path="description">[[localize('dru_des','Drug descr.',language)]]
                                </vaadin-grid-sorter>
                            </template>
                            <template>
                    ...
                    <template is="dom-if" if="[[!fullProfile]]">
                        <paper-button class="warning" on-tap="_myProfile">
                            <iron-icon icon="warning"></iron-icon>
                            [[localize('recipe_warning','To print recip-e, you HAVE to inquire your ADRESS and PHONE
                            NUMBER in your profile !',language)]]
                        </paper-button>
                    </template>
                </div>
                <div class="endline buttons">
                    ...
                    <div class="buttons">
                        ...
                        <paper-button class="modal-button modal-button--save" dialog-confirm="" autofocus="" on-tap="_printPrescriptions" disabled="[[!_printEnabled(_drugsOnPrescriptions,_drugsOnPrescriptions.*,_splitColumns)]]">
                            [[localize('pri','Print',language)]]
                        </paper-button>
                    </div>
                </div>
            </template>
...
`;
  ...
  _printRID(e) {
      ...
      const toPrint = this._formatPrescriptionsBody(splitCols, ridDrugs, this.patient, this.globalHcp, moment(this.deliveryDateString + "").format("DD/MM/YYYY"), element)
  ...
  _printPrescriptions(e) {
      ...
      if (this.patient.ssin && this.api.tokenId) { // if ehealth connected
          this.api.hcparty().getHealthcareParty(this.user.healthcarePartyId).then(hcp =>
              Promise.all(
                  splitColumns.map(c =>
                      this.api.fhc().Recipecontroller().createPrescriptionUsingPOST(this.api.keystoreId, this.api.tokenId, "persphysician", hcp.nihii, hcp.ssin, hcp.lastName, this.api.credentials.ehpassword, {
                          ...
                          hcp: hcp,
                          feedback: false,
                          medications: drugsToBePrescribed.filter(s => c.drugIds.includes(s.id)).map(s => this.addEmptyPosologyIfNeeded(this.api.contact().medicationValue(s, this.language))),
              ...
              .then(() => {
                  toPrint = this._formatPrescriptionsBody(splitColumns, drugsToBePrescribed, this.patient, this.globalHcp, moment(this.deliveryDateString + "").format("DD/MM/YYYY"), element)
  ...
  addEmptyPosologyIfNeeded(mv) {
      if (mv.instructionForPatient || mv.regimen && mv.regimen.length > 0) {
          ...
          mv.instructionForPatient = this.localize('not_applicable', 'not applicable', this.language);//"pas d'application";
          return mv;
      }
  }
  ...
  _formatPrescriptionsBody(splitColumns, drugsToBePrescribed, patient, hcp, deliveryDate, element) {
      ...
      splitColumns.forEach((c, idx) => {
          ...
          prescriToPrint.push({
              ...
              'prescriBody': prescri,
              'page': pageNum,
              'posology': articlePoso,
              'medWithPosology': articleMedWithPoso,
              'myRid': ridLabel,
              'myJpegUrl': jpegUrl
          }) // add a prescription with its datas
          ...
          if (typeof onePage == 'object') {
              ...
              onePage.map((onePrescription) => {
                  ...
                  if (inRecipeMode) {
                      ...
                      onePrescription.poso.map((singlePoso) => {
                          posoTable += "<tr><td><b>" + singlePoso.medR + "</b></td><td>" + singlePoso.poso.toString() + "</td></tr>"
                      }) // construct the posology body
                  } else {
                      thatBody += `
                      <div id="prescription">
  ...
  _pdfReport(drugsToBePrescribed, toPrint, size) {
      ...
      return this.api.pdfReport(mustache.render(toPrint, null), {
          ...
          .then(({pdf: pdfFileContent, printed: wasPrinted}) => _.assign({
...
//TODO                              metas: pdfPrintingData.documentMetas,
          ...
          .then(pdfPrintingData => {
              this.dispatchEvent(new CustomEvent('save-document-as-service', {
                  detail: {
  ...
  _markDrugsAsSent(drugsToBePrescribed) {
      ...
      this.dispatchEvent(new CustomEvent("save-contact", {composed: true, bubbles: true}))
  }

  _markDrugsAsNotSent(drugsToBePrescribed) {
      drugsToBePrescribed.forEach(s => {
          this.api.contact().medicationValue(s, this.language).prescriptionRID = null
          const tag = s.tags.find(t => t.type === 'CD-LIFECYCLE')
          if (tag) {
              tag.id = 'CD-LIFECYCLE|active|1';
              tag.code = 'active'
          } else {
              s.tags.push(this.api.code().normalize({id: 'CD-LIFECYCLE|active|1'}))
          }
      })
      this._refreshDrugsToBePrescribed()
