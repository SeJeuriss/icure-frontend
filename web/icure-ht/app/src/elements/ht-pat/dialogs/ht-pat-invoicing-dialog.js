    ...
    return html`
        ...
        <paper-dialog id="invoiceDialog" opened="{{opened}}">
            <div class="invoice-panel">
                <paper-listbox id="_invoice_listbox" focused="" selectable="paper-material" selected="{{selectedInvoiceIndex}}">
                    <template is="dom-repeat" items="[[invoices]]">
                        <paper-material class="layout vertical invoice-item invoice--big" id="[[item.id]]">
                            <paper-item class="invoice-text">
                                ...
                                <div class="invoice-text-row grey">
                                    <h4>[[getInvoiceType(item.invoiceType)]]
                                        [[getSentMediumType(item.sentMediumType)]]</h4>
                                    <p>
                                    ...
                                    <div class="invoiceDateInfo">
                                        <template is="dom-if" if="[[!_isInvoiceEattest(item,item.*)]]">
                                            <template is="dom-if" if="[[_ifStatusDateExist(item.printedDate,item.*)]]">
                                                <iron-icon icon="vaadin:print" class="invoiceDateInfoIcon"></iron-icon>
                                                <span>[[_formatInvoiceDate(item.printedDate)]]</span>
                                            </template>
                                        </template>
                                        <template is="dom-if" if="[[_isInvoiceEattest(item,item.*)]]">
                                            <template is="dom-if" if="[[_ifStatusDateExist(item.printedDate,item.*)]]">
                                                <paper-icon-button icon="vaadin:print" class="he-edit-btn" style="color:green" on-tap="_cancelPrintedDate" data-item\$="[[item.id]]"></paper-icon-button>
                                                <span>[[_formatInvoiceDate(item.printedDate)]]</span>
                                            </template>
                                            <template is="dom-if" if="[[_ifClosedButNotPrinted(item,item.*)]]">
                                                <paper-icon-button icon="vaadin:print" class="he-edit-btn" style="color:red" on-tap="_generatePrintedDate" data-item\$="[[item.id]]"></paper-icon-button>
                                            </template>
                                        </template>
                                        <template is="dom-if" if="[[_ifStatusDateExist(item.sentDate)]]">
                                            <iron-icon icon="vaadin:paperplane" class="invoiceDateInfoIcon"></iron-icon>
                                        ...
                                        <template is="dom-if" if="[[_ifEattestSaved(item,item.printedDate)]]">
                                            ...
                                            <paper-tooltip position="bottom" for="he-edit-btn-open_[[item.id]]">
                                                [[localize('ext','Extend',language)]]
                                            </paper-tooltip>
                                            <paper-tooltip position="bottom" for="he-edit-btn-xml_[[item.id]]-khmer">
                                                [[localize('gen_kmehr','Download Kmehr',language)]]
                                            </paper-tooltip>
                                            <paper-tooltip position="bottom" for="he-edit-btn-xml_[[item.id]]-xades">
                                                [[localize('gen_xades','Download Xades',language)]]
                                            </paper-tooltip>
                                            <paper-tooltip position="bottom" for="he-edit-btn-duplicate_[[item.id]]">
                                                [[localize('gen_duplicate','Generate duplicata',language)]]
                                            </paper-tooltip>
                                        </template>
                                    </div>
                                </div>
                ...
                <div class="invoice-header">
                    ...
                    <div class="mobile-col">
                        <div class="flex-container">
                            ...
                            <paper-input id="invoice-number" label="[[localize('inv_num','Invoice number',language)]]" value="{{selectedInvoice.invoiceReference}}" readonly="" always-float-label="true"></paper-input>
                            ...
                            <template is="dom-if" if="[[!_isSelectedInvoiceEattest(selectedInvoice.*)]]">
                                <vaadin-combo-box id="invoice-period" filtered-items="[[invoicePeriod]]" item-label-path="label.[[language]]" item-value-path="id" on-filter-changed="" label="[[localize('inv_per','Invoice period',language)]]" value="{{selectedInvoice.invoicePeriod}}" readonly="[[isInvoiceClosed]]"></vaadin-combo-box>
                            </template>
                        </div>
                        ...
                        <div class="flex-container">
                            <template is="dom-if" if="[[!_isSelectedInvoiceEattest(selectedInvoice.*)]]">
                                <div id="invoice-creditNote">
                                    ...
                                    <template is="dom-if" if="[[isInvoiceClosed]]">
                                        <vaadin-checkbox on-checked-changed="_isCreditNote" checked="[[selectedInvoice.creditNote]]">
                                            [[localize('inv_cr_no','Credit note',language)]]<br></vaadin-checkbox>
                                        <vaadin-checkbox on-checked-changed="_isLiftingLimitationPeriod" checked="[[_handleCheckboxCheckedStatusFromInteger(selectedInvoice.longDelayJustification)]]">
                                            [[localize('inv_llp','Lifting limitation period',language)]]
                                        </vaadin-checkbox>
                                    </template>
                                </div>
                            ...
                            <paper-tooltip for="invoice-dmg-icon">[[localize('gnotion','Gnotion',language)]]
                            </paper-tooltip>
                        </div>
                    </div>
                        ...
                        <div class="nmcl-descr">
                            ...
                            <div class="nmcl-list">
                                ...
                                <vaadin-grid id="nmclGrid" items="[[selectedInvoice.invoicingCodes]]" class="material" active-item="{{activeNmclItem}}" on-tap="_showDetail">
                                    ...
                                    <vaadin-grid-column width="80px">
                                        ...
                                        <template>
                                            <template is="dom-if" if="[[!item.contract]]">
                                                <div class="cell numeric unverifiedNmcl" title="[[localize('unverified_etarif','This nomenclature code has not (yet) been verified by eFact',language)]]">
                                                    [[_formatAmount(item.patientIntervention)]]&nbsp;€ <em class="unverifiedNmcl f-s-12em cursorPointer">*</em></div>
                                            </template>
                                            <template is="dom-if" if="[[item.contract]]">
                                                <div class="cell numeric verifiedNmcl" title="[[localize('verified_etarif','This nomenclature code has been verified by eFact',language)]]">
                                                    [[_formatAmount(item.patientIntervention)]]&nbsp;€ <em class="verifiedNmcl f-s-12em cursorPointer">*</em></div>
                                            </template>
                                        </template>
                                        <template class="footer">
                                            <div class="cell numeric">[[totalInvoice.patientIntervention]]&nbsp;€</div>
                                        </template>
                                    </vaadin-grid-column>
                                    <vaadin-grid-column width="80px">
                                        <template class="header">
                                        ...
                                        <template>
                                            <template is="dom-if" if="[[!item.contract]]">
                                                <div class="cell numeric unverifiedNmcl" title="[[localize('unverified_etarif','This nomenclature code has not (yet) been verified by eFact',language)]]">
                                                    [[_formatAmount(item.reimbursement)]]&nbsp;€ <em class="unverifiedNmcl f-s-12em cursorPointer">*</em></div>
                                            </template>
                                            <template is="dom-if" if="[[item.contract]]">
                                                <div class="cell numeric verifiedNmcl" title="[[localize('verified_etarif','This nomenclature code has been verified by eFact',language)]]">
                                                    [[_formatAmount(item.reimbursement)]]&nbsp;€<em class="verifiedNmcl f-s-12em cursorPointer">*</em></div>
                                            </template>
                                        </template>
                                        <template class="footer">
                                            <div class="cell numeric">[[totalInvoice.reimbursement]]&nbsp;€</div>
                                        </template>
                                    </vaadin-grid-column>
                                    <vaadin-grid-column width="80px">
                                        <template class="header">
                                        ...
                                        <template>
                                            <div class="cell numeric">[[_formatAmount(item.doctorSupplement)]]&nbsp;€
                                            </div>
                                        </template>
                                        <template class="footer">
                                            <div class="cell numeric">[[totalInvoice.doctorSupplement]]&nbsp;€</div>
                                        </template>
                                    </vaadin-grid-column>
                                    <vaadin-grid-column width="88px">
                                        ...
                                        <template class="footer">
                                            <div class="cell numeric">[[totalInvoice.totalAmount]]&nbsp;€</div>
                                        </template>
                                    </vaadin-grid-column>
                            ...
                            <template is="dom-if" if="[[!isInvoiceClosed]]">
                                ...
                                <div class="invoice-nomenclature-code-option">
                                    ...
                                    <paper-tooltip position="left" for="add-nmcl">[[localize('add_nmcl','Add
                                        nomenclature',language)]]
                                    </paper-tooltip>
                                    <paper-icon-button id="add-fav" icon="icons:star" on-tap="_addFavoriteCode"></paper-icon-button>
                                    <paper-tooltip position="left" for="add-fav">[[localize('add_to_fav','Add to
                                        favorites',language)]]
                                    </paper-tooltip>
                                </div>
                            </template>
                    ...
                    <template is="dom-if" if="[[isDetailNmcl]]">
                        ...
                        <div class="flex-container-nmcl">
                            ...
                            <template is="dom-if" if="[[isPrescriberIsNeeded]]">
                                ...
                                <template is="dom-if" if="[[_isSelectedInvoiceEattest(selectedInvoice.*)]]">
                                    <vaadin-date-picker id="prescription-date" label="[[localize('prescr_date','prescription date',language)]]" value="{{prescriptionDateAsString}}" i18n="[[i18n]]" readonly="[[isInvoiceClosed]]"></vaadin-date-picker>
                                </template>
                            </template>
                        </div>
                        <div class="flex-container-nmcl">
            ...
            <div class="buttons">
                ...
                <paper-button class="modal-button" on-tap="_closeDialog">[[localize('clo','close',language)]]
                </paper-button>

                <template is="dom-if" if="[[isEattestClosed]]">
                    <paper-button disabled="[[isLoadingEattest]]" class="modal-button modal-button--save invoiceBtnEnable" on-tap="_cancelDialogOpen">
                ...
                <template is="dom-if" if="[[!isInvoiceClosed]]">
                    <template is="dom-if" if="[[hasInsurability]]">
                        <paper-button class="modal-button modal-button--save tarificationBtnEnable" on-tap="_verifyTarification" disabled="[[_isETarButtonDisabled(selectedInvoice,isLoadingValidation,isTarificationMcnError,npi,selectedInvoice.sentMediumType)]]">
                            [[localize('eTarif','Validation eTarif',language)]]
                        </paper-button>
                    </template>
                    <template is="dom-if" if="[[!hasInsurability]]">
                        <paper-button class="modal-button modal-button--save modal-button-red" on-tap="" disabled="true">[[localize('patient_has_no_insurance','Ce patient n\\'est pas
                            assuré',language)]]
                        </paper-button>
                    </template>
                    <paper-button on-tap="_switchBetweenMethodForJustifAndCreation" class="modal-button modal-button--save invoiceBtnEnable" disabled="[[_isJustifButtonDisabled(selectedInvoice,isNmclVerified,isLoadingJustif,npi,selectedInvoice.sentMediumType)]]">
                        [[localize('val_invoice','Justificatory + create invoice',language)]]
                    </paper-button>
                </template>

                <template is="dom-if" if="[[isInvoiceClosed]]">
                    <paper-button on-tap="_switchBetweenMethodForJustif" class="modal-button invoiceBtnEnable" disabled="[[isLoadingJustif]]">
        ...
        <paper-dialog id="manualEncodingDialog">
            ...
            <div class="buttons">
                ...
                <paper-button on-tap="_confirmManualEncodingDialog">[[localize('add','Add',language)]]</paper-button>
            </div>
        </paper-dialog>
        ...
        <paper-dialog id="cancelDialog">
            ...
            <div class="buttons">
                <paper-button dialog-dismiss="">[[localize('can','Cancel',language)]]</paper-button>
                <paper-button dialog-confirm="" on-tap="_cancelEattest">[[localize('validate','Validate',language)]]
                </paper-button>
            </div>
        </paper-dialog>

        <paper-dialog id="paymentCheckDialog">
            <p>Veuillez introduire le type de paiement et le montant <b>encaissé</b><br>Le montant indiqué sera repris
                sur le justificatif et a valeur fiscale</p>
            ...
            <div class="buttons">
                <paper-button dialog-dismiss="">[[localize('can','Cancel',language)]]</paper-button>
                <paper-button dialog-confirm="" on-tap="_confirmPayment">[[localize('validate','Validate',language)]]
                </paper-button>
            </div>
        </paper-dialog>
`;
              ...
              .then(supervisor => {
                  ...
                  if (!invoiceOfDay) {
                      const newInvoice = {
                          ...
                          thirdPartyPaymentJustification: parseInt(this.patient.insurabilities.length) ? ((this.patient.insurabilities[0].parameters.tc1 % 2 === 1) ? "3" : "0") : "0",
                          invoiceType: parseInt(this.patient.insurabilities.length) ? ((this.patient.insurabilities[0].parameters.tc1 % 2 === 1) ? "mutualfund" : "patient") : 'patient',
                          sentMediumType: parseInt(this.patient.insurabilities.length) ? ((this.patient.insurabilities[0].parameters.tc1 % 2 === 1) ? "efact" : "eattest") : 'paper',
                          invoicePeriod: 0,
  ...
  _addNmcl(e) {
      ...
      if (this.shadowRoot.querySelector("#nmcl-search").selectedItem || e.target.dataset.item) {
          ...
          this.api.crypto().extractDelegationsSFKs(this.patient, this.user.healthcarePartyId).then(secretForeignKeys => {
              ...
              this.api.invoice().appendCodes((this.isDoctorAssistant(this.hcp.nihii) === true && this.supervisor && this.supervisor.id) ? this.supervisor.id : this.user.id, this.selectedInvoice.invoiceType, this.selectedMediumCodeType, (!!this.patient.insurabilities.length ? this.patient.insurabilities[0].insuranceId : ''), patientKeys, null, 0, [newNmcl])
                  .then(inv => (lastPatientAssurance && lastPatientAssurance.insuranceId ? this.api.insurance().getInsurance(lastPatientAssurance.insuranceId).then(ins => [inv[0] || null, ins || null]) : Promise.resolve([inv[0] || null, null])))
  ...
  _prevalidateAndPrepareInvoiceForBatchSending() {
      if (this.selectedInvoice) {
          this.set('selectedInvoice.paid', Number(this.selectedInvoice.invoicingCodes.reduce((tot, c) => tot + (Number(c.reimbursement) || 0) + (Number(c.patientIntervention) || 0) + (Number(c.doctorSupplement) || 0), 0).toFixed(2)))
  ...
  _validateAndPrepareInvoiceForBatchSending() {
      ...
      if (!this._isSelectedInvoiceEattest()) {
          ...
          if (!this.selectedInvoice.recipientId || this.selectedInvoice.recipientId === "") {
              const lastPatientAssurance = this.patient.insurabilities && this.patient.insurabilities.length && this.patient.insurabilities.find(ass => !ass.endDate && ass.insuranceId != "") || null
              lastPatientAssurance && lastPatientAssurance.insuranceId ? this.set("selectedInvoice.recipientId", lastPatientAssurance.insuranceId) : null
          }
  ...
  _transferNmcl() {
      if (this.activeNmclItem && this.activeNmclItem.id && this.bufferTypeCode && this.bufferTypeCode != this.selectedMediumCodeType) {
          this.api.crypto().extractDelegationsSFKs(this.patient, this.user.healthcarePartyId).then(secretForeignKeys => {
              const patientKeys = secretForeignKeys.extractedKeys.join(",")
              const lastPatientAssurance = this.patient.insurabilities && this.patient.insurabilities.length && this.patient.insurabilities.find(ass => !ass.endDate && ass.insuranceId != "") || null
              this.api.invoice().appendCodes(this.user.id, this.selectedInvoice.invoiceType, this.bufferTypeCode, (!!this.patient.insurabilities.length ? this.patient.insurabilities[0].insuranceId : ''), patientKeys, null, 0, [this.activeNmclItem])
                  .then(inv => (lastPatientAssurance && lastPatientAssurance.insuranceId ? this.api.insurance().getInsurance(lastPatientAssurance.insuranceId).then(ins => [inv[0] || null, ins || null]) : Promise.resolve([inv[0] || null, null])))
                  .then(([inv, ins]) => (ins && ins != null) ? this.api.insurance().getInsurance(ins.parent).then(parentIns => [inv, parentIns]) : Promise.resolve([inv, null]))
                  .then(([inv, parentIns]) => {
                      let invoice = inv
  ...
  _isJustifButtonDisabled() {
      return this.isLoadingJustif || !!((this._isSelectedInvoiceEattest() && (!this._invoiceDateIsToday() || !this.npi)) || (!this._isSelectedInvoiceEattest() && !((!this.isNmclVerified && (this.isTarificationError || this.isTarificationMcnError)) || this.isNmclVerified)));
  }
