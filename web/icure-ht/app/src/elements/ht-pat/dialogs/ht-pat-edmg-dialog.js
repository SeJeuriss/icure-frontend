  ...
  static get template() {
    return html`
        <style>
            ...
            #dialog .dmg-cons paper-button.action {
            ...
            #dialog .dmg-cons paper-button.action[disabled] {
                background-color: var(--app-secondary-color-dark);
                color: var(--app-text-color-disabled);
                box-shadow: none;
            }

            #dialog .dmg-cons .buttons {
                right: 24px;
                position: absolute;
            }
        ...
        <paper-dialog id="dialog" opened="{{opened}}">
            <h2 class="modal-title">[[localize('e_dmg','e-Dmg',language)]]</h2>
                ...
                <page>
                    <div class="dmg-cons">
                        <div>
                            ...
                            <paper-button class="action" on-tap="_consultDmg" disabled="[[isLoading]]">
                                [[localize('refresh','Refresh',language)]]
                            </paper-button>
                        </div>
                        ...
                        <div class="edmg-info">
                            <template is="dom-if" if="{{consultDmgResp}}">
                                ...
                                <template is="dom-if" if="{{!_hasErrors(curGenInsResp.errors)}}">
                                    <template is="dom-if" if="{{consultDmgResp.faultMessage}}"><p><b>[[localize('faultMessage','faultMessage',language)]]:</b>[[consultDmgResp.faultMessage]]
                                    </p></template>
                                    <template is="dom-if" if="{{consultDmgResp.faultSource}}"><p><b>[[localize('faultSource','faultSource',language)]]:</b>[[consultDmgResp.faultSource]]
                                    </p></template>
                                    <template is="dom-if" if="{{consultDmgResp.faultCode}}"><p><b>[[localize('faultCode','faultCode',language)]]:</b>[[consultDmgResp.faultCode]]
                                    </p></template>
                                </template>
                                <template is="dom-if" if="{{_hasNoDmg(consultDmgResp)}}">
                                    <div><p><b>[[localize('nam','Name',language)]]:</b> [[patient.firstName]]
                                        [[patient.lastName]]</p></div>
                                    <div><p><b>[[localize('no_dmg','No Dmg',language)]]</b></p></div>
                                </template>
                                <template is="dom-if" if="{{_hasDmg(consultDmgResp)}}">
                                    <div>
                                        <p><b>[[localize('nam','Name',language)]]:</b> [[consultDmgResp.firstName]]
                                            [[consultDmgResp.lastName]]</p>
                                        <p><b>[[localize('sex','Gender',language)]]:</b>
                                            [[localize(consultDmgResp.sex,consultDmgResp.sex,language)]]</p>
                                        <p><b>[[localize('dat_of_bir','Birthdate',language)]]:</b>
                                            [[_dateFormat(consultDmgResp.birthday,'DD/MM/YYYY')]]</p>
                                        <template is="dom-if" if="{{consultDmgResp.deceased}}"><p><b>[[localize('deceased','Deceased',language)]]:</b>[[_dateFormat(consultDmgResp.deceased,'DD/MM/YYYY')]]
                                        </p></template>
                                        <p><b>[[localize('ssin','SSIN',language)]]:</b> [[consultDmgResp.inss]]</p>
                                        <p><b>[[localize('mut','Mut',language)]]:</b> [[consultDmgResp.mutuality]]</p>
                                        <p><b>[[localize('regNrWithMut','Membership',language)]]:</b>
                                            [[consultDmgResp.regNrWithMut]]</p>
                                    </div>
                                    <div>
                                        <p><b>[[localize('dmg_holder','Dmg Holder',language)]]:</b></p>
                                        <template is="dom-if" if="{{consultDmgResp.hcParty.familyname}}"><p><b>[[localize('nam','Name',language)]]:</b>[[consultDmgResp.hcParty.firstname]]
                                            [[consultDmgResp.hcParty.familyname]]</p></template>
                                        <template is="dom-if" if="{{consultDmgResp.hcParty.name}}"><p><b>[[localize('nam','Name',language)]]:</b>[[consultDmgResp.hcParty.name]]
                                        </p></template>
                                        <p><b>[[_getHcpTypeDesc(consultDmgResp)]]:</b> [[_getHcpNihii(consultDmgResp)]]
                                        </p>
                                        <p><b>[[localize('from','From',language)]]:</b>
                                            [[_dateFormat(consultDmgResp.from, 'DD/MM/YYYY')]]
                                            <b>[[localize('to','To',language)]]:</b> [[_dateFormat(consultDmgResp.to,
                                            'DD/MM/YYYY')]]</p>
                                        <p><b>[[localize('pay_dmg','Payment',language)]]:</b>
                                            [[_yesOrNo(consultDmgResp.payment)]]</p>
                ...
                <page>
                    <div class="dmg-cons"> <!-- Notification input -->
                        ...
                        <template is="dom-if" if="[[_enableNotification(allEDmgregistered, asSupervisor)]]">
                            ...
                            <vaadin-checkbox checked="{{asSupervisor}}">[[localize('in_name_sup','In the name of the
                                supervisor',language)]]
                            </vaadin-checkbox>
                            <paper-button class="action" on-tap="_dmgNotify" disabled="[[isLoading]]">
                                [[localize('dmg_not','Notify',language)]]
                            </paper-button>
                            ...
                            <template is="dom-if" if="{{notifyDmgResp}}">
                                ...
                                <template is="dom-if" if="{{!_hasErrors(notifyDmgResp.errors)}}">
                                    <template is="dom-if" if="{{notifyDmgResp.hcParty.familyname}}"><p><b>[[localize('nam','Name',language)]]:</b>[[notifyDmgResp.hcParty.firstname]]
                                        [[notifyDmgResp.hcParty.familyname]]</p></template>
                                    <template is="dom-if" if="{{notifyDmgResp.hcParty.name}}"><p><b>[[localize('nam','Name',language)]]:</b>[[notifyDmgResp.hcParty.name]]
                                    </p></template>
                                    ...
                                    <p><b>[[localize('from','From',language)]]:</b> [[_dateFormat(notifyDmgResp.from,
                                        'DD/MM/YYYY')]]</p>
                                    <p><b>[[localize('pay_dmg','Payment',language)]]:</b>
                                        [[_yesOrNo(notifyDmgResp.payment)]]</p>
