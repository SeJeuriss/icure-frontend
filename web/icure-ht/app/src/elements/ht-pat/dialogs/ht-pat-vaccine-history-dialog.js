            ...
            <div class="content view">
                ...
                <template is="dom-if" if="[[!vaccines.length]]">
                    <h3 class="err">
                        ...
                        [[localize('no_hist','No history',language)]] !
                    </h3>
                </template>
...
`;
  ...
  static get observers() {
      return ['setList(contacts)', 'getAllVaccFromThesaurus(vaccines)']
  }
  ...
  setList() {
      ...
      this.api.contact().filterServices(this.contacts, s => s.label === 'Actes').then(services => {
          ...
          services.forEach(svc => {
              Object.keys(svc.tags).forEach(k => {
                  if (svc.tags[k].code == 'vaccine') {
                      let thesCode = '0'
                      Object.keys(svc.codes).forEach(c => {
                          if (svc.codes[c].type == "BE-THESAURUS-PROCEDURES") {
                              thesCode = svc.codes[c].code
                          }
                      })
                      const vacc = {
                          name: this._getName(svc),
                          intDate: svc.valueDate,
                          date: this._timeFormat(svc.valueDate),
                          status: this._getStatut(svc),
                          code: thesCode
                      }
          ...
          this.set("vaccines", _.orderBy(vaccines, 'intDate', 'desc'))
      })
  }

  _getName(svc) {
      const content = svc && this.api.contact().preferredContent(svc, this.language) || svc.content[this.language]
      return (content && content.medicationValue && content.medicationValue.medicinalProduct && content.medicationValue.medicinalProduct.intendedname) || this.localize('obs_val', 'obsolete value', this.language)
  }
      ...
      this.api.code().findPaginatedCodesByLabel('be', 'BE-THESAURUS-PROCEDURES', this.language || 'en', 'vaccin', null, null, 100).then(results => {
          ...
          results.rows.map(th => {
              this.vaccines.forEach(v => {
                  if (v.code == th.code && (tempVacc.indexOf(th) < 0)) tempVacc.push(th)
              })
          })
