  ...
  static get template() {
    return html`
        ...
        <li style\$="background-color: [[_ynToColour(verse.checkBoxInd, parentIncomplete)]]">
            <template is="dom-if" if="[[_isY(verse.checkBoxInd)]]">
                <vaadin-checkbox id="[[verse.verseNum]]" checked="[[_checked(verse.verseNum, checkedVerses.*)]]" on-checked-changed="_checkChanged"></vaadin-checkbox>
            </template>
            ...
            <template is="dom-if" if="[[_isParent(verse, checkedVerses.*)]]"><span style="background-color: #ffa">[Veuillez cocher au minimum [[verse.minCheckNum]] case(s) ci-dessous]</span>
            </template>
            <ul>
  ...
  _checkChanged(e) {
      if (e.detail) {
          this.dispatchEvent(new CustomEvent('check', {
              detail: {
                  uncheck: !e.detail.value,
                  verse: this.verse.verseNum
