...
class MedicationSelectionDialog extends TkLocalizerMixin(PolymerElement) {
  static get template() {
    return html`
        ...
        <paper-dialog id="medication-selection" always-on-top="true">
            ...
            <template is="dom-if" if="[[_isChronicalView(medicationType)]]">
                <vaadin-grid id="entities-list" active-item="{{selectedChronicalMedicationFromList}}" items="[[medications]]" on-tap="selectChronicalMedication">
                    ...
                    <vaadin-grid-column>
                        <template class="header">
                            <vaadin-grid-sorter path="[[column.key]]">[[localize('med','Medication',language)]]
                            </vaadin-grid-sorter>
                        </template>
                        <template>
            ...
            <div class\$="grid-container [[_noGridClass(medicationType)]]">
                <paper-input id="filter" label="[[localize('search_med','Search a medicine',language)]]" autofocus="" value="{{filterValue}}" on-keydown="refresh"></paper-input>
...
`;
      ...
      if (med.id) {
          ...
          this.api.bedrugs().getMppInfos(med.id.id, this.language === 'en' ? 'fr' : this.language || 'fr').then(mppInfos => {
              ...
              if (this.selectedMedicationContentWithId.isNew) {
                  this.dispatchEvent(new CustomEvent('new-medication', {
                      detail: {
                  ...
                  this.dispatchEvent(new CustomEvent('display-medication-details', {
                      detail: {
                          medication: this.newMedication,
                          isPrescription: this.selectedMedicationContentWithId.isPrescription
                      }, bubbles: true, composed: true
          ...
          if (med.inncluster) {
              ...
              this.set('selectedMedicationContentWithId.medicationValue.regimen', [{
                  administratedQuantity: {
                      time: '',
                      quantity: 1,
                      unit: this.localize('generic_unit', 'unit', this.language)
                  }
              }])
          } else {
              //compound
  ...
  selectChronicalMedication(e) {
      if (this.selectedChronicalMedicationFromList) {
          ...
          this.dispatchEvent(new CustomEvent('new-medication', {
              detail: {
                  medication: _.cloneDeep(_.omit(selected, ['id'])),
                  isPrescription: this.selectedMedicationContentWithId.isPrescription
              }, bubbles: true, composed: true
          }))
      }
