  ...
  static get template() {
    return html`
        ...
        <paper-dialog id="medication-selection" always-on-top="true">
                ...
                <div class="left-pane">
                        ...
                        <vaadin-grid id="entities-list" active-item="{{selectedChronicalMedicationFromList}}" items="[[medications]]" on-tap="clickChron">
                            <vaadin-grid-column>
                                <template class="header">
                                    <vaadin-grid-sorter path="[[column.key]]">[[localize('atc-categ','Atc
                                        category',language)]]
                                    </vaadin-grid-sorter>
                                </template>
                                <template>
                                    <div>
                                        <paper-icon-button id="[[item.id]]" class="add-btn" icon="icons:add" on-tap="_addChron"></paper-icon-button>&nbsp;[[atcCat(item.atcCat)]]
                                    </div>
                                </template>
                            </vaadin-grid-column>
                            <vaadin-grid-column>
                                <template class="header">
                                    <vaadin-grid-sorter path="[[column.key]]">
                                        [[localize('med','Medication',language)]]
                                    </vaadin-grid-sorter>
                                </template>
                                <template>
                            ...
                            <vaadin-grid-column>
                                <template class="header">
                                    <vaadin-grid-sorter path="[[column.key]]">[[localize('pos','Posology',language)]]
                                    </vaadin-grid-sorter>
                                </template>
                                <template>
                    ...
                    <template is="dom-if" if="[[_isCompoundPrescription(medicationType)]]">
                        <div style="margin-top: 12px; position: relative;">
                            ...
                            <paper-icon-button id="[[_id(item)]]" class="add-btn add-btn--compound" icon="icons:add" on-tap="_addCompound"></paper-icon-button>
                            <paper-input-container>
                                <label slot="label">[[localize('medication-compound','Compound
                                    Prescription',language)]]</label>
                    ...
                    <vaadin-grid id="accumulated-medications" items="[[medicationAccumulator]]" active-item="{{selectedMedicationFromAccumulator}}">
                        <vaadin-grid-column width="72px">
                            ...
                            <template>
                                ...
                                <paper-icon-button id="remove-[[_id(item)]]" class="add-btn" icon="icons:remove" on-tap="_removeFromAcc"></paper-icon-button>
                            </template>
                        </vaadin-grid-column>
            ...
            <div class="buttons">
                <template is="dom-if" if="[[_isMedicinalProduct(medicationType)]]">
                    <vaadin-checkbox checked="{{searchOnIngredients}}" style="position:absolute; left:20px; color:black;">
                        [[localize('also_search_on_ingredients','Also search on ingredients ?',language)]]
                    </vaadin-checkbox>
                </template>
                <paper-button class="modal-button modal-button--cancel" dialog-dismiss="" on-tap="skip">
                    [[localize('can','Cancel',language)]]
                </paper-button>
                <paper-button class="modal-button modal-button--save" dialog-confirm="" autofocus="" on-tap="validateMedications" disabled="[[!medicationAccumulator.length]]">
                    [[localize('valid','Validate',language)]]
                </paper-button>
            </div>
        </paper-dialog>
        <paper-dialog id="checkintol" always-on-top="true">
            ...
            <div class="warn">
                ...
                [[localize('patient_has_adr','Le patient possède une contre-indication pour la médication',language)]]
            ...
            <div class="list">
                <template is="dom-repeat" items="[[intolItems]]" as="intol">
                    <div>
                        <iron-icon icon="[[_getIntolIcon(intol)]]"></iron-icon>
                        [[getIcpcLabel(intol)]] <i>[[_heShortDateFormat(intol.openingDate, intol.valueDate)]]</i>:
                        [[intol.descr]]
                    </div>
                </template>
            </div>
            <div class="endline">
                ...
                <paper-button class="modal-button modal-button--force" dialog-confirm="" autofocus="" on-tap="continueAddMedication">[[localize('yes','Yes',language)]]
                </paper-button>
            </div>
        </paper-dialog>
`;
