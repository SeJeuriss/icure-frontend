...
class DynamicForm extends TkLocalizerMixin(PolymerElement) {
  static get template() {
    return html`
        <style include="icpc-styles dialog-style">
            ht-spinner.center {
                position: absolute;
                left: calc(50% - 14px);
                top: calc(50% - 14px);
            ...
            #delete-confirmation-dialog .modal-button--save {
        ...
        <paper-card class\$="{{_patCardClass(isSubForm)}}">
                ...
                <div class="form-title">
                    ...
                    <div>
                        <template is="dom-if" if="[[!readOnly]]">
                            ...
                            <template is="dom-if" if="[[_none(showEdit,noPrint)]]">
                                ...
                                <template is="dom-if" if="[[!isSubForm]]">
                                    <paper-menu-button class="form-title-bar-btn" horizontal-align="right" dynamic-align="true" vertical-offset="26">
                                        <paper-icon-button class="form-title-bar-btn" slot="dropdown-trigger" alt="menu" icon="print"></paper-icon-button>
                                        <paper-listbox slot="dropdown-content">
                                            <template is="dom-repeat" items="[[reports]]" as="report">
                                                <paper-item class="link" on-tap="newReport" id="report">
                                                    <iron-icon icon="vaadin:file-o"></iron-icon>
                                                    [[report.name]]
                                                </paper-item>
                                            </template>
                                        </paper-listbox>
                                ...
                                <template is="dom-if" if="[[_shouldDisplayPrintSubFormIcon(isSubForm, template)]]">
                                    <paper-icon-button icon="print" on-tap="printSubForm" id="printSubForm" class="form-title-bar-btn"></paper-icon-button>
                                    <paper-tooltip position="bottom" for="printSubForm">
                            ...
                            <paper-icon-button id="delform" class="form-title-bar-btn" icon="icons:delete" on-tap="_deleteConfirmation"></paper-icon-button>
                            <paper-tooltip position="bottom" for="delform">[[localize('del_form','Delete
                                form',language)]]
                            </paper-tooltip>
                        </template>
                    </div>
                            ...
                            <template id="layoutitems-repeat" is="dom-repeat" items="[[_sortedGroupedFormDataList(column.formDataList)]]" as="layoutItem">
                                <template is="dom-if" if="[[_shouldDisplay(layoutItem, readOnly, compact)]]">
                                    <template is="dom-if" if="[[_isTextField(layoutItem)]]">
                                        <dynamic-text-field id="[[_sanitizeId(layoutItem.name)]]" label="[[layoutItem.label]]" key="[[layoutItem.name]]" was-modified="[[_wasModified(layoutItem)]]" is-modified-after="[[_isModifiedAfter(layoutItem)]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" linkables="[[linkableHealthElements]]" last-modified="[[_lastModified(layoutItem)]]" pattern="[[layoutItem.editor.pattern]]" value="[[_value(layoutItem,dataMap.*)]]" api="[[api]]" width="[[layoutItem.editor.flow]]" on-field-changed="_valueChanged" read-only="[[readOnly]]" disabled="[[disabled]]"></dynamic-text-field>
                                    </template>
                                    <template is="dom-if" if="[[_isTextArea(layoutItem)]]">
                                        <dynamic-text-area id="[[_sanitizeId(layoutItem.name)]]" label="[[layoutItem.label]]" key="[[layoutItem.name]]" was-modified="[[_wasModified(layoutItem)]]" is-modified-after="[[_isModifiedAfter(layoutItem)]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" linkables="[[linkableHealthElements]]" last-modified="[[_lastModified(layoutItem)]]" value="[[_value(layoutItem,dataMap.*)]]" api="[[api]]" width="[[layoutItem.editor.flow]]" on-field-changed="_valueChanged" read-only="[[readOnly]]" disabled="[[disabled]]"></dynamic-text-area>
                                    </template>
                                    <template is="dom-if" if="[[_isCheckboxField(layoutItem)]]">
                                        <dynamic-checkbox id="[[_sanitizeId(layoutItem.name)]]" label="[[layoutItem.label]]" key="[[layoutItem.name]]" was-modified="[[_wasModified(layoutItem)]]" is-modified-after="[[_isModifiedAfter(layoutItem)]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" linkables="[[linkableHealthElements]]" last-modified="[[_lastModified(layoutItem)]]" value="[[_value(layoutItem,dataMap.*)]]" api="[[api]]" width="[[layoutItem.editor.flow]]" on-field-changed="_valueChanged" read-only="[[readOnly]]" disabled="[[disabled]]"></dynamic-checkbox>
                                    </template>
                                    <template is="dom-if" if="[[_isNumberField(layoutItem)]]">
                                        <dynamic-number-field id="[[_sanitizeId(layoutItem.name)]]" api="[[api]]" label="[[layoutItem.label]]" key="[[layoutItem.name]]" was-modified="[[_wasModified(layoutItem)]]" is-modified-after="[[_isModifiedAfter(layoutItem)]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" linkables="[[linkableHealthElements]]" last-modified="[[_lastModified(layoutItem)]]" value="[[_value(layoutItem,dataMap.*)]]" width="[[layoutItem.editor.flow]]" on-field-changed="_valueChanged" read-only="[[readOnly]]" disabled="[[disabled]]"></dynamic-number-field>
                                    </template>
                                    <template is="dom-if" if="[[_isMeasureField(layoutItem)]]">
                                        <dynamic-measure-field id="[[_sanitizeId(layoutItem.name)]]" api="[[api]]" label="[[layoutItem.label]]" key="[[layoutItem.name]]" was-modified="[[_wasModified(layoutItem)]]" is-modified-after="[[_isModifiedAfter(layoutItem)]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" linkables="[[linkableHealthElements]]" last-modified="[[_lastModified(layoutItem)]]" value-with-unit="[[_value(layoutItem,dataMap.*)]]" width="[[layoutItem.editor.flow]]" on-field-changed="_valueChanged" read-only="[[readOnly]]" disabled="[[disabled]]"></dynamic-measure-field>
                                    </template>
                                    <template is="dom-if" if="[[_isPopupMenu(layoutItem)]]">
                                        <dynamic-popup-menu id="[[_sanitizeId(layoutItem.name)]]" api="[[api]]" label="[[layoutItem.label]]" key="[[layoutItem.name]]" was-modified="[[_wasModified(layoutItem)]]" is-modified-after="[[_isModifiedAfter(layoutItem)]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" data-source="[[_popupDataSource(layoutItem, layoutItem.editor.menuOptions)]]" linkables="[[linkableHealthElements]]" last-modified="[[_lastModified(layoutItem)]]" value="[[_value(layoutItem,dataMap.*)]]" width="[[layoutItem.editor.flow]]" options="[[layoutItem.editor.menuOptions]]" on-field-changed="_valueChanged" read-only="[[readOnly]]" display-all-always="[[layoutItem.editor.displayAllAlways]]"></dynamic-popup-menu>
                                    </template>
                                    <template is="dom-if" if="[[_isDateField(layoutItem)]]">
                                        <dynamic-date-field id="[[_sanitizeId(layoutItem.name)]]" api="[[api]]" label="[[layoutItem.label]]" key="[[layoutItem.name]]" was-modified="[[_wasModified(layoutItem)]]" is-modified-after="[[_isModifiedAfter(layoutItem)]]" linkables="[[linkableHealthElements]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" last-modified="[[_lastModified(layoutItem)]]" value="[[_value(layoutItem,dataMap.*)]]" width="[[layoutItem.editor.flow]]" on-field-changed="_valueChanged" read-only="[[readOnly]]" disabled="[[disabled]]"></dynamic-date-field>
                                    </template>
                                    <template is="dom-if" if="[[_isValueDateField(layoutItem)]]">
                                        <dynamic-date-field id="[[_sanitizeId(layoutItem.name)]]" api="[[api]]" label="[[layoutItem.label]]" key="[[layoutItem.name]]" was-modified="[[_wasModified(layoutItem)]]" is-modified-after="[[_isModifiedAfter(layoutItem)]]" linkables="[[linkableHealthElements]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" last-modified="[[_lastModified(layoutItem)]]" value="[[_valueDate(layoutItem,dataMap.*)]]" width="[[layoutItem.editor.flow]]" on-field-changed="_valueDateChangedWithBooleanSet" read-only="[[readOnly]]" disabled="[[disabled]]"></dynamic-date-field>
                                    </template>
                                    <template is="dom-if" if="[[_isTokenField(layoutItem)]]">
                                        <dynamic-token-field id="[[_sanitizeId(layoutItem.name)]]" label="[[layoutItem.label]]" key="[[layoutItem.name]]" api="[[api]]" was-modified="[[_wasModified(layoutItem)]]" is-modified-after="[[_isModifiedAfter(layoutItem)]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" data-source="[[_tokenDataSource(layoutItem, layoutItem.codeTypes)]]" last-modified="[[_lastModified(layoutItem)]]" linkables="[[linkableHealthElements]]" value="[[_valueContainers(layoutItem,dataMap.*)]]" width="[[layoutItem.editor.flow]]" on-field-changed="_valueContainersChanged" read-only="[[readOnly]]" disabled="[[disabled]]"></dynamic-token-field>
                                    </template>
                                    <template is="dom-if" if="[[_isMedicationField(layoutItem)]]">
                                        <dynamic-medication-field id="[[_sanitizeId(layoutItem.name)]]" label="[[layoutItem.label]]" key="[[layoutItem.name]]" api="[[api]]" was-modified="[[_wasModified(layoutItem)]]" is-modified-after="[[_isModifiedAfter(layoutItem)]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" linkables="[[linkableHealthElements]]" last-modified="[[_lastModified(layoutItem)]]" value="[[_valueContainers(layoutItem,dataMap.*)]]" width="[[layoutItem.editor.flow]]" on-field-changed="_valueContainersChanged" read-only="[[readOnly]]" disabled="[[disabled]]" create-treatment="[[_hasTreatmentCdItem(layoutItem.tags)]]"></dynamic-medication-field>
                                    </template>
                                    <template is="dom-if" if="[[_isSubForm(layoutItem)]]">
                                        <dynamic-sub-form id="sf_[[_sanitizeId(layoutItem.name)]]" label="[[layoutItem.label]]" key="[[layoutItem.name]]" layout-item="[[layoutItem]]" api="[[api]]" user="[[user]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" sub-contexts="[[_subForms(layoutItem,dataMap.*)]]" on-delete-subform="_deleteSubForm" on-add-subform="_addSubForm" read-only="[[readOnly]]" disabled="[[disabled]]" no-print="[[noPrint]]"></dynamic-sub-form>
                                    </template>
            ...
            <paper-dialog id="delete-confirmation-dialog">
                ...
                <div class="buttons">
                    <paper-button dialog-confirm="" autofocus="" on-tap="cancel" class="modal-button">Cancel</paper-button>
                    <paper-button dialog-confirm="" class="modal-button modal-button--save" on-tap="_deleteForm">Delete
                    </paper-button>
                </div>
            </paper-dialog>
...
`;
  ...
  notify(path) {
      try {
          ...
          const item = this.root.querySelector('#sf_' + joinedId);
  ...
  _linkForm(e) {
      ...
      this.dispatchEvent(new CustomEvent('link-form', {
  ...
  _tokenDataSource(d) {
      return d && {
          filter: (text, uuid) => this.dataProvider && this.dataProvider.filter && this.dataProvider.filter(d.editor.dataSource || d.codeTypes && {
              source: "codes",
              types: d.codeTypes
          }, text, uuid) || Promise.resolve([])
      } || null;
  }
  ...
  _popupDataSource(d, options) {
      ...
      return d && (d.codeTypes && d.codeTypes.length || d.editor.dataSource) ? {
          filter: text => this.dataProvider && this.dataProvider.filter && this.dataProvider.filter(ds, text, uuid, null) || Promise.resolve([]),
          get: id => this.dataProvider && this.dataProvider.filter && this.dataProvider.filter(ds, null, uuid, id) || Promise.resolve(null)
      } : null;
  }
  ...
  newReport() {
      this.dispatchEvent(new CustomEvent('new-report', {
          detail: {dataProvider: this.dataProvider},
          composed: true,
          bubbles: true
      }));
  }
  ...
  printSubForm() {
      this.dispatchEvent(new CustomEvent('print-subform', {
          detail: {dataProvider: this.dataProvider},
          composed: true,
          bubbles: true
      }));
  ...
  _linkToSubcontactType(e) {
      if (e.detail) {
          this.dispatchEvent(new CustomEvent('subcontact-type-change', {
              detail: {
                  type: e.detail,
                  formId: this.dataProvider.getId()
              }
          }));
      }
  }
  ...
  _shouldDisplayPrintSubFormIcon(isSubForm, template) {
      return template && ["FFFFFFFF-FFFF-FFFF-FFFF-INCAPACITY00", "AEFED10A-9A72-4B40-981B-1D79ADB05516", "64DAB551-B007-4B5C-BD64-F886301F5326", "B4F2B274-10FF-4018-BC48-3ED3CADCED57"].includes(template.guid) &&
