  ...
  static get template() {
    return html`
        ...
        <template is="dom-if" if="[[_shouldDisplaySummary(form, form.deletionDate, contact)]]">
            <div class="fancy"><span>[[localize('modification_of','Modification of',language)]] [[form.template.name]] [[localize('of','of',language)]] [[_dateFormat(form.created)]]</span>
            </div>
        </template>
`;
  }
  ...
  linkForm(e) {
      ...
      if (!he.id) {
          this.promoteServiceToHealthElement(he).then(he => {
              _.assign(e.detail, he) //Prevent subsequent promotions
              if (!ctc.subContacts.some(sc => sc.formId === this.formId && sc.healthElementId === he.id)) {
                  if (this.contact === ctc) {
                      this.push('contact.subContacts', {
                          formId: this.formId,
  ...
  _openActionDialog(e, detail) {
      this.dispatchEvent(new CustomEvent('plan-action', {
          detail: this.dataProvider && this.dataProvider.getStringValue(detail.representedObject),
          bubbles: true,
          composed: true
  ...
  _loadReports() {
      ...
      Promise.all(reports.map(r => this.api.doctemplate().getDocumentTemplate(r)))
          .then(tpls => {
              this.formReports = [{name: 'Report'}].concat(tpls);
          })
  }
  ...
  scanServicesAndScheduleSave(ctc) {
      ...
      this.dataProvider.computeFormulas(['OnChange']).finally(() => {
          ...
          ctc.services.filter(s => s.tags.some(t => t.type === 'CD-ITEM' && (t.code === 'healthelement' || t.code === 'healthissue')))
              .forEach(svc => {
                  ...
                  if (descr && descr.length && !idLinkables.includes(svc.id)) {
                      ...
                      this.api.code().icpcChapters(he.codes.filter(c => c.type === 'ICPC' || c.type === 'ICPC2').map(x => x.code))
                          .then(([code]) => {
          ...
          const saveAction = this.saveAction = () => {
              ...
              return new Promise((resolve, reject) =>
                  this.dispatchEvent(new CustomEvent('must-save-contact', {
                      detail: {
                          contact: ctc,
                          preSave: () => this.dataProvider.computeFormulas(['OnSave']),
                          postSave: (ctc) => resolve(ctc),
                          postError: (e) => reject(e)
  ...
  getDataProvider(form, rootPath) {
      ...
      const self = {
          ...
          servicesMap: {},
          sortedItems: () => {
              const layoutItems = form.template && form.template.layout && _.flatten(_.flatten(this.form.template.layout.sections.map(s => s.formColumns)).map(c => c.formDataList))
              return _.sortBy(_.compact(
                  layoutItems && layoutItems.map(s => ({
                      name: s.name,
                      label: s.subForm ? s.label : s.name,
                      isSubForm: s.subForm,
                      type: s.type
                  })) ||
              ...
              const isModified = containers.map(container => {
                  ...
                  if (svc) {
                      ...
                      if (!prevSvc || !_.isEqual(prevSvc.content, container.content)) {
                          self.dispatchService(_.extend(this.createService(label, form.id, null, (this.highlightedHes || []).map(he => he.id), container.id, initWrapper(label)),
                              {
          ...
          setDateValue: function (label, value, save = true) {
              ...
              self.promoteOrCreateService(label, form.id, null, (this.highlightedHes || []).map(he => he.id), svc => {
                  ...
                  if (c && c.fuzzyDateValue !== this.api.moment(value).format("YYYYMMDD")) {
          ...
          filter: (data, text, deduplicationUuid, id) => {
              if (data.source !== "codes") {
                  return id ? Promise.resolve({}) : Promise.resolve([])
              }
              ...
              return !(data.types.length && words && words.length) ? Promise.resolve([]) : Promise.all(data.types.map(ct => {
                  ...
                  return this.api.code().filterBy(null, null, 1000, null, null, null, {
                      filter: {
                          '$type': 'IntersectionFilter', 'filters': words.map(w => ({
                      ...
                      .then(results => {
                          if (reqIdx < uuids[deduplicationUuid]) {
                              return []
                          }
  ...
  _formIdChanged(formId) {
      ...
      const loadForms = function (templates, forms, root) {
          ...
          return Promise.all([
              ...
              forms.forEach(f => f.template = f.formTemplateId ? templates[f.formTemplateId] : {
                  layout: null,
                  name: "Dynamic"
              })
  ...
  createService(label, formId, poaId, heId, serviceId, init) {
      ...
      const svc = this.api.contact().service().newInstance(this.user, serviceId ? {
              id: serviceId,
              label: label
          } : {label: label})
  ...
  promoteServiceToHealthElement(heSvc) {
      return this.api.helement().serviceToHealthElement(this.user, this.patient, heSvc.svc,
          ...
          this.scanServicesAndScheduleSave(this.currentContact);
          this.dispatchEvent(new CustomEvent('health-elements-change', {
              detail: {hes: [he]},
              bubbles: true,
              composed: true
          }));
