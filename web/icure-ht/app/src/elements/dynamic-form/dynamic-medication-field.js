        ...
        <div class="subform-container">
            ...
            <div class="buttons">
                <template is="dom-if" if="[[!isReadOnlyOrMedication()]]">
                    <paper-button class="add-medication" on-tap="addMedication">
                        <iron-icon icon="add"></iron-icon>
                        [[localize('add_pre_dru','Add prescription drug',language)]]
                    </paper-button>
                </template>
                <template is="dom-if" if="[[createTreatment]]">
                    <paper-icon-button class="printSubForm" icon="print" on-tap="openPrescriptionDialog">
                        [[localize('pri_pre','Print',language)]]
                    </paper-icon-button>
                </template>
            </div>
        </div>
`;
  ...
  _medicationCreated(e) {
      ...
      this.dispatchEvent(new CustomEvent('medications-detail', {
          detail: {
              services: e.detail.medications,
  ...
  _displayMedicationDetails(e) {
      this.set('previouslySavedValue', _.cloneDeep(this.value))
      this.dispatchEvent(new CustomEvent('medication-detail', {
          detail: {
              service: e.detail.medication,
              onValueChanged: this._valueChanged.bind(this)
          }, bubbles: true, composed: true
      }))
  }
  ...
  localizedMedicationValueWithId(id, e, lng) {
      ...
      return {
          id: id,
          stringValue: this.api.contact().medication().medicationToString((this.api.contact().preferredContent({content: e}, this.language) || {}).medicationValue || "", this.language.toLowerCase() || 'fr')
      };
  }
  ...
  addMedication() {
      this.set('previouslySavedValue', _.cloneDeep(this.value))
      this.dispatchEvent(new CustomEvent('medications-selection', {
          detail: {
              onCreate: this._medicationCreated.bind(this),
              onShowDetails: this._displayMedicationDetails.bind(this),
              onValueChanged: this._valueChanged.bind(this)
          }, bubbles: true, composed: true
      }))
  }
  ...
  editMedication(el) {
      ...
      if (item) {
          this.dispatchEvent(new CustomEvent('medication-detail', {
              detail: {
                  service: item,
                  content: this.extractContentWithIdFromMedicationService(item, false, this.createTreatment),
                  onValueChanged: this._valueChanged.bind(this)
              }, bubbles: true, composed: true
          }))
      }
