        ...
        <template is="dom-if" if="[[_display(linkables, initialised)]]">
            <template is="dom-if" if="[[!readOnly]]">
                <paper-menu-button id="mb" horizontal-align="left" dynamic-align="true" opened="{{opened}}" allow-outside-scroll="">
                    ...
                    <paper-tooltip for="subctc-type">[[localize('contact_type','Contact type',language)]]
                    </paper-tooltip>
                    <paper-listbox slot="dropdown-content">
                        <template is="dom-repeat" items="[[_linkables(linkables, openedOnce)]]" as="he">
                            <paper-item id="[[getHeId(he)]]" class="link" on-tap="link"><label class\$="colour-code [[he.colour]]"><span></span></label>[[he.descr]]
                            </paper-item>
                        </template>
                    </paper-listbox>
                </paper-menu-button>
...
`;
  ...
  link(e) {
      ...
      this.dispatchEvent(new CustomEvent('link-to-health-element', {
          bubbles: true,
          composed: true,
          detail: {representedObject: this.representedObject, healthElement: he}
      }));
  }

  showPlanActionForm(e) {
      this.dispatchEvent(new CustomEvent('dynamically-event', {
          bubbles: true,
          composed: true,
          detail: {representedObject: this.representedObject}
      }));
  }
  ...
  _setStatus(e) {
      this.dispatchEvent(new CustomEvent('status-changed', {
          bubbles: true,
          composed: true,
          detail: {representedObject: this.representedObject, status: e.target.id}
      }));
  }
