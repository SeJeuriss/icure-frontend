...
`;
  ...
  hcpChanged() {
      if (this.api && this.user && this.hcp) {
          ...
          this.api.getRegistry('hcp').listeners['ht-hcp-admin'] = {
              target: this,
              pool: [this.hcp.id],
              callbacks: [() => this.hcpChanged()]
          }
          // this.flushSave();
  ...
  scheduleSave() {
      ...
      this.saveAction = () => {
          this.api.hcparty().modifyHealthcareParty(this.hcp)
              ...
              .finally(() => this.dispatchEvent(new CustomEvent("hcp-saved", {
                  detail: {hcp: this.hcp},
                  params: null
              })));
      };
  ...
  hcpDataProvider(root, rootPath, id, codes) {
      ...
      return {
          ...
          getBooleanValue: function (key) {
              root ? _.get(root, key) && _.get(root, key) !== 'false' : null
          }.bind(this),
          ...
          setValueContainers: function (key, value) {
              setValue(key, value.map(s => (s.codes && s.codes[0] || {}).code || (this.api.contact().preferredContent(s, this.language) || {}).stringValue))
          }.bind(this),
              ...
              if (data.source === 'insurances') {
                  ...
                  return (id || '').length >= 1 ?
                      this.api.hcparty().getHealthcareParty(id).then(results => {
                          return {
                              'id': results.id,
                              'name': _.upperFirst(_.lowerCase(results.firstName)) + " " + _.upperFirst(_.lowerCase(results.lastName)) + ' ' + (typeof results.nihii === 'undefined' || !results.nihii ? '' : ' - ' + this.localize('nihii', 'INAMI', language) + ': ' + results.nihii)
                          }
                      }) :
                      (text || '').length >= 2 ? Promise.all([this.api.hcparty().findBySsinOrNihii(text), this.api.hcparty().findByName(text)]).then(results => {
                          return _.flatten(_.chain(_.concat(results[0].rows, results[1].rows)).uniqBy('nihii').filter(i => {
                              return _.trim(_.get(i, "type", "")) !== "medicalhouse" && _.trim(_.get(i, "type", "")) !== _.trim(this.hcp || "") && !_.trim(_.get(i, "supervisorId", ""))
                          }).value().map(i => ({
                              'id': i.id,
