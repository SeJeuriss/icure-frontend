...
class HtHcpPatList extends TkLocalizerMixin(PolymerElement) {
  static get template() {
    return html`
...
`;
  ...
  ready() {
      ...
      const promesse = this.hcp && this.hcp.type && this.hcp.type.toLowerCase() === "medicalhouse" ?
          this.api.hcparty().listHealthcareParties(null, null, 1000, "asc") :
          Promise.resolve({rows: [this.hcp]})

      promesse.then(hcps => hcps && hcps.rows && Promise.all(hcps.rows.map(hcp => this.api.patient().listPatientsByHcPartyWithUser(this.user, hcp.id, "name", null, null, 500, "asc"))))
          .then(patientsArrays => patientsArrays.map(patients => this.set("patientsList", _.uniqBy(this.patientsList.concat(patients && patients.rows), "id"))))
  }
