    ...
    return html`
        ...
        <paper-dialog id="firstConnectionDialog" opened="{{opened}}" api="[[api]]" route="[[route]]" user="[[user]]" modal="">
            ...
            <template is="dom-if" if="[[!user.passwordHash]]">
                <h4>[[localize('generate_key_mess','Entrer a password for the application and click on the following
                    button to generate your key',language)]]</h4>
            ...
            <template is="dom-if" if="[[user.passwordHash]]">
                <h4>[[localize('generate_key_mess_bypass','Generate your key and keep it somewhere
                    safe',language)]]</h4>
            </template>
            <paper-button class="generateKeyBtn" on-tap="generateKey">[[localize('gen','Generate',language)]]
            </paper-button>
