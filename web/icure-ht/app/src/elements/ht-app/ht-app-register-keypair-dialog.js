        ...
        <paper-dialog id="registerKeyPairDialog" opened="{{opened}}" modal="">
            <form is="form" id="keys-form">
                <div class="layout vertical center">
                    <h3>[[localize('a_pri_key_fil_is_nee_for_thi_use','A private key file is needed for this
                        user',language)]]</h3>
                    <h5>ID: [[keyhcpid]]</h5>
                    <paper-input type="file" id="rsaFileInput" on-change="rsaKeySelected"></paper-input>
                    <div class="submit-container">
                        <paper-button raised="true" id="submit-button" type="submit" on-tap="selectFile" autofocus="">
                            [[localize('imp','Import',language)]]
                        </paper-button>
                    </div>
                    <div>[[message]]</div>
                </div>
...
`;
  ...
  rsaKeySelected() {
      ...
      if (this.selectedRsaFile != null) this.selectFile()
  }

  selectFile() {
      console.log('File selected');
      this.dispatchEvent(new CustomEvent('file-selected', {
          detail: this.selectedRsaFile,
          bubbles: true,
          composed: true
      }))
  }
  ...
  _qrCodeDetected(e, code) {
      ...
      const jwk = combinedCode && (combinedCode.startsWith('{') && JSON.parse(combinedCode) || {
          alg: 'RSA-OAEP',
          key_ops: ['decrypt'],
          kty: 'RSA',
          ext: true
