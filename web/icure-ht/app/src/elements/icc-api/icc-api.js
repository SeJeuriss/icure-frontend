...
`;
  ...
  shortLivedCache(key, loader) {
      ...
      const clean = () => {
          const now = +new Date();
          Object.keys(cache).filter(k => cache[k].ttl < now).forEach(k => delete cache[k]);
          console.log(`slc: ${cache.length} items`)
      }
  ...
  getRowsUsingPagination(paginator, filter, startIdx, endIdx, cache) {
      const executePaginator = (latestResult, acc, limit) =>
          paginator(latestResult.nextKey, latestResult.nextDocId, endIdx ? endIdx - startIdx : undefined).then((newResult) => {
              ...
              acc.push(...rows)
              return newResult.done || (limit && (acc.length >= limit)) ? ({
                  rows: acc,
                  nextKey: newResult.nextKey,
                  nextDocId: newResult.nextDocId
              }) : executePaginator(newResult, acc, limit)
          })
      ...
      if (cache) {
              ...
              if (chunk.endIdx <= lastTreatedIdx) {
                  ...
                  if (chunk.startIdx <= lastTreatedIdx) {
                      if (chunk.endIdx <= endIdx) {
                          ...
                          // [-chunk-]
                          rows.push({
                              startIdx: lastTreatedIdx,
                              rows: chunk.rows.slice(lastTreatedIdx - chunk.startIdx, chunk.endIdx - chunk.startIdx)
                          })
                          ...
                          rows.push({
                              startIdx: lastTreatedIdx,
                              rows: chunk.rows.slice(lastTreatedIdx - chunk.startIdx, endIdx - chunk.startIdx)
                          })
                          lastTreatedIdx = endIdx
                      }
                  } else {
                      if (chunk.endIdx >= endIdx) {
                          ...
                          rows.push({
                              ...
                              rows: chunk.rows.slice(0, endIdx - chunk.startIdx)
                          })
                          lastTreatedIdx = endIdx
                      } else {
                          //  [-------zoi-------]
          ...
          return Promise.all(rows.filter(r => r.missing).map(r => executePaginator({
              nextKey: r.lastKey,
              nextDocId: r.lastDocId
          }, [], r.missing[1] - r.lastEndIdx).then(({rows, nextKey, nextDocId}) => {
  ...
  queue(entity, domain) {
      if (!entity || !entity.id) {
          return Promise.resolve([entity, undefined])
      }
      ...
      const timeout = thisPromise ? setTimeout(() => {
          thisPromise.resolve(entity)
      }, 10000) : null
      reg.queues[entity.id] = defer

      return (thisPromise || Promise.resolve(entity)).then(e => {
  ...
  pdfReport(html, options = {}) {
      ...
      return this.isElectronAvailable().then(elect => {
          ...
          return (!electron || !options.type ? Promise.resolve(electron) : fetch('http://localhost:16042/getPrinterSetting', {
              method: "POST",
              headers: {
              ...
              .then(data => {
                  ...
                  const paperSize = typePreference ? typePreference.type === "sticker-mut" ? "stickers" : typePreference.format === "A4" ? "a4" : typePreference.format === "A5" ? "a5" : typePreference.format === "DL" ? "presc" : "a4"
  ...
  getUpdatedEdmgStatus(user, patient, requestDate, edmgNiss, edmgOA, genInsOA, genInsAFF, byPassCache = false) {
      ...
      if (cachedEdmgResp && !byPassCache) {
          return Promise.resolve(JSON.parse(cachedEdmgResp));
      }
  ...
  flushObsoleteEdmgStatusCache() {
      ...
      Object.keys(localStorage).filter(k => k.startsWith(patternToLookFor)).forEach(k => {
          ...
          if (moment(dateFormCache).isBefore(moment(), 'day')) {
      ...
      if (inputData && typeof inputData === "object") {
          if (Array.isArray(inputData)) {
              for (let i = 0; i < inputData.length; i++) {
                  this.browseObject(inputData[i], sub + "[" + i + "]");
              }
          } else {
              for (let p in inputData) {
                  if ((/^[a-z_$][a-z0-9_$]*$/i).test(p)) {
                      this.browseObject(inputData[p], sub + "." + p);
                      if (p === this.propName && inputData[p] === this.propValue) {
                          this.variablesPath.push(sub.substr(1));
                      }
                  } else {
                      this.browseObject(inputData[p], sub + "[\"" + p + "\"]");
                  }
      ...
      (inputContent.match(/<col style="([^"]*)">/gi) || []).map(tagMatch => {
          (tagMatch.match(/(width:)(\s*)([\d]*)(px)/gi) || []).map(widthMatch => {
              (widthMatch.match(/(\d*)/gi) || []).map(widthValue => {
                  if (parseInt(widthValue)) {
                      inputContent = inputContent.replace(tagMatch, tagMatch.replace(widthMatch, widthMatch.replace(widthValue, Math.floor(widthValue * ratioFactor))));
                  }
              });
          });
      })
  ...
  hrFormatNumber(inputNumber) {
      return parseFloat(_.trim(inputNumber).replace(',', '.')).toLocaleString("de-DE", {
