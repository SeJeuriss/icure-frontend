    ...
    return html`
        ...
        <div class="list-of-attestations flex col">
            ...
            <div class="flex">
                ...
                <div class="form-boxes flex grow-1 space-between">
                    <div class="flex col space-between full-height">
                        <paper-checkbox checked="{{invoiceIsSent}}">[[localize('sent','Envoyé',language)]]
                        </paper-checkbox>

                        <paper-checkbox checked="{{invoiceType_mutualfund}}">[[localize('inv_mut','Mutual',language)]]
                        </paper-checkbox>
                        <paper-checkbox checked="{{invoiceType_patient}}">[[localize('inv_pat','Patient',language)]]
                        </paper-checkbox>
                        <paper-checkbox checked="{{invoiceType_others}}">[[localize('oth_orgs','Other
                            organisms',language)]]
                        </paper-checkbox>
                    </div>
                    <div class="flex col space-between full-height">
                        ...
                        <paper-checkbox checked="{{sentMediumType_paper}}">[[localize('paper','Papier',language)]]
                        </paper-checkbox>
                    </div>
                    <div class="flex col space-between full-height">
                        <paper-checkbox checked="{{paymentType_cash}}">[[localize('cash','Cash',language)]]
                        </paper-checkbox>
                        <paper-checkbox checked="{{paymentType_debitcard}}">[[localize('card_flow','Card
                            flow',language)]]
                        </paper-checkbox>
                        <paper-checkbox checked="{{paymentType_creditcard}}">[[localize('card_credit','Credit
                            card',language)]]
                        </paper-checkbox>
                        <paper-checkbox checked="{{paymentType_wired}}">[[localize('transfer','Transfert',language)]]
                        </paper-checkbox>
                    </div>
                </div>
            </div>
            ...
            <div class="flex space-between">
                <paper-checkbox class="anon-check" checked="{{isExportAnonyme}}">[[localize('anon_export','Anonymous
                    export',language)]]
                </paper-checkbox>
                <div>[[localize("nr_of_rows","Nombre de lignes", language)]] :
                    [[filteredInvoiceItems.length]]/[[invoiceItems.length]]
                </div>
                <paper-button on-tap="exportToCsv" class="modal-button modal-button--save export-btn">
                    [[localize('export','Export',language)]]
                </paper-button>
            </div>
      ...
      return {
          filter: function (filterValue) {
              ...
              return Promise.all([this.api.patient().findByNameBirthSsinAutoWithUser(this.user, this.user.healthcarePartyId, filterValue, null, null, 100, "asc")]).then(results => {
                  ...
                  const filtered = _.flatten(patList.rows.filter(pat => pat.lastName && pat.lastName !== "" || pat.firstName && pat.firstName !== "").map(pat => ({
                      id: pat.id,
