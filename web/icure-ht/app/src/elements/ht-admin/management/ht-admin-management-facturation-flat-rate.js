  ...
  static get template() {
    return html`
        ...
        <div class="facturation-flat-rate-panel">
            ...
            <div class="add_line">
                <h5>[[localize('flat_rat_tar', 'Flat rate tarifications', language)]]
                    <paper-icon-button class="add-btn" icon="icons:add-circle" on-tap="_addFlatRateTarification"></paper-icon-button>
                </h5>
            </div>
        ...
        <paper-dialog id="addedFlatRateTarificationDialog">
            ...
            <div class="buttons">
                ...
                <paper-button class="modal-button modal-button--save" dialog-confirm="" autofocus="" on-tap="_addTarification">[[localize('save','Save',language)]]
                </paper-button>
            </div>
...
`;
  ...
  _flatRateTarificationDataProvider() {
      ...
      this.api.hcparty().getHealthcareParty(this.user.healthcarePartyId)
          ...
          .then(hcpParent => {
              ...
              this.set('flatRateTarificationTab', _.flatten(flatRateTarifications.map(tar => tar.valorisations.map(val => ({
                  valorisation: val,
                  code: tar.code,
                  label: tar.label,
                  flatRateType: tar.flatRateType
              })))))
  ...
  _addFlatRateTarification() {
      this.set('selectedTarificationMonth', {
          id: parseInt(moment().format('MM')),
          label: this.localize('month_' + parseInt(moment().format('MM')), this.language)
      })
      this.set('selectedTarificationYear', {
  ...
  _addTarification() {
      ...
      if (selectedTar) {
          selectedTar ? selectedTar.valorisations.push({
              startOfValidity: parseInt(startOfValidity),
              reimbursement: this.flatRateTarification.reimbursement.replace(',', '.')
          }) : null
      } else {
          const valorisation = []
          valorisation.push({
