  ...
  static get template() {
    return html`
        ...
        <div class="bank-panel">
            <h4>[[localize('my_pro', 'My profil', language)]] - [[localize('acc_bank_info', 'Bank informations',
                language)]]</h4>
            ...
            <div class="financialInstitutionInformationContainer" id="financialInstitutionInformationContainer">
                ...
                <template is="dom-repeat" items="{{hcp.financialInstitutionInformation}}">
                    ...
                    <div class\$="singleFinancialInstitutionInformationContainer [[_getFiiCssDisplayClass(item, index)]]" id="fiiContainer_{{index}}">
                        <div class="header">
                            <iron-icon icon="icons:payment" class=""></iron-icon>
                            Compte [[_plusOne(index)]]
                            <div class="delete">
                                <paper-icon-button icon="delete" on-tap="_deleteFii" id="fii_{{index}}_delete"></paper-icon-button>
                            </div>
                        </div>
...
`;
  ...
  _getFiiCssDisplayClass(item, index) {
      return (typeof item.bankAccount !== 'undefined' && typeof item.bic !== 'undefined') ? '' : ' fii_hidden ';
  }
  ...
  getVisibleFiiCount() {
      return _.size(this.shadowRoot.querySelectorAll('.singleFinancialInstitutionInformationContainer:not(.fii_hidden):not(#newFiiContainer)')) || 0;
  }
  ...
  _deleteFii(event, fieldObject) {
      ...
      let targetContainer = _.chain(event.path).filter((item) => {
          return /(?:.*)singleFinancialInstitutionInformationContainer(?:.*)/.exec(item.className)
      }).head().value();
