...
export class ProseEditor extends Polymer.Element {
    ...
    templateInstanceNodeSpec: NodeSpec = {
        ...
        parseDOM: [{
            tag: "div.template-instance", getAttrs() {
                return {}
            }
        }]
    }
    ...
    templateNodeSpec: NodeSpec = {
        ...
        toDOM: (node: any) => {
            ...
            return ["div", {
                ...
                'data-ts': renderTimestamp.toString()
            }, 0]
        },
        parseDOM: [{
    ...
    tabNodeSpec: NodeSpec = {
        ...
        parseDOM: [{
            tag: "span.var", getAttrs(dom) {
                return {expr: (dom as HTMLElement).dataset.expr}
            }
        }]
    }
    ...
    editorSchema = new Schema({
        nodes: (schema.spec.nodes as any)
            ...
            .update("paragraph", Object.assign((schema.spec.nodes as any).get("paragraph"), {
                ...
                parseDOM: [{
                    tag: "p", getAttrs(value: HTMLElement) {
                        return {align: value.style && value.style.textAlign || 'inherit'}
                    }
                }],
                toDOM(node: any) {
                    return ["p", {style: "text-align:" + (node.attrs.align || 'inherit')}, 0]
                }
            }))
            .update("heading", Object.assign((schema.spec.nodes as any).get("heading"), {
                ...
                parseDOM: (schema.spec.nodes as any).get("heading").parseDOM.map((r: ParseRule) => Object.assign(r, {
                    getAttrs(value: HTMLElement) {
                        return {
                            level: parseInt(value.tagName.replace(/.+([0-9]+)/, '$1')),
                            align: value.style && value.style.textAlign || 'inherit'
                        }
                    }
                })),
                toDOM(node: any) {
            ...
            .append(tableNodes({
                ...
                cellAttributes: {
                    borderColor: {
                        default: null,
                        getFromDOM(dom) {
                            return (dom as HTMLElement).style.borderColor || null
                        },
                        setDOMAttr(value, attrs) {
                            if (value) attrs.style = (attrs.style || "") + `border-color: ${value};`
                        }
                    },
                    background: {
                        default: null,
    ...
    ready() {
                ...
                if (tr[0] && tr[0].steps && tr[0].steps.length) {
                    ...
                    for (let l = a.depth - 1; l > 0; l--) {
                        ...
                        if (ti.type.name === 'ti') {
                            ...
                            return newState.tr.replaceWith(a.start(l - 1) - 1, a.end(l - 1) + 1, newState.schema.nodes.template.create({
                                expr: tplt.attrs.expr,
                                template: newTemplate,
                                renderTimestamp: tplt.attrs.renderTimestamp
                            }, tplt.content))
                        }
                    }
                }
        ...
        this.editorView = new EditorView(this.$.editor, {
            state: state,
            nodeViews: {
                variable(node, view, getPos) {
                    return new VariableView(node, view, getPos)
                }
            },
                                ...
                                if (selected.node.type === this.editorSchema.nodes.template) {
                                    return visit(
                                        ctxFn(selected.node.attrs.expr, selected.ctx)
                                            .catch(e => {
                                                console.log(`Error during expression ${selected.node.attrs.expr} evaluation`, e);
                                                return null
                                            })
                                            .then((ctx) => {
                                                return tr.replaceWith(selected.pos, selected.pos + selected.node.nodeSize,
                                                    this.editorSchema.nodes.template.create({
                                                            expr: selected.node.attrs.expr,
                                                            template: selected.node.attrs.template,
                                                            renderTimestamp: ts
                                                        },
                                                        ctx && ctx[0] && ctx.map((ctxi: any, i: number) => this.editorSchema.nodes.ti.create({
                                                            idx: i,
                                                            d: ctxi.dataProvider.form().template.id
                                                        },
                                    ...
                                    return visit(
                                        ctxFn(selected.node.attrs.expr, selected.ctx)
                                            ...
                                            .then((ctx) => {
                                                return tr.replaceWith(selected.pos, selected.pos + selected.node.nodeSize, this.editorSchema.nodes.variable.create({
                                                    expr: selected.node.attrs.expr,
                                                    renderTimestamp: ts,
                                                    rendered: ctx && ctx.toString() || " "
                                                }))
                                            })
                                    )
                                }
        ...
        if (this.editorView) {
            ...
            if (this.editorView.dispatch) {
                ...
                if (nodes && nodes.length) {
                    ...
                    this.editorView.dispatch(newState)
                    this.dispatchEvent(new CustomEvent("refresh-context", {
                        bubbles: true,
                        detail: {name: _.get(e, "target.dataVar", "")}
                    }));
