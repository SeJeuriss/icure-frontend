...
export function cellAround($pos: ResolvedPos) {
    for (let d = $pos.depth - 1; d > 0; d--)
        if (($pos.node(d).type.spec as any).tableRole == "row") return $pos.node(0).resolve($pos.before(d + 1))
    return null
}

export function cellWrapping($pos: ResolvedPos) {
    for (let d = $pos.depth - 1; d > 0; d--) {
        const role = ($pos.node(d).type.spec as any).tableRole;
...
export function isInTable(state: EditorState) {
    ...
    for (let d = $head.depth; d > 0; d--) if (($head.node(d).type.spec as any).tableRole == "row") return true
    return false
}
...
function cellNear($pos: ResolvedPos) {
    for (let after = $pos.nodeAfter, pos = $pos.pos; after; after = after.firstChild, pos++) {
        let role = (after.type.spec as any).tableRole
        if (role == "cell" || role == "header_cell") return $pos.doc.resolve(pos)
    }
    for (let before = $pos.nodeBefore, pos = $pos.pos; before; before = before.lastChild, pos--) {
        let role = (before.type.spec as any).tableRole
        if (role == "cell" || role == "header_cell") return $pos.doc.resolve(pos - before.nodeSize)
    }
}

export function pointsAtCell($pos: ResolvedPos) {
    return ($pos.parent.type.spec as any).tableRole == "row" && $pos.nodeAfter
}
...
export function rmColSpan(attrs: { [key: string]: any }, pos: number, n = 1) {
